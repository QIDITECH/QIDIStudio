...
std::map<int, std::string> cli_errors = {
    ...
    {CLI_GCODE_PATH_CONFLICTS, " G-code conflicts detected after slicing. Please make sure the 3mf file can be successfully sliced in the latest QIDI Studio. If the file slices normally in QIDI Studio, try moving the wipe tower further from other models, as we use more conservative parameters for it during upload."},
    {CLI_GCODE_PATH_IN_UNPRINTABLE_AREA, "Found G-code in unprintable area of multi-extruder printers after slicing. Please make sure the 3mf file can be successfully sliced in the latest QIDI Studio."},
    ...
    {CLI_GCODE_PATH_OUTSIDE, "Found G-code outside of the printable area. The issue may be caused by support, wipe tower, brim, or skirt. If the file slices normally in QIDI Studio, try moving the wipe tower further inside the build plate, as we use more conservative parameters for it during upload."},
    {CLI_GCODE_IN_WRAPPING_DETECT_AREA, "Found G-code in the wrapping detect area. Please make sure the 3mf file can be successfully sliced in the latest QIDI Studio."}
};
    ...
    const std::string& default_filament_preset)
{
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format(
            "estimate_filament_preset_name: src_filament_name %1%, base %2%, new_machine_qdt_tag %3%, new_printer_system_name %4%, default_filament_preset %5%") % src_filament_name %base_name %new_machine_qdt_tag %new_printer_system_name %default_filament_preset;
    ...
    // 1. Try exact name match: "<base> @QDT <new_machine_QDT_tag>", and verify compatible_printers
    std::string target_name = base_name + " @QDT " + new_machine_qdt_tag;
...
int CLI::run(int argc, char **argv)
{
    ...
    bool first_file = true, is_qdt_3mf = false, need_arrange = true, has_thumbnails = false, up_config_to_date = false, normative_check = true, duplicate_single_object = false, use_first_fila_as_default = false, minimum_save = false, enable_timelapse = false, has_support = false, estimate_mode = false;
    ...
    if (estimate_mode && load_filaments.empty()) {
        // Derive the new machine's QDT tag and default preset from its default_filament_profile
        std::string new_machine_qdt_tag;
        ...
        if (default_fp_opt && !default_fp_opt->values.empty()) {
            ...
            auto tag_pos = default_filament_preset.find(" @QDT ");
            if (tag_pos != std::string::npos)
                new_machine_qdt_tag = default_filament_preset.substr(tag_pos + 6); // skip " @QDT "
        }
        if (new_machine_qdt_tag.empty()) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << boost::format(" %1%: estimate_mode: cannot determine QDT machine tag from default_filament_profile") % __LINE__;
            record_exit_reson(outfile_dir, CLI_INVALID_PARAMS, 0, cli_errors[CLI_INVALID_PARAMS], sliced_info);
            flush_and_exit(CLI_INVALID_PARAMS);
        }

        std::string filament_full_dir = resources_dir() + "/profiles/QDT/filament_full/";
        ...
        for (int index = 0; index < (int)converted_filaments_system_name.size(); index++) {
            std::string preset_name = estimate_filament_preset_name(
                converted_filaments_system_name[index], new_machine_qdt_tag, filament_full_dir, new_printer_system_name, default_filament_preset);
        ...
        if (!file.empty()) {
            ...
            if ((config_from == "User")||(config_from == "user")) {
                ...
                if (!inherits.empty()) {
                    ...
                    std::string parent_filament_path = resources_dir() + "/profiles/QDT/filament_full/"+inherits+".json";
    ...
    if (is_qdt_3mf && (load_filament_count > 0) && (load_filaments_set.size() == 1) && !estimate_mode)
    {
    ...
    if (machine_switch) {
        ...
        if (new_process_name.empty() && ((current_extruder_count != new_extruder_count) || (current_print_variant_count != new_printer_variant_count))) {
            if (new_default_process_name.empty()) {
                ...
                std::string default_path = resources_dir() + "/profiles/QDT/process_full/";
    ...
    */
    ...
    if (is_qdt_3mf && remove_wrapping_detect) {
        enable_wrapping_detection_option->value = false;
    }

    if (is_qdt_3mf && skirt_per_object_reset) {
    ...
    auto translate_models = [machine_switch, is_qdt_3mf, translate_old, shrink_to_new_bed, old_printable_width, old_printable_depth, old_printable_height, current_printable_width, current_printable_depth, current_printable_height,
    ...
    //1.9.7.52
    if ((!need_arrange) && is_qdt_3mf && !shrink_to_new_bed && (plate_to_slice > 0) && !new_printer_system_name.empty() && (new_printer_system_name!= current_printer_system_name))
    {
...
#endif
                        if (is_qdt_3mf && !no_check && load_filament_count == 0)
                        {
