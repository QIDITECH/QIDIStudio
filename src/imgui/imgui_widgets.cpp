...
bool ImGui::QDTButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::QDTButton(const char* label, const ImVec2& size_arg)
{
    return QDTButtonEx(label, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::QDTImageButtonEx(ImGuiID id,ImTextureID   texture_id,ImTextureID texture_id_hover, ImTextureID texture_id_press, const ImVec2 &size, bool &value,const ImVec2 &uv0,const ImVec2 &uv1,const ImVec2 &padding,const ImVec4 &bg_col,const ImVec4 &tint_col,const ImVec2 &margin)
{
    ImGuiContext &g      = *GImGui;
    ImGuiWindow * window = GetCurrentWindow();
    if (window->SkipItems) return false;
...
bool ImGui::QDTImageButton(ImTextureID user_texture_id,ImTextureID user_texture_id_hover, ImTextureID user_texture_id_press, const ImVec2 &size, bool &value, const ImVec2 &uv0, const ImVec2 &uv1, int frame_padding, const ImVec4 &bg_col, const ImVec4 &tint_col, const ImVec2 &margin)
{
    ImGuiContext &g      = *GImGui;
    ImGuiWindow * window = g.CurrentWindow;
    if (window->SkipItems) return false;
    ...
    const ImVec2 padding = (frame_padding >= 0) ? ImVec2((float) frame_padding, (float) frame_padding) : g.Style.FramePadding;
    return QDTImageButtonEx(id, user_texture_id, user_texture_id_hover, user_texture_id_press, size,value, uv0, uv1, padding, bg_col, tint_col, margin);
}
...
bool ImGui::QDTCheckbox(const char *label, bool *v)
{
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems) return false;
...
bool ImGui::QDTRadioButton(const char *label, bool active)
{
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems) return false;
...
void ImGui::QDTProgressBar(float fraction, const ImVec2 &size_arg, const char *overlay)
{
    ImGuiWindow *window = GetCurrentWindow();
...
void ImGui::QDTProgressBar2(float fraction, const ImVec2 &size_arg, const char *overlay)
{
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems) return;
...
bool ImGui::QDTBeginCombo(const char *label, const char *preview_value, ImGuiComboFlags flags)
{
    // Always consume the SetNextWindowSizeConstraint() call in our early return paths
    ImGuiContext &g = *GImGui;
    bool has_window_size_constraint = (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint) != 0;
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton)) {
        ...
        if (value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x)
            QDTRenderArrow(window->DrawList, ImVec2(frame_bb.Min.x + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y), text_col, ImGuiDir_Down, 1.0f);
    }
    if (!(flags & ImGuiComboFlags_NoPreview))
...
bool ImGui::QDTDragScalar(const char *label, ImGuiDataType data_type, void *p_data, float v_speed, const void *p_min, const void *p_max, const char *format, ImGuiSliderFlags flags)
{
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems) return false;
...
bool ImGui::QDTDragFloat(const char *label, float *v, float v_speed, float v_min, float v_max, const char *format, ImGuiSliderFlags flags)
{
...
bool ImGui::QDTSliderScalarIn(const char *label, ImGuiDataType data_type, void *p_data, const void *p_min, const void *p_max, const char *format, ImGuiSliderFlags flags)
{
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems) return false;
...
bool ImGui::QDTSliderScalar(const char *label, ImGuiDataType data_type, void *p_data, const void *p_min, const void *p_max, const char *format, ImGuiSliderFlags flags)
{
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems) return false;
...
bool ImGui::QDTSliderFloat(const char *label, float *v, float v_min, float v_max, const char *format, float power)
{
    ...
    return QDTSliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, format, slider_flags);
}
...
bool ImGui::QDTInputScalar(const char *label, ImGuiDataType data_type, void *p_data, const void *p_step, const void *p_step_fast, const char *format, ImGuiInputTextFlags flags)
{
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems) return false;
...
bool ImGui::QDTInputDouble(const char *label, double *v, double step, double step_fast, const char *format, ImGuiInputTextFlags flags)
{
    ImGui::PushStyleColor(ImGuiCol_BorderActive, ImVec4(0.27f, 0.47f, 0.98f, 1.00f));   // change color
    flags |= ImGuiInputTextFlags_CharsScientific;
    bool qdt_input_scalar = QDTInputScalar(label, ImGuiDataType_Double, (void *) v, (void *) (step > 0.0 ? &step : NULL), (void *) (step_fast > 0.0 ? &step_fast : NULL), format,flags);
    ImGui::PopStyleColor(1);
    return qdt_input_scalar;
}
...
bool ImGui::QDTSelectable_LeftImage(const char *label, bool selected, ImTextureID texture_id,ImGuiSelectableFlags flags, const ImVec2 &size_arg)
{
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    if (texture_id) {
        ...
        ImGui::GetWindowDrawList()->AddImage(texture_id, bb_image.Min, bb_image.Max);
    }
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.LastItemStatusFlags);
    return pressed;
}

bool ImGui::QDTImageSelectable(ImTextureID user_texture_id, const ImVec2& size_arg, const ImVec2& font_size, int font_line, const ImVec4& tint_col, const ImVec2& uv0, const ImVec2& uv1, bool selected)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    PushID((void*)(intptr_t)user_texture_id);
    const ImGuiID id = window->GetID("#QDTImageSelectable");
    PopID();
...
bool ImGui::QDTSelectable(const char *label, bool selected, ImGuiSelectableFlags flags, const ImVec2 &size_arg)
{
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems) return false;
...
bool ImGui::QDTMenuItem(const char* label, const char* shortcut, bool selected, bool enabled, float size_arg_y)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        window->DC.CursorPos.x += IM_FLOOR(style.ItemSpacing.x * 0.5f);
        PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(style.ItemSpacing.x * 2.0f, style.ItemSpacing.y));
        pressed = QDTSelectable(label, selected, flags, ImVec2(w, size_arg_y));
        PopStyleVar();
        window->DC.CursorPos.x += IM_FLOOR(style.ItemSpacing.x * (-1.0f + 0.5f)); // -1 spacing to compensate the spacing added when Selectable() did a SameLine(). It would also work to call SameLine() ourselves after the PopStyleVar().
    }
    else
    {
        ...
        float extra_w = ImMax(0.0f, GetContentRegionAvail().x - min_w);
        pressed = QDTSelectable(label, false, flags | ImGuiSelectableFlags_SpanAvailWidth | ImGuiComboFlags_NoArrowButton, ImVec2(min_w, size_arg_y));
        if (shortcut_w > 0.0f)
        {
            PushStyleColor(ImGuiCol_Text, g.Style.Colors[ImGuiCol_TextDisabled]);
