...
// QDS: generate brim area by objs
...
// QDS: the brims of different objs will not overlapped with each other, and are stored by objs and by extruders
    ...
    for (unsigned int extruderNo : print.extruders()) {
        ...
        for (const auto &objectWithExtruder : objPrintVec) {
            ...
            if (support_material_extruder == extruderNo && brimToWrite.at(object->id()).sup) {
                if (!object->support_layers().empty()) {
                    for (const Polygon& support_contour : object->support_layers().front()->support_fills.polygons_covered_by_spacing()) {
                        //QDS: no brim offset for supports
                        if ((brim_type == BrimType::btOuterOnly || brim_type == BrimType::btOuterAndInner || brim_type == BrimType::btAutoBrim) && is_top_outer_brim)
...
//QDS maximum temperature difference from print object class
...
//QDS adhesion coefficients from print object class
double getadhesionCoeff(const PrintObject* printObject)
{
   ...
   def->enum_values.push_back("TPU");//QDS
...
// QDS: second moment of area of a polygon
...
// QDS: properties of an expolygon
struct ExPolyProp
{
    double aera = 0;
    Vec2d  centroid;
...
// QDS: second moment of area of an expolyon
...
// QDS: second moment of area of expolygons
...
//QDS: config brimwidth by volumes
    ...
    std::vector<unsigned int>& printExtruders)
{
    ...
    auto save_polygon_if_is_inner_island = [](const Polygons& holes_area, const Polygon& contour, int& hole_index) {
        for (size_t i = 0; i < holes_area.size(); i++) {
            ...
            if (diff_ex(contour_polys, { holes_area[i] }).empty()) {
                // QDS: this is an inner island inside holes_area[i], save
                hole_index = i;
                return;
            }
        }
    ...
    for (unsigned int extruderNo : printExtruders) {
        ...
        for (const auto& objectWithExtruder : objPrintVec) {
            ...
            if (objectWithExtruder.second == extruderNo && brimToWrite.at(object->id()).obj) {
                ...
                //QDS: collect holes area which is used to limit the brim of inner island
                Polygons holes_area;
                for (const ExPolygon& ex_poly : object->layers().front()->lslices)
                    polygons_append(holes_area, ex_poly.holes);

                // QDS: brims are generated by volume groups
                for (const auto& volumeGroup : object->firstLayerObjGroups()) {
                    ...
                    for (auto& volumeID : volumeGroup.volume_ids) {
                        ...
                        //QDS: support shared object logic
                    ...
                    for (const ExPolygon& ex_poly : volumeGroup.slices) {
                        // QDS: additional brim width will be added if part's adhension area is too small and brim is not generated
                        ...
                        //QDS: brim width should be limited to the 1.5*boundingboxSize of a single polygon.
                        if (has_brim_auto) {
    ...
    // printable areas ¨C sacrificing some exclusive-zone area to guarantee correctness.
    ...
    brim_area.clear();
    for (const PrintObject* object : print.objects()) {
        // QDS: brim should be contacted to at least one object's island or brim area
        if (brimAreaMap.find(object->id()) != brimAreaMap.end()) {
...
// QDS: this function is used to generate brim for inner island inside holes
    ...
    ExtrusionEntityCollection &brim, ExPolygons &islands_area_ex)
{
    ...
    for (const PrintObject* object : top_level_objects_with_brim) {
        const BrimType brim_type = object->config().brim_type.value;
        // QDS: don't need to handle this object if hasn't enabled outer_brim
        if (brim_type == BrimType::btNoBrim)
            continue;

        //QDS: 1 collect holes area which is used to limit the brim of inner island
        Polygons holes_area;
        for (const ExPolygon& ex_poly : object->layers().front()->lslices)
            polygons_append(holes_area, ex_poly.holes);


        //QDS: 2 get the island polygons inside holes, saved as map
        ...
        //QDS: 3 generate loops, only save part of loop which inside hole
        const float    brim_offset = scale_(object->config().brim_object_gap.value);
        ...
        if (brim_type == BrimType::btInnerOnly) {
            ...
            for (auto it = hole_island_pair.begin(); it != hole_island_pair.end(); it++) {
                ...
                // QDS: Reduce down to the ordered list of polylines.
                ...
                //QDS: save all inner island and inner island brim area here, which is necesary if generate inner brim for holes
...
//QDS: the brim are generated one by one, and sorted by objs/supports and extruders
static void make_inner_island_brim(const Print& print, const ConstPrintObjectPtrs& top_level_objects_with_brim,
    std::map<ObjectID, ExPolygons>& innerbrimAreaMap,
    std::map<ObjectID, ExPolygons>& innerSupportBrimAreaMap,
                            ...
                            ExtrusionEntityCollection     &brim)
{
    ...
    //QDS: generate brim for inner island first
    ...
    //QDS: brim of hole must not overlap with inner island and inner island brim
...
// QDS: generate inner brim by objs
...
//QDS: generate out brim by offseting ExPolygons 'islands_area_ex'
...
//QDS: a function creates the ExtrusionEntityCollection from the brim area defined by ExPolygons
ExtrusionEntityCollection makeBrimInfill(const ExPolygons& singleBrimArea, const Print& print, const Polygons& islands_area) {
    ...
    //QDS: finally apply the plate offset which may very large
...
//QDS: an overload of the orignal brim generator that generates the brim by obj and by extruders
    ...
    std::vector<unsigned int>& printExtruders)
{
    ...
    // QDS: Find boundingbox of the first layer
