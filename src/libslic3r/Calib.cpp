...
std::string CalibPressureAdvanceLine::print_pa_lines(double start_x, double start_y, double start_pa, double step_pa, int num)
{
    ...
    for (int i = 0; i < num; ++i) {
        gcode << writer.set_pressure_advance(start_pa + i * step_pa, m_is_qdt_bowden);
    ...
    gcode << writer.set_pressure_advance(m_is_qdt_bowden ? 0.4 : 0.0, m_is_qdt_bowden);
...
void CalibPressureAdvancePattern::generate_custom_gcodes(const DynamicPrintConfig &config, bool is_qdt_machine, Model &model, const Vec3d &origin)
{
    ...
    gcode << m_writer.set_pressure_advance(m_is_qdt_bowden ? 0.4 : m_params.start, m_is_qdt_bowden);
    ...
    for (int i = 0; i < m_num_layers; ++i) {
        ...
        if (i == 0)
            gcode << m_writer.set_pressure_advance(m_params.start, m_is_qdt_bowden);
        ...
        for (int j = 0; j < num_patterns; ++j) {
            ...
            gcode << m_writer.set_pressure_advance(m_params.start + (j * m_params.step), m_is_qdt_bowden);
