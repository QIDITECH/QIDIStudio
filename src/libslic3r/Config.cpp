    ...
    for (auto category : categories) {
        ...
        for (const auto& opt : this->options) {
            ...
            for (auto& arg : cli_args) {
                ...
                //QDS: refine the print help format
...
bool ConfigBase::set_deserialize_nothrow(const t_config_option_key &opt_key_src, const std::string &value_src, ConfigSubstitutionContext& substitutions_ctxt, bool append)
{
    ...
    if (opt_key.empty()) {
        // Ignore the option.
        //QDS: record these options, keep only one repeated opt_key
    ...
    try {
        ...
        for (auto it = j.begin(); it != j.end(); it++) {
            if (boost::iequals(it.key(),QDT_JSON_KEY_VERSION)) {
                key_values.emplace(QDT_JSON_KEY_VERSION, it.value());
            }
            else if (boost::iequals(it.key(), QDT_JSON_KEY_IS_CUSTOM)) {
                //skip it
            }
                ...
                key_values.emplace(QDT_JSON_KEY_URL, it.value());
            }
            else if (boost::iequals(it.key(), QDT_JSON_KEY_TYPE)) {
                key_values.emplace(QDT_JSON_KEY_TYPE, it.value());
            }
            else if (boost::iequals(it.key(), QDT_JSON_KEY_SETTING_ID)) {
                key_values.emplace(QDT_JSON_KEY_SETTING_ID, it.value());
            }
            else if (boost::iequals(it.key(), QDT_JSON_KEY_FILAMENT_ID)) {
                key_values.emplace(QDT_JSON_KEY_FILAMENT_ID, it.value());
            }
            else if (boost::iequals(it.key(), QDT_JSON_KEY_FROM)) {
                key_values.emplace(QDT_JSON_KEY_FROM, it.value());
            }
            else if (boost::iequals(it.key(), QDT_JSON_KEY_DESCRIPTION)) {
                key_values.emplace(QDT_JSON_KEY_DESCRIPTION, it.value());
            }
            else if (boost::iequals(it.key(), QDT_JSON_KEY_INSTANTIATION)) {
                key_values.emplace(QDT_JSON_KEY_INSTANTIATION, it.value());
            }
            else if (!load_inherits_to_config && boost::iequals(it.key(), QDT_JSON_KEY_INHERITS)) {
                key_values.emplace(QDT_JSON_KEY_INHERITS, it.value());
            }
            else if (!load_inherits_to_config && boost::iequals(it.key(), QDT_JSON_KEY_INCLUDES)) {
                key_values.emplace(QDT_JSON_KEY_INCLUDES, it.value().dump());
            }
            else {
                ...
                if (it.value().is_string()) {
                    ...
                    if (opt_key == "support_type") {
                        ...
                        //QDS: check wall_infill order to decide if it be different and append to diff_setting_append
...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Look for Slic3r or QIDIStudio header.
    ...
    {
        //QDS
        std::string qidislicer_gcode_header = "; ";
        qidislicer_gcode_header += SLIC3R_APP_NAME;
        ...
        while (std::getline(ifs, header)) {
            ...
            // QDS
            if (strncmp(qidislicer_gcode_header.c_str(), line_c, strlen(qidislicer_gcode_header.c_str())) == 0) {
                header_found = true;
                break;
            }
        }
...
void ConfigBase::save_to_json(const std::string &file, const std::string &name, const std::string &from, const std::string &version) const
{
    json j;
    //record the headers
    j[QDT_JSON_KEY_VERSION] = version;
    j[QDT_JSON_KEY_NAME] = name;
    j[QDT_JSON_KEY_FROM] = from;
...
bool DynamicConfig::read_cli(int argc, const char* const argv[], t_config_option_keys* extra, t_config_option_keys* keys)
{
    ...
    for (int i = 1; i < argc; ++ i) {
        ...
        bool no = false;
        if (it == opts.end()) {
            //QDS: don't use 'no-' for boolean options
