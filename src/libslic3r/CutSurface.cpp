...
using QDS = std::vector<BoundingBoxf3>;
/// <summary>
...
QDS create_qds(const VCutAOIs &cuts, const CutMeshes &cut_models);
...
/// <param name="qds">All bounding boxes from VCutAOIs</param>
...
std::vector<BoundingBoxf3> priv::create_qds(const VCutAOIs        &cuts,
                                            const CutMeshes &cut_models)
{
    size_t count = 0;
    for (const CutAOIs &cut : cuts) count += cut.size();

    std::vector<BoundingBoxf3> qds;
    qds.reserve(count);
    for (size_t model_index = 0; model_index < cut_models.size(); ++model_index) {
        ...
        for (size_t cut_index = 0; cut_index < cutAOIs.size(); ++cut_index) {
            ...
            qds.push_back(bounding_box(cut, cut_model));
        }
    }
    return qds;
}
                               ...
                               const ModelCut2index &m2i)
{
    ...
    for (size_t bb_index = start; bb_index < end; bb_index++)
        if (bb.intersects(qds[bb_index])) return true;
    return false;
}
                                       ...
                                       const Project3d     &projection)
{
    ...
    std::vector<BoundingBoxf3> qds = create_qds(cuts, cut_models);
        ...
        for (size_t cut_index = 0; cut_index < model_cuts.size(); ++cut_index, ++index) {
            ...
            for (size_t model_index2 = 0; model_index2 < models.size(); ++model_index2) {
                ...
                for (SurfacePatchEx &patch_ex : aoi_patches) {
                    SurfacePatch &patch = patch_ex.patch;
                    if (has_bb_intersection(patch.bb, model_index2, qds, m2i) &&
