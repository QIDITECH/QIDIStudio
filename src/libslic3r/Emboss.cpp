...
PolygonPoints Emboss::sample_slice(const TextLine &slice, const BoundingBoxes &qds, double scale)
{
    // find BB in center of line
    size_t first_right_index = 0;
    for (const BoundingBox &bb : qds)
    ...
    PolygonPoints samples(qds.size());
    int32_t shapes_x_cursor = 0;

    PolygonPoint cursor = slice.start; //copy

    auto create_sample = [&] //polygon_cursor, &polygon_line_index, &line_qds, &shapes_x_cursor, &shape_scale, &em_2_polygon, &line, &offsets]
    ...
    bool is_reverse = true;
    for (size_t index = first_right_index; index < qds.size(); ++index)
        samples[index] = create_sample(qds[index], is_reverse);

    // calc transformation for letters on the Left side from center
    if (first_right_index < qds.size()) {
        shapes_x_cursor = qds[first_right_index].center().x();
