...
namespace Emboss
{
    ...
    /// <param name="qds">Bounding boxes of letter on one line[in font scales]</param>
    /// <param name="scale">Scale for qds (after multiply bb is in milimeters)</param>
    /// <returns>Sampled polygon by bounding boxes</returns>
    PolygonPoints sample_slice(const TextLine &slice, const BoundingBoxes &qds, double scale);
