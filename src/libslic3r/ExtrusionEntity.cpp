...
// QDS: slowdown slope path seep to get better seam
...
//QDS: only check angle of seam point while the seam has been decided.
 ...
 : ExtrusionLoop(role)
{
    ...
    for (; path != original_paths.end() && remaining_length > 0; ++path) {
        ...
        if (path_len > remaining_length) {
            ...
            // QDS: protection for accuracy issues
