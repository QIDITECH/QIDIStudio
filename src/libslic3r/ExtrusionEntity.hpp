...
public:
    ...
    virtual bool can_sort() const { return true; }//QDS: only used in ExtrusionEntityCollection
...
public:
    ...
    ExtrusionPath(const ExtrusionPath &rhs)
        ...
        , width(rhs.width)
        , height(rhs.height)
        , smooth_speed(rhs.smooth_speed)    //1.9.5
    ...
    //QDS: add new simplifing method by fitting arc
...
//QDS: a kind of special extrusion path has start and end wiping for half spacing
