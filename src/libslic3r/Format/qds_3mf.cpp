...
bool qds_has_attribute_value_int(const char **attributes, unsigned int attributes_size, const char *attribute_key)
{
    if (const char *text = qds_get_attribute_value_charptr(attributes, attributes_size, attribute_key); text != nullptr)
        return true;
    return false;
...
//        model.adjust_min_z();
    ...
    void _QDS_3MF_Importer::_extract_filament_sequence_from_archive(mz_zip_archive& archive, const mz_zip_archive_file_stat &stat)
    {
        if(stat.m_uncomp_size <=0){
            add_error("Error while reading layer config ranges data to buffer");
            return;
    ...
    bool _QDS_3MF_Importer::_handle_start_object(const char** attributes, unsigned int num_attributes)
    {
        ...
        if (qds_is_valid_object_type(object_type)) {
            ...
            if (qds_has_attribute_value_int(attributes, num_attributes, PID_ATTR)) {
                m_curr_object->pid = qds_get_attribute_value_int(attributes, num_attributes, PID_ATTR);
            }
            if (qds_has_attribute_value_int(attributes, num_attributes, PINDEX_ATTR)) {
                m_curr_object->pindex = qds_get_attribute_value_int(attributes, num_attributes, PINDEX_ATTR);
            }
        }
    ...
    bool _QDS_3MF_Importer::_handle_start_triangle(const char** attributes, unsigned int num_attributes)
    {
        ...
        if (m_curr_object) {
            ...
            m_curr_object->geometry.custom_fuzzy_skin.push_back(qds_get_attribute_value_string(attributes, num_attributes, CUSTOM_FUZZY_SKIN_ATTR));
            ...
            if (!m_is_qdt_3mf) {
                ...
                if (qds_has_attribute_value_int(attributes, num_attributes, PID_ATTR)) {
                    color_info.pid = qds_get_attribute_value_int(attributes, num_attributes, PID_ATTR);
                }
                else {
                ...
                if (color_info.pid >= 0) {
                    ...
                    for (int idx = 0; idx < 3; ++idx) {
                        if (qds_has_attribute_value_int(attributes, num_attributes, color_attrs[idx])) {
                            color_info.indices[idx] = qds_get_attribute_value_int(attributes, num_attributes, color_attrs[idx]);
                        }
    ...
    bool _QDS_3MF_Importer::_handle_start_config_filament(const char** attributes, unsigned int num_attributes)
    {
        if (m_curr_plater) {
            ...
            std::string group_id = qds_get_attribute_value_string(attributes,num_attributes, FILAMENT_NOZZLE_GROUP_ID_TAG);
            std::string nozzle_diameter = qds_get_attribute_value_string(attributes, num_attributes, FILAMENT_NOZZLE_DIAMETER_TAG);
            std::string volume_type = qds_get_attribute_value_string(attributes, num_attributes, FILAMENT_NOZZLE_VOLUME_TYPE_TAG);
    ...
    bool _QDS_3MF_Importer::_handle_start_config_nozzle(const char** attributes, unsigned int num_attributes)
    {
        if (m_curr_plater) {
            // id="0" extruder_id="1" nozzle_diameter="0.4" volume_type="nvtNormal"
            std::string id = qds_get_attribute_value_string(attributes, num_attributes, "id");
            std::string extruder_id = qds_get_attribute_value_string(attributes, num_attributes, "extruder_id");
            std::string nozzle_diameter= qds_get_attribute_value_string(attributes, num_attributes, "nozzle_diameter");
            std::string volume_type = qds_get_attribute_value_string(attributes, num_attributes, "volume_type");
    ...
    bool _QDS_3MF_Importer::_handle_end_config_nozzle()
    {
        // do nothing
        return true;
    }
    ...
    bool _QDS_3MF_Importer::_handle_start_text_info_item(const char **attributes, unsigned int num_attributes)
    {
        ...
        text_info.m_font_name = xml_unescape(qds_get_attribute_value_string(attributes, num_attributes, FONT_NAME_ATTR));
        ...
        if (qds_has_attribute_value_int(attributes, num_attributes, SURFACE_TYPE)) {
            text_info.m_surface_type = qds_get_attribute_value_int(attributes, num_attributes, SURFACE_TYPE);
        } else {//old version
    ...
    bool _QDS_3MF_Importer::_generate_volumes_new(ModelObject& object, const std::vector<Component> &sub_objects, const ObjectMetadata::VolumeMetadataList& volumes, ConfigSubstitutionContext& config_substitutions)
    {
        ...
        for (unsigned int index = 0; index < sub_objects.size(); index++)
        {
            ...
            if (!m_is_qdt_3mf && sub_object->geometry.triangle_colors.size() == triangles_count) {
    ...
    bool _QDS_3MF_Importer::ObjectImporter::_handle_object_start_object(const char** attributes, unsigned int num_attributes)
    {
        ...
        if (qds_is_valid_object_type(object_type)) {
            ...
            if (qds_has_attribute_value_int(attributes, num_attributes, PINDEX_ATTR))
                current_object->pindex = qds_get_attribute_value_int(attributes, num_attributes, PINDEX_ATTR);
        }
    ...
    bool _QDS_3MF_Importer::ObjectImporter::_handle_object_start_triangle(const char** attributes, unsigned int num_attributes)
    {
        ...
        if (current_object) {
            ...
            current_object->geometry.custom_fuzzy_skin.push_back(qds_get_attribute_value_string(attributes, num_attributes, CUSTOM_FUZZY_SKIN_ATTR));
    ...
    bool _QDS_3MF_Exporter::_add_model_file_to_archive(const std::string& filename, mz_zip_archive& archive, const Model& model, ObjectToObjectDataMap& objects_data, Export3mfProgressFn proFn, QDTProject* project) const
    {
...

        {
            ...
            if (!sub_model) {
                ...
                metadata_item_map[QDT_PROFILE_TITLE_TAG]        = xml_escape(profile_title);
                metadata_item_map[QDT_PROFILE_COVER_TAG]        = xml_escape(profile_cover);
                metadata_item_map[QDT_PROFILE_DESCRIPTION_TAG]  = xml_escape(profile_description);
...
bool _QDS_3MF_Exporter::_add_filament_sequence_file_to_archive(mz_zip_archive& archive, const PlateDataPtrs& plate_data_list)
{
