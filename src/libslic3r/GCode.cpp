        ...
        for (; print_object_instance_sequential_active != print_object_instances_ordering.end(); ++ print_object_instance_sequential_active) {
            ...
            if ((initial_extruder_id = tool_ordering.first_extruder()) != static_cast<unsigned int>(-1)) {
                ...
                if (tool_ordering.all_extruders().size() > 1 && print.config().filament_is_support.get_at(initial_extruder_id)) {
                    ...
                    //QDS: find the non-support filament extruder of object
    ...
    const bool                               prime_extruder)
{
    ...
    for (unsigned int extruder_id : layer_tools.extruders)
    {
        ...
        // QDS: ordering instances by extruder
...
std::string GCode::_extrude(const ExtrusionPath &path, std::string description, double speed, bool is_first_slope)
{
        ...
        if (path.role() == erPerimeter) {
            ...
            if (m_config.detect_overhang_wall && m_config.smooth_speed_discontinuity_area && path.smooth_speed != 0)
                ...
                if (m_resonance_avoidance) {
                    if (new_speed > 0) {
                        if (new_speed > m_config.max_resonance_avoidance_speed.value)
                            m_resonance_avoidance = false;
                    }
                }
                speed = new_speed == 0.0 ? speed : new_speed;
    ...
    if (m_resonance_avoidance && path.role() == erExternalPerimeter) {
        if (speed <= m_config.max_resonance_avoidance_speed.value) {
