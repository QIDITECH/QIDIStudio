...
void GCodeEditor::reset(const Vec3d &position)
{
    ...
    m_current_pos[4] = float(m_config.travel_speed.get_at(get_process_config_idx(m_config, m_current_extruder)));
    m_fan_speed = -1;
                                                                     ...
                                                                     bool                            join_z_smooth)
{
    ...
    for (; *line_start != 0; line_start = line_end)
    {
        ...
        //y72
        else if (boost::starts_with(sline, "TIMELAPSE_TAKE_FRAME")) {
            ;
        } 
...
#define EXTRUDER_CONFIG(OPT) m_config.OPT.get_at(m_current_extruder)
        ...
        // QDS: Independent control for additional fan
