...
bool GCodeProcessor::s_IsQDTPrinter = true;
...
void GCodeProcessor::TimeMachine::calculate_time(size_t keep_last_n_blocks, float additional_time, ExtrusionRole target_role, block_handler_t block_handler)
{
    ...
    for (size_t i = 0; i < n_blocks_process; ++i) {
        ...
        //QDS: don't calculate travel of start gcode into travel time
...
void GCodeProcessor::TimeProcessor::post_process(const std::string& filename, std::vector<GCodeProcessorResult::MoveVertex>& moves, std::vector<size_t>& lines_ends, const TimeProcessContext& context)
{
    ...
    */
    ...
    // collect the position to insert remaining filament changes
    {
        ...
        for (const auto& fb : filament_blocks) {
            int extruder_id = context.nozzle_group_result.get_extruder_id(fb.filament_id);
            if (curr_filament != -1 && curr_filament != fb.filament_id) {
...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    //QDS: QIDIStudio is also "qidi". Otherwise the time estimation didn't work.
    //FIXME: Workaround and should be handled when do removing-qidi
    { EProducer::QIDIStudio, SLIC3R_APP_NAME },
    { EProducer::QIDIStudio, "generated by QIDIStudio" },
...
void GCodeProcessor::register_commands()
{
    ...
    std::unordered_map<std::string, CommandProcessor::command_handler_t> command_handler_list = {
        ...
        {"M400", [this](const GCodeReader::GCodeLine& line) { process_M400(line); }}, // QDS delay
...
void GCodeProcessor::apply_config(const DynamicPrintConfig& config)
{
    ...
    if (extruder_offset != nullptr) {
        //QDS: for single extruder multi material, only use the offset of first extruder
...
void GCodeProcessor::process_file(const std::string& filename, std::function<void()> cancel_callback)
{
    ...
    // parse the gcode file to detect its producer
    {
        ...
        // if the gcode was produced by QIDIStudio,
        // extract the config from it
        if (m_producer == EProducer::QIDIStudio || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r || m_producer == EProducer::QIDISlicer || m_producer == EProducer::BambuStudio) {
            DynamicPrintConfig config;
            config.apply(FullPrintConfig::defaults());
            // Silently substitute unknown values by new ones for loading configurations from QIDIStudio's own G-code.
...
//QDS: get last z position from gcode
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    if (!producers_enabled || m_producer == EProducer::QIDIStudio) {
        // height tag
...
bool GCodeProcessor::process_producers_tags(const std::string_view comment)
{
    ...
    case EProducer::QIDIStudio: { return process_qidislicer_tags(comment); }
...
void GCodeProcessor::process_G1(const GCodeReader::GCodeLine& line)
{
    ...
    for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i) {
        ...
        //QDS: calculeta enter and exit direction
        ...
        if ((prev.exit_direction(0) != 0.0f || prev.exit_direction(1) != 0.0f) &&
            ...
            //QDS: don't need to consider limitation of centripetal acceleration
    ...
    if (m_seams_detector.is_active()) {
        ...
        if (type == EMoveType::Extrude && m_extrusion_role == erExternalPerimeter && !m_seams_detector.has_first_vertex()) {
            //QDS: m_result.moves.back().position has plate offset, must minus plate offset before calculate the real seam position
...
// QDS: this function is absolutely new for G2 and G3 gcode
void  GCodeProcessor::process_G2_G3(const GCodeReader::GCodeLine& line)
{
    ...
    auto move_type = [this](const float& delta_E) {
     ...
     auto arc_interpolation = [this](const Vec3f& start_pos, const Vec3f& end_pos, const Vec3f& center_pos, const bool is_ccw) {
         ...
         //QDS: radius is too small to draw
    ...
    //QDS: enable processing of lines M201/M203/M204/M205
    m_time_processor.machine_envelope_processing_enabled = true;

    //QDS: get axes positions from line
    for (unsigned char a = X; a <= E; ++a) {
        m_end_position[a] = absolute_position((Axis)a, line);
    }
    //QDS: G2 G3 line but has no I and J axis, invalid G code format
    if (!line.has(I) && !line.has(J))
        return;
    //QDS: P mode, but xy position is not same, or P is not 1, invalid G code format
    if (line.has(P) &&
    ...
    //QDS: G2 is CW direction, G3 is CCW direction
    const std::string_view cmd = line.cmd();
    m_move_path_type = (::atoi(&cmd[1]) == 2) ? EMovePathType::Arc_move_cw : EMovePathType::Arc_move_ccw;
    //QDS: get arc length,interpolation points and radian in X-Y plane
    ...
    //QDS: Attention! arc_onterpolation does not support P mode while P is not 1.
    ...
    //QDS: updates feedrate from line, if present
    if (line.has_f())
        m_feedrate = line.f() * MMMIN_TO_MMSEC;

    //QDS: calculates movement deltas
    AxisCoords delta_pos;
    for (unsigned char a = X; a <= E; ++a) {
        delta_pos[a] = m_end_position[a] - m_start_position[a];
    }

    //QDS: no displacement, return
    ...
    if (type == EMoveType::Extrude) {
        ...
        if(m_extrusion_role == ExtrusionRole::erSupportMaterial || m_extrusion_role == ExtrusionRole::erSupportMaterialInterface || m_extrusion_role ==ExtrusionRole::erSupportTransition)
            ...
            //QDS: save wipe tower volume to the cache
            m_used_filaments.increase_wipe_tower_caches(volume_extruded_filament);
        }
        else {
            //QDS: save extruded volume to the cache
            m_used_filaments.increase_model_caches(volume_extruded_filament);
        }
        //QDS: volume extruded filament / tool displacement = area toolpath cross section
        m_mm3_per_mm = area_toolpath_cross_section;
#if ENABLE_GCODE_VIEWER_DATA_CHECKING
        m_mm3_per_mm_compare.update(area_toolpath_cross_section, m_extrusion_role);
#endif // ENABLE_GCODE_VIEWER_DATA_CHECKING
        ...
        if (m_forced_width > 0.0f)
            ...
            //QDS: cross section: rectangle
            m_width = delta_pos[E] * static_cast<float>(M_PI * sqr(1.05f * filament_radius)) / (delta_xyz * m_height);
        else if (m_extrusion_role == erBridgeInfill || m_extrusion_role == erNone)
            //QDS: cross section: circle
            m_width = static_cast<float>(m_result.filament_diameters[filament_id]) * std::sqrt(delta_pos[E] / delta_xyz);
        else
            //QDS: cross section: rectangle + 2 semicircles
            m_width = delta_pos[E] * static_cast<float>(M_PI * sqr(filament_radius)) / (delta_xyz * m_height) + static_cast<float>(1.0 - 0.25 * M_PI) * m_height;

        if (m_width == 0.0f)
            m_width = DEFAULT_TOOLPATH_WIDTH;

        //QDS: clamp width to avoid artifacts which may arise from wrong values of m_height
        m_width = std::min(m_width, std::max(2.0f, 4.0f * m_height));
    ...
    for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i) {
        ...
        // QDS: calculates block cruise feedrate
        ...
        for (unsigned char a = X; a <= E; ++a) {
            if (a == X || a == Y)
                //QDS: use resultant feedrate in x-y plane
        ...
        //QDS: calculates block acceleration
        ...
        //QDS: calculates block exit feedrate
        for (unsigned char a = X; a <= E; ++a) {
            float axis_max_jerk = get_axis_max_jerk(static_cast<PrintEstimatedStatistics::ETimeMode>(i), static_cast<Axis>(a));
            if (curr.abs_axis_feedrate[a] > axis_max_jerk)
                curr.safe_feedrate = std::min(curr.safe_feedrate, axis_max_jerk);
        ...
        //QDS: calculates block entry feedrate
        static const float PREVIOUS_FEEDRATE_THRESHOLD = 0.0001f;
        float vmax_junction = curr.safe_feedrate;
        if (!blocks.empty() && prev.feedrate > PREVIOUS_FEEDRATE_THRESHOLD) {
            ...
            //QDS: Not coasting. The machine will stop and start the movements anyway, better to start the segment from start.
            if ((prev.safe_feedrate > vmax_junction_threshold) && (curr.safe_feedrate > vmax_junction_threshold))
                vmax_junction = curr.safe_feedrate;
        }
        ...
        //QDS: calculates block trapezoid
        block.calculate_trapezoid();

        //QDS: updates previous
        prev = curr;
    ...
    if (m_seams_detector.is_active()) {
        //QDS: check for seam starting vertex
        if (type == EMoveType::Extrude && m_extrusion_role == erExternalPerimeter && !m_seams_detector.has_first_vertex()) {
        ...
        //QDS: check for seam ending vertex and store the resulting move
        else if ((type != EMoveType::Extrude || (m_extrusion_role != erExternalPerimeter && m_extrusion_role != erOverhangPerimeter)) && m_seams_detector.has_first_vertex()) {
            auto set_end_position = [this](const Vec3f& pos) {
                m_end_position[X] = pos.x(); m_end_position[Y] = pos.y(); m_end_position[Z] = pos.z();
            };
            ...
            const std::optional<Vec3f> first_vertex = m_seams_detector.get_first_vertex();
            //QDS: the threshold value = 0.0625f == 0.25 * 0.25 is arbitrary, we may find some smarter condition later
...
{
    //QDS: for QIDI machine ,we both use M106 P1 and M106 to indicate the part cooling fan
...
void GCodeProcessor::process_M191(const GCodeReader::GCodeLine& line)
{
    ...
    // QDS: when chamber_temp>40,caculate time required for heating
...
void GCodeProcessor::process_T(const std::string_view command, int nozzle_id)
{
    ...
    if (command.length() > 1) {
        ...
        if (! parse_number(command.substr(1), eid) || eid < 0 || eid > 254) {
            //QDS: T255, T1000 and T1100 is used as special command for QDT machine and does not cost time. return directly
...
void GCodeProcessor::store_move_vertex(EMoveType type, EMovePathType path_type)
{
    ...
    //QDS: apply plate's and extruder's offset to arc interpolation points
    ...
    m_result.moves.push_back({
        ...
        //QDS: add plate's offset to the rendering vertices
...
{
    //QDS: change load time to machine config and all extruder has same value
...
{
    //QDS: change unload time to machine config and all extruder has same value
...
void GCodeProcessor::update_slice_warnings()
{
    ...
    //qds:HRC checker // remove the checker
