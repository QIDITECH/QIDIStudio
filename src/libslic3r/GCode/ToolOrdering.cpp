...
void ToolOrdering::collect_extruders(const PrintObject &object, const std::vector<std::pair<double, unsigned int>> &per_layer_extruder_switches)
{
    ...
    // QDS: collect first layer extruders of an object's wall, which will be used by brim generator
    int layerCount = 0;
...
//QDS: find first non support filament
...
ToolOrdering::LayerData ToolOrdering::collect_layer_and_unprintable_data()
{
    ...
    if (has_any_mixed_filament(is_mixed))
        expand_mixed_slots_in_unprintables(data.geometric_unprintables, is_mixed, comp_strs);
...
float WipingExtrusions::mark_wiping_extrusions(const Print& print, unsigned int old_extruder, unsigned int new_extruder, float volume_to_wipe)
{
    ...
    // QDS: fix the exception caused by not fixed order between different objects
