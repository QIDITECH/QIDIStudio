...
{
    // QDS: toolchange unload is done in change_filament_gcode
#if 0
    ...
    const std::string&  new_material)
{
    ...
    // QDS: do travel in GCode::append_tcr() for lazy_lift
#if 0
...
void WipeTower::toolchange_Load(
	...
	const box_coordinates  &cleaning_box)
{
    // QDS: tool load is done in change_filament_gcode
#if 0
	...
	float wipe_length)
{
    ...
    // QDS: add the note for gcode-check, when the flow changed, the width should follow the change
    if (is_first_layer()) {
	...
	for (int i = 0; true; ++i)	{
        ...
        // QDS: check the bridging area and use the bridge flow
		...
		if (x_to_wipe < WT_EPSILON) {
            // QDS: Delete some unnecessary travel
    ...
    writer.set_extrusion_flow(m_extrusion_flow); // Reset the extrusion flow.
    // QDS: add the note for gcode-check when the flow changed
    if (is_first_layer()) {
...
WipeTower::ToolChangeResult WipeTower::finish_layer(bool extrude_perimeter, bool extruder_fill)
{
    ...
    if (extruder_fill && dy > m_perimeter_width)
    {
        ...
        if (solid_infill) {
            ...
            // QDS: add wipe_path for this case: only with finish rectangle
                                ...
                                unsigned int new_tool, float wipe_volume_ec,float wipe_volume_nc,float purge_volume)
{
    ...
    // QDS: if the wipe tower width is too small, the depth will be infinity
    if (width <= EPSILON)
        return;

    // QDS: remove old filament ramming and first line
#if 0
...
WipeTower::NozzleChangeResult WipeTower::ramming(int old_filament_id, int new_filament_id, bool solid_infill, bool extruder_change)
{
    ...
    }//QDS:nozzle change does not require forcing a cooldown to a specific temperature.
...
void WipeTower::toolchange_wipe_new(WipeTowerWriter &writer, const box_coordinates &cleaning_box, float wipe_length,bool solid_tool_toolchange)
{
    ...
    for (int i = 0; true; ++i) {
        ...
        if (i == 0 && m_use_gap_wall) { // QDS: add ironing after extruding start
            if (m_left_to_right) {
