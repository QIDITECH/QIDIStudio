...
namespace Slic3r
{
std::vector<int>        G_physical_extruder_map;
...
std::string format_line_M104(int target_temp, int target_extruder = -1, const std::string &comment = std::string()) {
    std::string buffer = "M104";
    if (target_extruder != -1) buffer += (" T" + std::to_string(G_physical_extruder_map[target_extruder]));
...
std::string format_line_M109(int target_temp, int target_extruder = -1, const std::string &comment = std::string())
{
...
{
    G_physical_extruder_map          = config.physical_extruder_map.values;
...
WipeTower::ToolChangeResult WipeTower::finish_block_solid(const WipeTowerBlock &block, int filament_id, bool extrude_fill, WipeTowerLayerType layer_type)
{
    ...
    std::vector<Vec2f> finish_rect_wipe_path;
    {
        ...
        if (layer_type == WipeTowerLayerType::Contact && m_enable_tower_interface_features && m_filpar[m_current_tool].filament_tower_interface_print_temp != m_filpar[m_current_tool].nozzle_temperature)
            writer.append(format_line_M104(m_filpar[m_current_tool].nozzle_temperature, m_filament_map[this->m_current_tool] - 1));//may cause issues
        writer.append("; CP EMPTY GRID END\n"
                      ";------------------\n\n\n\n\n\n\n");
    }
...
void WipeTower::toolchange_wipe_new(WipeTowerWriter &writer, const box_coordinates &cleaning_box, float wipe_length,bool solid_tool_toolchange)
{
    ...
    for (int i = 0; true; ++i) {
        ...
        if (should_cooling_before_object && i == cooling_begin_line) {
            writer.append(format_line_M104(m_filpar[m_current_tool].nozzle_temperature, m_filament_map[this->m_current_tool] - 1));
        }
