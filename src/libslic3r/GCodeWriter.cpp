...
std::string GCodeWriter::set_pressure_advance(double pa, bool is_qdt_bowden) const
{
    std::ostringstream gcode;
    if (pa < 0) return gcode.str();
    if (false) { // todo: qdt printer
...
std::string GCodeWriter::travel_to_xyz(const Vec3d &point, const std::string &comment, bool use_short_travel_acceleration)
{
    ...
    //QDS: a z_hop need to be handle when travel
    if (std::abs(m_to_lift) > EPSILON) {
        assert(std::abs(m_lifted) < EPSILON);
        //QDS: don't need to do real lift if the current position is absolutely same with target.
        ...
        //QDS: don'need slope travel because we don't know where is the source position the first time
        //QDS: Also don't need to do slope move or spiral lift if x-y distance is absolute zero
        if (delta(2) > 0 && delta_no_z.norm() != 0.0f)    {
            //QDS: SpiralLift
            if (m_to_lift_type == LiftType::SpiralLift && this->is_current_position_clear()) {
                //QDS: todo: check the arc move all in bed area, if not, then use lazy lift
