...
public:
    ...
    //During the reset process, the circular hole compensation flag needs to be retained 
    // to prevent abnormal speed of circular hole compensation when modifying parameters and then slicing.
    std::vector <bool>             raw_counter_circle_compensation; //vector match to size of slices.surfaces (expolygons)
    std::vector <std::vector<int>> raw_holes_circle_compensation; // hole index, size of slices.surfaces, size of holes
    ...
    // QDS: Unspecified fill polygons, used for interecting when we don't want the infill/perimeter overlap
...
public:
    ...
    ExPolygons 				 lslices;
    ExPolygons 				 lslices_extrudable;  // QDS: the extrudable part of lslices used for tree support
    ...
    //QDS: this function calculate the maximum void grid area of sparse infill of this layer. Just estimated value
