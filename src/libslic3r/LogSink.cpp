...
void LogSinkBackend::consume(const boost::log::record_view& rec, const std::string& formatted_message)
{
    ...
    if (in && output) {
        ...
        switch (m_log_enc_options.enc_type) {
            case Slic3r::LogEncOptions::LogEncType::LOG_ENC_AES_256_CBC:
            {
                ...
                if (QDT_Encrypt::AES256CBC_Encrypt((unsigned char*)in, int_len, output, output_len, m_log_enc_key, m_log_enc_key_iv)) {
                    try {
    ...
    if (0) {
        ...
        bool ok = QDT_Encrypt::AES256CBC_Decrypt((unsigned char*)encrpted_data, encpted_size, decrpted_output, output_len, m_log_enc_key, m_log_enc_key_iv_base);
...
bool LogSinkBackend::DecodeAES256LogFile(const std::string& file_path, const std::string& output_path, const std::string& log_enc_key, const std::string& log_enc_key_iv)
{
    ...
    unsigned out_len = 0;
    bool ok = QDT_Encrypt::AES256CBC_Decrypt(const_cast<unsigned char*>(enc_ptr),
