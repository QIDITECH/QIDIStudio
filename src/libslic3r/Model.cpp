...
Model& Model::assign_copy(Model &&rhs)
{
    ...
    //QDS: add auxiliary path logic
    // QDS: backup, all in one temp dir
    this->design_id = rhs.design_id;
    this->stl_design_id = rhs.stl_design_id;
    this->stl_design_country = rhs.stl_design_country;
    this->makerlab_region = rhs.makerlab_region;
...
Model::~Model()
{
    this->clear_objects();
    this->clear_materials();
    // QDS: clear backup dir of temparary model
                            ...
                            ObjImportColorFn                                    objFn)
{
    ...
    //else if (boost::algorithm::iends_with(input_file, ".amf") || boost::algorithm::iends_with(input_file, ".amf.xml"))
        ...
        //QDS: is_xxx is used for is_inches when load amf
        ...
        //QDS: is_xxx is used for is_qds_3mf when load 3mf
        result = load_qds_3mf(input_file.c_str(), config, config_substitutions, &model, plate_data, project_presets, is_xxx, file_version, proFn, options, project, plate_id);
#ifdef __APPLE__
...
Model Model::read_from_archive(const std::string& input_file, DynamicPrintConfig* config, ConfigSubstitutionContext* config_substitutions, En3mfType& out_file_type, LoadStrategy options,
        PlateDataPtrs* plate_data, std::vector<Preset*>* project_presets, Semver* file_version, Import3mfProgressFn proFn, QDTProject *project,
        std::unordered_map<int, std::vector<std::string>>* color_group_map, VolumeColorInfoMap* volume_color_data)
{
    ...
    if (boost::algorithm::iends_with(input_file, ".3mf")) {
        ...
        if (prusa_file_parser.check_3mf_from_prusa(input_file)) {
            ...
            // QDS: add part plate related logic
            // QDS: backup & restore
            result = load_qds_3mf(input_file.c_str(), config, config_substitutions, &model, plate_data, project_presets, &is_qdt_3mf, file_version, proFn, options, project, 0, color_group_map, volume_color_data);
        }
    }
    else if (boost::algorithm::iends_with(input_file, ".zip.amf"))
        result = load_amf(input_file.c_str(), config, config_substitutions, &model, &is_qdt_3mf);
    else
    ...
    if (out_file_type != En3mfType::From_Prusa) {
        out_file_type = is_qdt_3mf ? En3mfType::From_QDS : En3mfType::From_Other;
    }
...
ModelObject* Model::add_object(const char *name, const char *path, TriangleMesh &&mesh)
{
    ...
    new_volume->source.object_idx = (int)this->objects.size() - 1;
    new_volume->source.volume_idx = (int)new_object->volumes.size() - 1;
    // QDS: set default extruder id to 1
    if (!new_object->config.has("extruder") || new_object->config.extruder() == 0)
        new_object->config.set_key_value("extruder", new ConfigOptionInt(1));
    new_object->invalidate_bounding_box();
    return new_object;
	...
	for (const ModelObject* o : this->objects)
    	for (const ModelVolume* v : o->volumes) {
            ...
            auto copy_volume = [o, v, max_extruders, &counter, &extruder_counter](ModelVolume *new_v) {
                ...
                //QDS: Use extruder priority: volumn > object > default
...
//QDS: add auxiliary files temp path
// QDS: backup all in one dir
...
// QDS: Boolean Operations impl. - MusangKing
...
Polygon ModelObject::convex_hull_2d(const Transform3d& trafo_instance) const
{
    ...
    for (const ModelVolume* v : volumes) {
        if (v->is_model_part())
            //QDS: use convex hull vertex instead of all
...
void ModelObject::rotate(double angle, const Vec3d& axis)
{
    ...
    //QDS update assemble transformation when modify volume rotation
...
ModelObjectPtrs ModelObject::cut(size_t instance, std::array<Vec3d, 4> plane_points, ModelObjectCutAttributes attributes)
{
    ...
    const auto instance_matrix = Geometry::assemble_transform(
        ...
        instances[instance]->get_rotation().cwiseProduct(Vec3d(1.0, 1.0, 1.0)),   // QDS: do apply Z-rotation
...
ModelObjectPtrs ModelObject::segment(size_t instance, unsigned int max_extruders, double smoothing_alpha, int segment_number)
{
    ...
    const auto instance_matrix = Geometry::assemble_transform(
        Vec3d::Zero(),  // don't apply offset
        instances[instance]->get_rotation(),   // QDS: keep Z-rotation
        instances[instance]->get_scaling_factor(),
        instances[instance]->get_mirror()
    );
...
void ModelObject::split(ModelObjectPtrs* new_objects)
{
    ...
    for (int volume_idx = 0; volume_idx < this->volumes.size(); volume_idx++) {
        ...
        if (!is_multi_volume_object) {
            ...
            //QDS: multi volume object, then only split to volume
    ...
    for (int i = 0; i < volume_mesh_counts.size(); i++) {
        ...
        for (TriangleMesh& mesh : meshes) {
            ...
            if (is_multi_volume_object) {
                // QDS: volume geometry not changed, so we can keep the color paint facets
                if (new_vol->mmu_segmentation_facets.timestamp() == volume->mmu_segmentation_facets.timestamp())
                    new_vol->mmu_segmentation_facets.reset(); // QDS: let next assign take effect
                new_vol->mmu_segmentation_facets.assign(volume->mmu_segmentation_facets);
            }

            // QDS: clear volume's config, as we already set them into object
            ...
            for (ModelInstance* model_instance : new_object->instances)
            {
                ...
                //QDS: add assemble_view related logic
...
double ModelObject::get_instance_min_z(size_t instance_idx) const
{
    ...
    for (const ModelVolume* v : volumes) {
        ...
        //QDS: in some case the convex hull is empty due to the qhull algo
    ...
    //QDS: add some logic to avoid wrong compute for min_z
    if (min_z == DBL_MAX)
        min_z = 0;
...
unsigned int ModelObject::update_instances_print_volume_state(const BuildVolume &build_volume)
{
    ...
    for (ModelInstance* model_instance : this->instances) {
        ...
        for (const ModelVolume *vol : this->volumes) {
            if (vol->is_model_part()) {
                //QDS: add bounding box empty check logic, for some volume is empty before split(it will be removed after split to object)
...
//QDS: convex_hull_2d using convex_hull_3d
...
//QDS: refine the model part names
...
size_t ModelVolume::split(unsigned int max_extruders, float scale_det)
{
    ...
    for (TriangleMesh &mesh : meshes) {
        ...
        //QDS: always set the extruder id the same as original
...
void ModelVolume::scale_geometry_after_creation(const Vec3f& versor)
{
    ...
    if (m_convex_hull->empty())
        //QDS: recompute the convex hull if it is null for previous too small
        this->calculate_convex_hull();
    else
...
// QDS: thermal length is calculated according to the material of a volume
...
// QDS: thermal length calculation for a group of volumes
...
//QDS: invalidate instance's convex_hull_2d
void ModelInstance::invalidate_convex_hull_2d()
{
    convex_hull.clear();
}

//QDS adhesion coefficients from model object class
...
//QDS maximum temperature difference from model object class
