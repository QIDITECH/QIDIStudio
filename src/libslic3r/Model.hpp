...
public:
    std::string             name;
    //QDS: add module name for assemble
    ...
    // QDS: save for compare with new load volumes
    ...
    //QDS: add instance convex hull bounding box
    ...
    // QDS: Boolean opts - Musang King
    bool make_boolean(ModelObject *cut_object, const std::string &boolean_opts);

    ModelObjectPtrs merge_volumes(std::vector<int>& vol_indeces);//QDS
...
public:
    ...
    // QDS: quick access for volume extruders, 1 based
    ...
    //QDS: add convex_hell_2d related logic
...
private:
    ...
    //QDS: add convex hull 2d related logic
    mutable Polygon                     m_convex_hull_2d; //QDS, used for convex_hell_2d acceleration
    mutable Transform3d                 m_cached_trans_matrix{Transform3d::Identity()}; // QDS, used for convex_hell_2d acceleration
    mutable Polygon                     m_cached_2d_polygon;   //QDS, used for convex_hell_2d acceleration
...
public:
    ...
    int arrange_order = 0; // QDS
    size_t loaded_id = 0; // QDS
...
private:
    ...
    // QDS. Add added members to archive.
    template<class Archive> void serialize(Archive& ar) {
...
// QDS structure stores extruder parameters and speed map of all models
...
public:
    ...
    // QDS static members store extruder parameters and speed map of all models
    ...
    void SetDesigner(std::string designer, std::string designer_user_id) {
        ...
        //QDS tips: clean design user id when set designer
    ...
    // QDS: remove explicit, prefer use move constructor in function return model
    ...
    // QDS: backup
    //QDS: is_xxx is used for is_qds_3mf when loading 3mf, is used for is_inches when loading amf
    ...
    //QDS static functions that update extruder params and speed table
    ...
    static Model read_from_archive(
        ...
        LoadStrategy options = LoadStrategy::AddDefaultInstances, PlateDataPtrs* plate_data = nullptr, std::vector<Preset*>* project_presets = nullptr, Semver* file_version = nullptr, Import3mfProgressFn proFn = nullptr, QDTProject* project = nullptr);
