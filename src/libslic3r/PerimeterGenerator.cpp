...
static void lowpass_filter_by_paths_overhang_degree(ExtrusionPaths& paths) {
    ...
    while (it != paths.end())
    {
        if (last_overhang == it->get_overhang_degree()) {
            //QDS: don't need to append duplicated points, remove the last point
...
{
    // QDS: collect lower_polygons points
    //Polylines;
    ...
    std::deque<PolylineWithDegree> out;
    std::deque<double>             points_overhang;
    //QDS: get overhang degree and split path
    for (size_t polyline_idx = 0; polyline_idx < middle_overhang_polyines.size(); ++polyline_idx) {
        ...
        for (size_t point_idx = 0; point_idx < middle_poly.points.size(); ++point_idx) {
            ...
            // QDS : calculate overhang degree -- overhang length / width
            double this_degree  = (overhang_dist - lower_bound) / (upper_bound - lower_bound) *100;
            // QDS: covert to terraced overhang
...
static ExtrusionEntityCollection traverse_loops(const PerimeterGenerator &perimeter_generator, const PerimeterGeneratorLoops &loops, ThickPolylines &thin_walls)
{
    ...
    for (const PerimeterGeneratorLoop &loop : loops) {
        ...
        // QDS: get lower polygons series, width, mm3_per_mm
        ...
        if (is_external) {
            if (is_small_width) {
                //QDS: smaller width external perimeter
        ...
        if (perimeter_generator.config->detect_overhang_wall && perimeter_generator.layer_id > perimeter_generator.object_config->raft_layers) {
            ...
            //QDS: don't calculate overhang degree when enable fuzzy skin. It's unmeaning
            ...
            if (!detect_overhang_degree) {
                ...
                if (!zero_degree_polines.empty())
                    extrusion_paths_append(
                        ...
                        extrusion_width,
                        (float)perimeter_generator.layer_height);
                //QDS: detect middle line overhang
...
void PerimeterGenerator::process_classic()
{
    ...
    // QDS: don't simplify too much which influence arc fitting when export gcode if arc_fitting is enabled
    ...
    //QDS: reorder the surface to reduce the travel time
    ...
    for (size_t order_idx = 0; order_idx < surface_order.size(); order_idx++) {
        ...
        //QDS: set the topmost and bottom most layer to be one wall
        ...
        if (loop_number >= 0) {
            ...
            for (int i = 0;; ++ i) {  // outer loop is 0
                ...
                if (i == 0) {
                    ...
                    if (this->config->detect_thin_wall) {
                        ...
                        for (const ExPolygon& expolygon : last) {
                            // QDS: judge whether it's narrow but not too long island which is hard to place two line
                            ExPolygons expolys;
                            ...
                            if (offset_result.empty() &&
                                ...
                                // QDS: for narrow external loop, use smaller line width
                                ...
                                //QDS: for not narrow loop, use normal external perimeter line width
                    ...
                    if (m_spiral_vase && (offsets.size() > 1 || offsets_with_smaller_width.size() > 1)) {
                        ...
                        //QDS
                        if (offsets.empty())
                            //QDS: only have small width loop, then keep the largest in spiral vase mode
                            keep_largest_contour_only(offsets_with_smaller_width);
                        else
                            //QDS: have large area, clean the small width loop
                    ...
                    //QDS: For internal perimeter, we should "enable" thin wall strategy in which offset2 is used to
                ...
                if (i == 0 && i != loop_number && this->object_config->top_one_wall_type == TopOneWallType::Alltop && this->upper_slices != NULL) {
                    ...
                    // QDS: check whether surface be bridge or not
                    if (this->lower_slices != NULL) {
                        ...
                        // QDS: Check which piece the bridge belongs to. If the bridge has a connection with the non-top area, it should belong to the non-top area, otherwise it should belong to the top area to get a better surface.
        ...
        if (! gaps.empty()) {
            ...
            for (ExPolygon& ex : gaps_ex) {
                //QDS: Use DP simplify to avoid duplicated points and accelerate medial-axis calculation as well.
        ...
        // QDS: get the no-overlap infill expolygons
        {
