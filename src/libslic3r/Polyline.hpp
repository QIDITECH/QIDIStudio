...
public:
	...
	static Polyline new_scale(const std::vector<Vec2d> &points) {
        ...
        //QDS: new_scale doesn't support arc, so clean
        pl.fitting_result.clear();
		return pl;
    }
    
    void append(const Point &point) {
        //QDS: don't need to append same point
...
//    template <class T> void simplify_by_visibility(const T &area);
    ...
    //QDS: store arc fitting result
    std::vector<PathFittingData> fitting_result;
    //QDS: simplify points by arc fitting
