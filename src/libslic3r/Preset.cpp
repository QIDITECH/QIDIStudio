...
Semver get_version_from_json(std::string file_path)
{
    try {
        ...
        json j;
        ifs >> j;
        std::string version_str = j.at(QDT_JSON_KEY_VERSION);
...
//QDS: add a function to load the key-values from xxx.json
...
VendorProfile VendorProfile::from_ini(const ptree &tree, const boost::filesystem::path &path, bool load_all)
{
    ...
    for (auto &section : tree) {
        if (boost::starts_with(section.first, printer_model_key)) {
            ...
            if (model.family.empty() && res.name == "QDT") {
...
void Preset::save_info(std::string file)
{
    ...
    //QDS: hold is used for stop requesting to server this time
...
bool Preset::save(DynamicPrintConfig* parent_config)
{
    ...
    if (parent_config) {
        ...
        temp_config.set_key_value(QDT_JSON_KEY_FILAMENT_ID, new ConfigOptionString(filament_id));
        temp_config.save_to_json(this->file, this->name, from_str, this->version.to_string());
    } else {
...
//QDS: If one filament or process preset is compatible with one system printer preset,
...
// Because printer_model and nozzle_diameter in QDT system machine preset
...
bool is_compatible_with_parent_printer(const PresetWithVendorProfile& preset, const PresetWithVendorProfile& active_printer)
{
    ...
    //QDS: FIXME only check the parent now, but should check grand-parent as well.
...
bool Preset::has_lidar(PresetBundle *preset_bundle)
{
    ...
    if (preset_bundle) {
        ...
        if (!vendor_name.empty())
            has_lidar = vendor_name.compare("QDT") == 0 ? true : false;
    }
    return has_lidar;
}
...
bool Preset::is_qdt_vendor_preset(PresetBundle *preset_bundle)
{
    ...
    if (preset_bundle) {
        ...
        if (printer_model_opt) {
            ...
            if (!vendor_name.empty())
                is_qdt_vendor_preset = (vendor_name.compare("QDT") == 0);
        }
    }
    return is_qdt_vendor_preset;
}
...
static std::vector<std::string> s_Preset_filament_options{/*"filament_colour", */ "default_filament_colour", "required_nozzle_HRC", "filament_diameter", "volumetric_speed_coefficients", "filament_type",
    ...
    //y58
    ,"box_temperature_range_low", "box_temperature_range_high", "box_temperature"
};
    ...
    PresetsConfigSubstitutions& substitutions, ForwardCompatibilitySubstitutionRule substitution_rule)
{
    ...
    //QDS: get the extruder related info for this preset collection
    std::string extruder_id_name, extruder_variant_name;
    std::set<std::string> *key_set1 = nullptr, *key_set2 = nullptr;
        ...
        if (Slic3r::is_json_file(file_name)) {
            ...
            try {
                ...
                try {
                    ...
                    if (key_values.find(QDT_JSON_KEY_DESCRIPTION) != key_values.end())
                        preset.description = key_values[QDT_JSON_KEY_DESCRIPTION];
                    if (key_values.find(QDT_JSON_KEY_INSTANTIATION) != key_values.end())
                        preset.is_visible = key_values[QDT_JSON_KEY_INSTANTIATION] != "false";

                    //QDS: use inherit config as the base
...
Preset* PresetCollection::get_preset_differed_for_save(Preset& preset)
{
    ...
    //QDS: only save difference for user preset
...
//QDS:get the differencen values to update
int PresetCollection::get_differed_values_to_update(Preset& preset, std::map<std::string, std::string>& key_values)
{
    ...
    //add other values
    key_values[QDT_JSON_KEY_VERSION] = preset.version.to_string();
    if (!preset.base_id.empty()) {
        key_values[QDT_JSON_KEY_BASE_ID] = preset.base_id;
    } else {
        key_values.erase(QDT_JSON_KEY_BASE_ID);
...
//QDS: save user presets to local
    ...
    for (it = project_presets.begin(); it != project_presets.end(); it++) {
        ...
        try {
            ...
            if (inherit_preset) {
                ...
                // Find a default preset for the config. The PrintPresetCollection provides different default preset based on the "printer_technology" field.
                //QDS 202407: don't load project embedded preset when can not find inherit
                //preset->config = default_preset.config;
...
//QDS: get project embedded presets from
...
int PresetCollection::get_user_presets(PresetBundle *preset_bundle, std::vector<Preset> &result_presets)
{
    ...
    for (Preset &preset : m_presets) {
        ...
        //if (!preset.is_qdt_vendor_preset(preset_bundle)) continue;
...
//QDS: update user presets directory
...
//QDS: load one user preset from key-values
bool PresetCollection::load_user_preset(std::string name, std::map<std::string, std::string> preset_values, PresetsConfigSubstitutions& substitutions, ForwardCompatibilitySubstitutionRule rule)
{
    ...
    if (preset_values.find(QDT_JSON_KEY_VERSION) == preset_values.end()) {
        BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << boost::format("can not find version, not loading for user preset %1%")%name;
        return false;
    }
    std::string version_str = preset_values[QDT_JSON_KEY_VERSION];
    ...
    //setting_id
    if (preset_values.find(QDT_JSON_KEY_SETTING_ID) == preset_values.end()) {
        BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << boost::format("can not find setting_id, not loading for user preset %1%")%name;
        return false;
    }
    std::string cloud_setting_id = preset_values[QDT_JSON_KEY_SETTING_ID];

    //update_time
    long long cloud_update_time = 0;
    if (preset_values.find(QDT_JSON_KEY_UPDATE_TIME) != preset_values.end()) {
        cloud_update_time = std::atoll(preset_values[QDT_JSON_KEY_UPDATE_TIME].c_str());
    }

    //user_id
    if (preset_values.find(QDT_JSON_KEY_USER_ID) == preset_values.end()) {
        BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << boost::format("can not find user_id, not loading for user preset %1%")%name;
        return false;
    }
    std::string cloud_user_id = preset_values[QDT_JSON_KEY_USER_ID];
    ...
    if ((iter != m_presets.end()) && (iter->name == name)) {
        ...
        //QDS: we should compare the time between cloud and local
    ...
    // base_id
    if (preset_values.find(QDT_JSON_KEY_BASE_ID) == preset_values.end()) {
        BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << boost::format("can not find base_id, not loading for user preset %1%") % name;
        unlock();
        return false;
    }
    std::string cloud_base_id = preset_values[QDT_JSON_KEY_BASE_ID];

    //filament_id
    std::string cloud_filament_id;
    if ((m_type == Preset::TYPE_FILAMENT) && preset_values.find(QDT_JSON_KEY_FILAMENT_ID) != preset_values.end()) {
        cloud_filament_id = preset_values[QDT_JSON_KEY_FILAMENT_ID];
    ...
    try {
        ...
        //QDS: use inherit config as the base
        Preset* inherit_preset = nullptr;
        ConfigOption* inherits_config = cloud_config.option(QDT_JSON_KEY_INHERITS);
        if (inherits_config) {
            ConfigOptionString * option_str = dynamic_cast<ConfigOptionString *> (inherits_config);
            std::string inherits_value = option_str->value;
            /*size_t pos = inherits_value.find_first_of('*');
    ...
    const std::string           filament_id)
{
    ...
    //QDS: add different settings check logic, replace the old system preset's default value with new system preset's default values
    ...
    if (found) {
        //QDS: only select preset for always
        //if (select != LoadAndSelect::Never) {
        if (select == LoadAndSelect::Always) {
        ...
        //QDS: for other filaments under AMS
        if (it->is_project_embedded) {
    ...
    if (from_project) {
        ...
        //external config
        preset.file = path_for_preset(preset);
        //QDS: save full config here for external
...
{
    //QDS: set first visible filament to fla
...
//QDS: skip these keys for dirty check
...
//QDS: add function for dirty_options_without_option_list
...
{
    if (m_type == Preset::Type::TYPE_FILAMENT && preset.is_user() && preset.config.has(QDT_JSON_KEY_INHERITS) &&
        preset.config.option<ConfigOptionString>(QDT_JSON_KEY_INHERITS)->value.empty()) {
