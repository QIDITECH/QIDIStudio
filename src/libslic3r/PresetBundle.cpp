...
void PresetBundle::reset(bool delete_files)
{
    ...
    // QDS: filament_presets is load from project config, not handled here
                                                      ...
                                                      const PresetPreferences& preferred_selection/* = PresetPreferences()*/)
{
    ...
    //QDS: change system config to json
    std::tie(substitutions, errors_cummulative) = this->load_system_presets_from_json(substitution_rule);

    // QDS load preset from user's folder, load system default if
    // QDS: change directories by design
...
std::optional<FilamentBaseInfo> PresetBundle::get_filament_by_filament_id(const std::string& filament_id, const std::string& printer_name, bool only_system) const
{
    ...
    for (auto iter = filaments.begin(); iter != filaments.end(); ++iter) {
        ...
        if (filament_preset.filament_id == filament_id) {
            ...
            //y58
            if (config.has("box_temperature_range_high"))
...
//QDS: load project embedded presets
...
//QDS: get current project embedded presets
...
//QDS: reset project embedded presets
...
//QDS: get bed texture for printer model
...
//QDS: get stl model for printer model
                                                           ...
                                                           ForwardCompatibilitySubstitutionRule                       substitution_rule)
{
    ...
    for (it = my_presets.begin(); it != my_presets.end(); it++) {
        ...
        std::map<std::string, std::string>::iterator inherits_iter = value_map.find(QDT_JSON_KEY_INHERITS);
        ...
        //get the type first
        std::map<std::string, std::string>::iterator type_iter = value_map.find(QDT_JSON_KEY_TYPE);
                                                        ...
                                                        ForwardCompatibilitySubstitutionRule    rule)
{
    ...
    for (auto &file : files) {
        ...
        if (boost::iends_with(file, ".qdscfg") || boost::iends_with(file, ".qdsflmt") || boost::iends_with(file, ".zip")) {
                                       ...
                                       std::vector<std::string> &              result)
{
    try {
        ...
        std::string                        name                 = key_values[QDT_JSON_KEY_NAME];
        ...
        ConfigOption *     inherits_config = config.option(QDT_JSON_KEY_INHERITS);
        ...
        if (key_values.find(QDT_JSON_KEY_FILAMENT_ID) != key_values.end())
            preset.filament_id = key_values[QDT_JSON_KEY_FILAMENT_ID];
...
//QDS save user preset to user_id preset folder
...
void PresetBundle::update_user_presets_directory(const std::string preset_folder)
{
    ...
    // y4
    this->physical_printers.update_user_presets_directory(dir_user_presets, PHYSICAL_PRINTER);
    BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << boost::format(" finished");
}
...
void PresetBundle::update_system_preset_setting_ids(std::map<std::string, std::map<std::string, std::string>>& system_presets)
{
    for (auto iterator: system_presets)
    {
        ...
        if (value_map.count(QDT_JSON_KEY_SETTING_ID) > 0)
            setting_id = value_map[QDT_JSON_KEY_SETTING_ID];
        else {
...
void PresetBundle::remove_users_preset(AppConfig &config, std::map<std::string, std::map<std::string, std::string>> *my_presets)
{
    ...
    if (need_reset_printer_preset) {
        std::string default_printer_model = QDT_DEFAULT_PRINTER_MODEL;
...
//QDS: add json related logic, load system presets from json
...
{
    // for a visible/selected filament recorded in QIDIStudio.conf fialments section
...
void PresetBundle::load_selections(AppConfig &config, const PresetPreferences& preferred_selection/* = PresetPreferences()*/)
{
    ...
    // y4
    std::string initial_physical_printer_profile_name = remove_ini_suffix(config.get("presets", PHYSICAL_PRINTER));
    ...
    //QDS: set default print/filament profiles to QDT's default setting
...
/**
 * @brief Updates the filament multi-color configuration from the AMS multi-color filament data.
 * 
 * This function processes the ams_multi_color_filment data to generate a list of color strings,
 * where each string represents either a single color or a space-separated list of multiple colors.
 * The resulting color strings are then stored in the project_config's "filament_multi_colour" option.
 */
...
//w42
unsigned int PresetBundle::sync_box_list(unsigned int& unknowns)
{
...
DynamicPrintConfig PresetBundle::full_config_secure(std::optional<std::vector<int>>filament_maps) const
{
    ...
    //QDS example: config.erase("print_host");
...
void PresetBundle::load_config_file_config(const std::string &name_or_path, bool is_external, DynamicPrintConfig &&config, Semver file_version, bool selected)
{
    ...
    size_t num_extruders = (printer_technology == ptFFF) ?
        ...
        1;
#else
    // QDS: use filament_colour insteadof filament_settings_id, filament_settings_id sometimes is not generated
    ...
    //QDS: add different settings check logic
    bool has_different_settings_to_system                           = config.option("different_settings_to_system")?true:false;
    ...
    case ptFFF:
    {
        //QDS: add different settings logic
...
//QDS: Load a config bundle file from json
    ...
    const std::string &path, const std::string &vendor_name, LoadConfigBundleAttributes flags, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
                ...
                if (iter1.value().is_object()) {
                    ...
                    for (auto iter2 = iter1.value().begin(); iter2 != iter1.value().end(); iter2++) {
                        if (iter2.value().is_string()) {
                            if (boost::iequals(iter2.key(), QDT_JSON_KEY_NAME)) {
                                name = iter2.value();
                            } else if (boost::iequals(iter2.key(), QDT_JSON_KEY_SUB_PATH)) {
                                subpath = iter2.value();
                            }
                        }
                        else {
    ...
    try {
        ...
        for (auto it = j.begin(); it != j.end(); it++) {
            if (boost::iequals(it.key(), QDT_JSON_KEY_VERSION)) {
                //get version
            ...
            else if (boost::iequals(it.key(), QDT_JSON_KEY_URL)) {
                //get url
            ...
            else if (boost::iequals(it.key(), QDT_JSON_KEY_MACHINE_MODEL_LIST)) {
                //get machine model list
                get_name_and_subpath(it, machine_model_subfiles);
            }
            else if (boost::iequals(it.key(), QDT_JSON_KEY_PROCESS_LIST)) {
                //get process list
                get_name_and_subpath(it, process_subfiles);
            }
            else if (boost::iequals(it.key(), QDT_JSON_KEY_FILAMENT_LIST)) {
                //get filament list
                get_name_and_subpath(it, filament_subfiles);
            }
            else if (boost::iequals(it.key(), QDT_JSON_KEY_MACHINE_LIST)) {
                //get machine list
    ...
    auto parse_config = [this, path, vendor_name, presets_loaded,
        ...
        try {
            ...
            auto includes_it = key_values.find(QDT_JSON_KEY_INCLUDES);
    ...
    try {
        ...
        for (auto it = j.begin(); it != j.end(); it++) {
            if (boost::iequals(it.key(), QDT_JSON_KEY_VERSION)) {
                ...
                // get name
                model.name = it.value();
            } else if (boost::iequals(it.key(), QDT_JSON_KEY_MODEL_ID)) {
                // get model_id
                model.model_id = it.value();
            } else if (boost::iequals(it.key(), QDT_JSON_KEY_NOZZLE_DIAMETER)) {
                // get nozzle diameter
            ...
            } else if (boost::iequals(it.key(), QDT_JSON_KEY_PRINTER_TECH)) {
                // get printer tech
                if (boost::algorithm::starts_with(it.value(), "SL"))
                    model.technology = ptSLA;
                else
                    model.technology = ptFFF;
            } else if (boost::iequals(it.key(), QDT_JSON_KEY_FAMILY)) {
                // get family
                model.family = it.value();
            } else if (boost::iequals(it.key(), QDT_JSON_KEY_BED_MODEL)) {
                // get bed model
            ...
            } else if (boost::iequals(it.key(), QDT_JSON_KEY_USE_DOUBLE_EXTRUDER_DEFAULT_TEXTURE)) {
            ...
            } else if (boost::iequals(it.key(), QDT_JSON_KEY_BOTTOM_TEXTURE_RECT_LONGER)) {
            ...
            } else if (boost::iequals(it.key(), QDT_JSON_KEY_HOTEND_MODEL)) {
                model.hotend_model = it.value();
            } else if (boost::iequals(it.key(), QDT_JSON_KEY_DEFAULT_MATERIALS)) {
                // get machine list
            ...
            } else if (boost::iequals(it.key(), QDT_JSON_KEY_NOT_SUPPORT_BED_TYPE)) {
                // get machine list
...
void PresetBundle::update_compatible(PresetSelectCompatibleType select_other_print_if_incompatible, PresetSelectCompatibleType select_other_filament_if_incompatible)
{
    ...
    public:
        PreferedFilamentsProfileMatch(const Preset *preset, const std::vector<std::string> &prefered_names) :
            ...
            m_prefered_filament_type(preset ? preset->config.opt_string("filament_type", 0) : std::string("PLA")), // QDS: default choose PLA
...
//QDS: add a API to dump current configbundle as default configbundle
/*void PresetBundle::export_current_configbundle(const std::string &path)
{
    boost::nowide::ofstream c;
    c.open(path, std::ios::out | std::ios::trunc);

    c << "# generate the configbundle for QDT" << std::endl;
    ...
    // Export the print
    std::string print_qdt = "@QDT-3DP";
    size_t pos = print_selected_preset.name.find(print_qdt);
    if (std::string::npos != pos)
    {
        std::string system_print_name = print_selected_preset.name.substr(0, pos + print_qdt.size());
    ...
    // Export the filament
    std::string filament_qdt = "@QDT-3DP";
    pos = filament_selected_preset.name.find(filament_qdt);
    if (std::string::npos != pos)
    {
        std::string system_filamant_name = filament_selected_preset.name.substr(0, pos + filament_qdt.size());
	...
	// Export the printer
    std::string printer_qdt = "QDT-3DP-001";
    pos = printer_selected_preset.name.find(printer_qdt);
    if (std::string::npos != pos)
    {
        std::string system_printer_name = printer_selected_preset.name.substr(0, pos + printer_qdt.size());
...
//QDS: add export system preset functions
...
void PresetBundle::load_support_recommended_params()
{
    ...
    boost::filesystem::path json_path = (boost::filesystem::path(resources_dir()) / "profiles" / "QDT" / "filament" / "support_recommended_params.json").make_preferred();
