...
bool Print::invalidate_state_by_config_options(const ConfigOptionResolver & /* new_config */, const std::vector<t_config_option_key> &opt_keys)
{
    ...
    static std::unordered_set<std::string> steps_gcode = {
        ...
        "bed_exclude_area",
        //w12
        "thumbnail_size","thumbnails_formats",
    ...
    for (const PrintObject* object : m_objects) {
        ...
        for (auto layer_range : mo->layer_config_ranges) {
            if (layer_range.second.has("extruder")) {
                //QDS: actually when user doesn't change filament by height range(value is default 0), height range should not save key "extruder".
...
//QDS: this function is used to check whether multi filament can be printed
...
StringObjectException Print::validate(StringObjectException *warning, Polygons* collison_polygons, std::vector<std::pair<Polygon, float>>* height_polygons) const
{
    ...
    if (this->has_wipe_tower() && ! m_objects.empty()) {
        ...
        // QDS: remove following logic and _L()
#if 0
        ...
        // QDS: When prime tower is on, object layer and support layer must be aligned. So support gap should be multiple of object layer height.
        ...
        if (m_objects.size() > 1) {
            ...
            for (size_t i = 1; i < m_objects.size(); ++ i) {
                ...
                // QDS: support gap can be multiple of object layer height, remove _L()
#if 0
            ...
            // QDS: remove obsolete logics and _L()
            if (has_custom_layering) {
                ...
                for (size_t idx_object = 0; idx_object < m_objects.size(); ++idx_object) {
                    ...
                    while (i < layer_z_series[idx_object].size() && i < layer_z_series[tallest_object_idx].size()) {
                        // QDS: remove the break condition, because a variable layer height object and a new object will not be checked when slicing
...
//        	double extrusion_width_max = config.get_abs_value(opt_key, max_nozzle_diameter);
        ...
        for (PrintObject *object : m_objects) {
            if (object->has_support_material()) {
                // QDS: remove useless logics and L()
#if 0
...
// QDS: map print object with its first layer's first extruder
...
void Print::process(std::unordered_map<std::string, long long>* slice_time, bool use_cache)
{
    ...
    if (this->set_started(psSkirtBrim)) {
        ...
        //QDS: get the objects' indices when GCodes are generated
        ...
        if (is_sequential_print()) {
            ...
            // QDS: build instance ordering so objPrintVec and make_brim get all objects; otherwise m_brimMap stays empty and m_objsWithBrim is never populated in GCode export.
        ...
        // QDS: m_brimMap and m_supportBrimMap are used instead of m_brim to generate brim of objs and supports seperately
        m_brimMap.clear();
        m_supportBrimMap.clear();
        m_first_layer_convex_hull.points.clear(); // QDS: plate offset is contained in this convexhull
        if (this->has_brim()) {
...
std::string Print::export_gcode(const std::string& path_template, GCodeProcessorResult* result, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    GCode gcode;
    //QDS: compute plate offset for gcode-generator
...
void Print::_make_skirt()
{
    ...
    for (size_t i = n_skirts, extruder_idx = 0; i > 0; -- i) {
        ...
        Polygon loop;
        {
            // QDS. skirt_distance is defined as the gap between skirt and outer most brim, so no need to add max_brim_width
...
//QDS: add gcode file preload logic
...
//QDS: instance_shift is too large because of multi-plate, apply without plate offset.
