...
struct AutoContourHolesCompensationParams
{
    ...
    // QDS: params for auto contour and holes compensation
...
public:
    ...
    // QDS: Boundingbox of the first layer
...
private:
 ...
 public:
    //QDS: When printing multi-material objects, this settings will make slicer to clip the overlapping object parts one by the other.
...
public:
    ...
    */
    ...
    //QDS: add modify_count logic
    int get_modified_count() const {return m_modified_count;}
    //QDS: add status for whether support used
    bool is_support_used() const {return m_support_used;}
    bool is_QDT_Printer() const { return m_isQDTPrinter;}
    void set_QDT_Printer(const bool isQDT) { m_isQDTPrinter = isQDT;}
...
private:
    ...
    //QDS.
    bool m_isQDTPrinter = false;
    // Ordered collections of extrusion paths to build skirt loops and brim.
    ExtrusionEntityCollection               m_skirt;
    // QDS: collecting extrusion paths to build brim by objs
...
public:
    //QDS: this was a print config and now seems to be useless so we move it to here
