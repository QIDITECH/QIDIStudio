...
static const t_config_enum_values s_keys_map_BrimType = {
    ...
    {"auto_brim", btAutoBrim},  // QDS
    {"brim_ears", btBrimEars}  // QDS
};
...
void PrintConfigDef::init_fff_params()
{
    ...
    def->enum_labels.emplace_back(L("QIDI Cool Plate SuperTack"));
...
#if 1 //!QDT_RELEASE_TO_PUBLIC
    // QDS: The following two types are disabled
    ...
    //QDS: add logic for checking between different system presets
    ...
    def->gui_type = ConfigOptionDef::GUIType::color;
    def->mode = comDevelop;
    def->set_default_value(new ConfigOptionStrings{ "#4479FB" });   // y96
    ...
    def->set_default_value(new ConfigOptionStrings { "" });
    //QDS: open this option to command line
    //def->cli = ConfigOptionDef::nocli;
    ...
    def->readonly = true;
    def->gui_type = ConfigOptionDef::GUIType::one_string;
    def->set_default_value(new ConfigOptionString("50x50"));
    ...
    //y58
    def =this->add("support_box_temp_control",coBool);
    def->label=L("Support control box temperature");
    def->tooltip=L("This option is enabled if machine support controlling box temperature");
    def->mode=comDevelop;
    def->set_default_value(new ConfigOptionBool(false));
    def->readonly=false;
    ...
    //w13
    def = this->add("additional_cooling_fan_speed_unseal", coInts);
    def->label = L("Unseal Fan speed");
    def->tooltip = L("Speed of auxiliary part cooling fan under unseal. Auxiliary fan will run at this speed during printing except the first several layers "
        "which are defined by no cooling layers");
    def->sidetext = "%";
    def->min = 0;
    def->max = 100;
    ...
    //QDS: remove material behind support
    ...
    // QDS: change type to common float.
...
//    def->min = 0;
    ...
    def->tooltip = L("The Tips!!");
    def->sidetext = "°C";
    def->full_label = L("Box temperature");
    def->min = 0;
    def->max = 65;
    def->set_default_value(new ConfigOptionInts{0});
    ...
    //y58
    def = this->add("box_temperature_range_low", coInts);
    def->label = L("Min");
    //def->tooltip = "";
    def->sidetext = "°C";
    def->min = 0;
    ...
    def = this->add("box_temperature_range_high", coInts);
    def->label = L("Max");
    //def->tooltip = "";
    def->sidetext = "°C";
    def->min = 0;
    ...
    def->tooltip = L("If your firmware requires relative E values, check this, "
        "otherwise leave it unchecked. Must use relative e distance for QIDI printer");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(true));
    ...
    //def->sidetext = L("mm");
    def->mode = comDevelop;
    // QDS: change data type to floats to add partplate logic
    def->set_default_value(new ConfigOptionFloats{ 15. });
    ...
    //w12
    def = this->add("thumbnails_formats", coEnum);
    def->label = L("Format of G-code thumbnails");
    def->tooltip = L("Format of G-code thumbnails: PNG for best quality, JPG for smallest size");
    def->mode = comAdvanced;
    def->enum_keys_map = &ConfigOptionEnum<GCodeThumbnailsFormat>::get_enum_values();
    def->enum_values.push_back("PNG");
    def->enum_values.push_back("QIDI");
    ...
    for (auto& opt_key : filament_extruder_override_keys) {
        ...
        //QDS: shown specific filament retract config because we hide the machine retract into comDevelop mode
...
void PrintConfigDef::handle_legacy(t_config_option_key &opt_key, std::string &value)
{
    ...
    if (opt_key == "enable_wipe_tower") {
        ...
        //QDS: this is old profile in which value is expressed as percentage.
...
{
    if (config.has("thumbnail_size")) {
        std::string extention;
        if (config.has("thumbnails_formats")) {
            if (const ConfigOptionDef* opt = config.def()->get("thumbnails_formats")) {
                extention = opt->enum_values.at(config.option("thumbnails_formats")->getInt());
            }
        }

        std::string thumbnails_str = config.opt_string("thumbnail_size");
        auto [thumbnails_list, errors] = GCodeThumbnails::make_and_check_thumbnail_list(thumbnails_str, extention);

        if (errors != enum_bitmask<ThumbnailError>()) {
            std::string error_str = "\n" + format("Invalid value provided for parameter %1%: %2%", "thumbnail_size", thumbnails_str);
        ...
        if (!thumbnails_list.empty()) {
            ...
            config.set_key_value("thumbnail_size", new ConfigOptionString(thumbnails_str));
        }
    }
}
...
double min_object_distance(const ConfigBase &cfg)
{
    ...
    if (printer_technology == ptSLA)
        ...
        //QDS: duplicate_distance seam to be useless
...
void DynamicPrintConfig::normalize_fdm(int used_filaments)
{
    ...
    if (used_filaments > 0 && ept_opt != nullptr) {
        ...
        /* QDS: MusangKing - not sure if this is still valid, just comment it out cause "Independent support layer height" is re-opened.
        else {
            if (islh_opt)
                islh_opt->value = true;
        }
...
//QDS:divide normalize_fdm to 2 steps and call them one by one in Print::Apply
...
t_config_option_keys DynamicPrintConfig::normalize_fdm_2(int num_objects, int used_filaments)
{
    ...
    if (used_filaments > 0 && ept_opt != nullptr) {
        ...
        /* QDS：MusangKing - use "global->support->Independent support layer height" widget to replace previous assignment
        else {
            if (islh_opt) {
                if (!islh_opt->value) {
                    islh_opt->value = true;
...
//QDS: pass map to recording all invalid valies
...
//QDS: remove unused command currently
...
*/
    ...
    def = this->add("export_gcode", coString);
    def->label = "Export G-code";
    def->tooltip = "Export project as G-code.";
    def->cli_params = "filename.gcode";
    def->set_default_value(new ConfigOptionString("output.gcode"));
...
CLIMiscConfigDef::CLIMiscConfigDef()
{
    ...
    def->tooltip = L("This version of QIDIStudio may not understand configurations produced by the newest QIDIStudio versions. "
                     "For example, newer QIDIStudio may extend the list of supported firmware flavors. One may decide to "
    ...
    def->tooltip = L("If enabled, the command line arguments are sent to an existing instance of GUI QIDIStudio, "
                     "or an existing QIDIStudio window is activated. "
