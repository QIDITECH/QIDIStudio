...
void PrintObject::make_perimeters()
{
    ...
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++ region_id) {
        ...
        //QDS: remove extra_perimeters, always false
...
void PrintObject::generate_support_material()
{
    if (this->set_started(posSupportMaterial)) {
        ...
        if(!has_support() && !m_print->get_no_check_flag()) {
            // QDS: pop a warning if objects have significant amount of overhangs but support material is not enabled
    ...
    const ConfigOptionResolver &old_config, const ConfigOptionResolver &new_config, const std::vector<t_config_option_key> &opt_keys)
{
    ...
    for (const t_config_option_key &opt_key : opt_keys) {
        if (   opt_key == "brim_width"
            ...
            || opt_key == "brim_type"
            // QDS: brim generation depends on printing speed
            ...
            if (opt_key == "brim_type") {
                ...
                //QDS: When switch to manual brim, the object must have brim, then re-generate perimeter
            ...
            || opt_key == "support_bottom_interface_spacing" //QDS
            ...
            //|| opt_key == "independent_support_layer_height" // QDS
...
void PrintObject::bridge_over_infill()
{
    ...
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++ region_id) {
		...
		for (LayerPtrs::iterator layer_it = m_layers.begin(); layer_it != m_layers.end(); ++ layer_it) {
            ...
            //QDS: enable thick bridge for internal bridge only
            ...
            ExPolygons to_bridge;
            {
                ...
                // QDS: expand to make avoid gap between bridge and inner wall
            ...
            for (ExPolygon &ex : to_bridge) {
                ...
                // QDS: detect angle for internal bridge infill
            ...
            //QDS: modify stInternal to be stInternalWithLoop to give better support to internal bridge
            if (!to_bridge.empty()){
                ...
                //QDS: lighting infill doesn't support this feature. Don't need to add loop when infill density is high than 50%
                ...
                for (size_t cluster_idx = r.begin(); cluster_idx < r.end(); cluster_idx++) {
                    for (size_t job_idx = 0; job_idx < clustered_layers_for_threads[cluster_idx].size(); job_idx++) {
                        ...
                        for (const CandidateSurface &candidate : surfaces_by_layer[lidx]) {
                            ...
                            // QDS: substract top area
                            bridging_area          = diff(bridging_area, top_area);
                            // QDS: open and close again to filter some narrow parts
                            bridging_area          = opening(bridging_area, flow.scaled_spacing());
                            bridging_area          = closing(bridging_area, flow.scaled_spacing());
    ...
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++ region_id) {
        ...
        std::vector<size_t> combine(m_layers.size(), 0);
        {
            ...
            for (size_t layer_idx = 0; layer_idx < m_layers.size(); ++ layer_idx) {
                ...
                // QDS: automatically calculate how many layers should be combined
