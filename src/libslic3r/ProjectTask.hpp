...
class QDTSliceInfo {
public:
    QDTSliceInfo(QDTProfile* profile = nullptr)
    {
        profile_ = profile;
        prediction = 0;
        weight = 0.0f;
    ...
    QDTSliceInfo(const QDTSliceInfo& obj) {
...
public:
    ...
    QDTSubTask(QDTTask* task = nullptr);

    QDTSubTask(const QDTSubTask& obj) {
        task_id             = obj.task_id;
    ...
    QDTSliceInfo    slice_info;         /* slice info of subtask */
    ...
    static QDTSubTask::SubTaskStatus parse_status(std::string status);
    static QDTSubTask::SubTaskStatus parse_user_service_task_status(int status);
};

typedef std::function<void(QDTModelTask* subtask)> OnGetSubTaskFn;
...
public:
    ...
    QDTTask(QDTProfile* profile = nullptr);
    ...
    std::vector<QDTSubTask*>    subtasks;
    std::map<std::string, QDTSliceInfo*> slice_info; /* slice info of subtasks, key: plate idx, 1, 2, 3, etc... */
...
class QDTProfile {
public:
    QDTProfile(QDTProject* project = nullptr);
    ~QDTProfile() {}

    std::vector<QDTTask*>   tasks;
    ...
    QDTProject*             project_;
    std::map<std::string, QDTSliceInfo*>    slice_info; /* key: plate_idx, start at 1, 2, 3, etc. */
    QDTSliceInfo* get_slice_info(std::string plate_idx);
};
...
public:
    ...
    std::vector<QDTProfile*>   profiles;
