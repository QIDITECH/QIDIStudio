    ...
    )
{
    ...
    thresh_angle = std::min(thresh_angle, 89.); // QDS should be smaller than 90
    ...
    SupportGeneratorLayerStorage                        &layer_storage)
{
    ...
    if (layer_id == 0) {
        ...
        // QDS: need to consider adaptive layer heights
            ...
            if (bridging_print_z >= min_print_z) {
                ...
                if (bridging_print_z < print_z - EPSILON) {
                    ...
                    if (bridging_print_z == slicing_params.first_print_layer_height) {
                        ...
                        // QDS: if independent_support_layer_height is not enabled, the support layer_height should be the same as layer height.
    ...
    const PrintObject &object, const std::vector<Polygons> &buildplate_covered, SupportGeneratorLayerStorage &layer_storage) const
{
    ...
    // QDS: tree support is selected so normal supports need not be generated.
            ...
            if (top_contacts[top_idx]->print_z > layer_new.print_z - support_params.support_layer_height_min - EPSILON) {
                ...
                if (diff > 0.F) {
                    if (layer_new.height - diff > support_params.support_layer_height_min) {
                        ...
                        // QDS: The trimmed layer height is smaller than support_layer_height_min. Walk to the next top contact layer.
                        continue;
                    }
                }
                else {
    ...
    SupportGeneratorLayerStorage      &layer_storage)
{
    ...
    if (object.has_brim()) {
        ...
        // QDS: the pattern of raft and brim are the same, thus the brim can be serpated by support raft.
    ...
    if (slicing_params.raft_layers() > 1) {
        ...
        // Expand the bases of the support columns in the 1st layer.
            ...
            Polygons  trimming;
            // QDS: if first layer of support is intersected with object island, it must have the same function as brim unless in nobrim mode.
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(n_raft_layers, support_layers.size()),
        ...
        for (size_t support_layer_id = range.begin(); support_layer_id < range.end(); ++ support_layer_id)
        {
            ...
            for (size_t i = 0; i < 3; ++ i) {
                ...
                // QDS support more interface patterns
                FillParams fill_params;
                fill_params.density = density;
                fill_params.dont_adjust = true;
            ...
            if (! base_layer.empty() && ! base_layer.polygons_to_extrude().empty()) {
                ...
                if (base_layer.layer->bottom_z < EPSILON) {
                    ...
                    // QDS: the 1st layer use the same fill direction as other layers(in rectilinear) to avoid
