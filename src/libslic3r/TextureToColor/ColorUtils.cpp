...
bool remesh_mesh(TriMesh& qds_mesh, std::vector<std::size_t>& face_labels, double target_edge_length_ratio) {
    if (face_labels.size() != qds_mesh.indices.size()) {
    ...
    CGALMesh cgal_mesh;
    cgalutils::convert_trimesh_to_cgal(qds_mesh, cgal_mesh);

    // AABBTreeIndirect references vertices/faces externally, so snapshot the
    // pre-remesh geometry by moving it out of qds_mesh (which is overwritten
    // below with the post-remesh mesh). std::move on std::vector is O(1).
    TriVertices old_vertices = std::move(qds_mesh.vertices);
    TriFaces    old_indices  = std::move(qds_mesh.indices);
    ...
    std::unordered_map<CGAL::SM_Vertex_index, std::size_t> map_cgal_vtx_to_qds_vtx;
    TriVertices qds_vertices;
    TriFaces qds_faces;
    face_labels.clear();
    ...
    for (const auto& cgal_vtx : cgal_mesh.vertices()) {
        ...
        if (!map_cgal_vtx_to_qds_vtx.count(cgal_vtx)) {
            map_cgal_vtx_to_qds_vtx[cgal_vtx] = qds_vertices.size();
            const auto& cgal_point = cgal_mesh.point(cgal_vtx);
            qds_vertices.emplace_back(TriVertex(cgal_point.x(), cgal_point.y(), cgal_point.z()));
        }
    }
    ...
    for (const auto& cgal_face : cgal_mesh.faces()) {
        ...
        for (auto cgal_vtx : cgal_mesh.vertices_around_face(cgal_mesh.halfedge(cgal_face))) {
            do { if (!(map_cgal_vtx_to_qds_vtx.count(cgal_vtx))) { BOOST_LOG_TRIVIAL(warning) << "CGAL mesh contains a face with an invalid vertex"; return false; } } while(0);
            qds_face[face_vid] = map_cgal_vtx_to_qds_vtx[cgal_vtx];
            ++face_vid;
        }
        qds_faces.push_back(std::move(qds_face));
    }

    qds_mesh = TriMesh(qds_faces, qds_vertices);

    face_labels.resize(qds_mesh.indices.size());
    tbb::parallel_for(tbb::blocked_range<std::size_t>(0, qds_mesh.indices.size()), [&](const tbb::blocked_range<size_t>& range) {
        for (std::size_t fid = range.begin(); fid < range.end(); ++fid) {
            auto& face = qds_mesh.indices[fid];
            Vec3f face_centroid = (qds_mesh.vertices[face[0]] + qds_mesh.vertices[face[1]] + qds_mesh.vertices[face[2]]) / 3.0;
            size_t hit_idx = 0;
            Vec3f  closest;
...
bool is_closed(const TriMesh& qds_mesh) {
    CGALMesh cgal_mesh;
    cgalutils::convert_trimesh_to_cgal(qds_mesh, cgal_mesh);
...
bool get_components(const TriMesh& qds_mesh, const std::vector<Vec2f>& qds_vertex_uvs, std::vector<TriMesh>& component_meshes,
                    std::vector<std::vector<Vec2f>>& component_vertex_uvs) {
    component_meshes.clear();
    component_vertex_uvs.clear();

    if (qds_mesh.vertices.size() != qds_vertex_uvs.size()) {
    ...
    if (!cgalutils::convert_trimesh_to_cgal(qds_mesh, qds_vertex_uvs, cgal_mesh, cgal_vertex_uvs)) {
    ...
    for (const auto& faces : face_groups) {
        ...
        TriVertices qds_vertices;
        TriFaces qds_faces;
        std::vector<Vec2f> qds_vertex_uvs;
        ...
        for (auto cgal_face : faces) {
            ...
            for (auto cgal_vtx : cgal_mesh.vertices_around_face(cgal_mesh.halfedge(cgal_face))) {
                if (!map_cgal_vtx_to_qds_vtx.count(cgal_vtx)) {
                    map_cgal_vtx_to_qds_vtx[cgal_vtx] = qds_vertices.size();
                    const auto& cgal_point = cgal_mesh.point(cgal_vtx);
                    qds_vertices.push_back(TriVertex(cgal_point.x(), cgal_point.y(), cgal_point.z()));
                    qds_vertex_uvs.push_back(cgal_vertex_uvs[cgal_vtx]);
                }
                qds_face[face_vid] = map_cgal_vtx_to_qds_vtx[cgal_vtx];
                ++face_vid;
            }
        ...
        component_meshes.push_back(TriMesh(qds_faces, qds_vertices));
        component_vertex_uvs.push_back(std::move(qds_vertex_uvs));
    }
...
bool mesh_cluster(const TriMesh& qds_mesh, const std::vector<RGB>& cluster_centers, std::vector<RGB>& map_face_to_rgb,
    ...
    CGALMesh mesh;
    cgalutils::convert_trimesh_to_cgal(qds_mesh, mesh);
    if (mesh.number_of_faces() != qds_mesh.indices.size()) {
        BOOST_LOG_TRIVIAL(warning) << "CGAL mesh face count does not match QDS mesh face count";
        return false;
    }
