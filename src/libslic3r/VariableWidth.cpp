...
//QDS: new function to filter width to avoid too fragmented segments
static ExtrusionPaths thick_polyline_to_extrusion_paths_2(const ThickPolyline& thick_polyline, ExtrusionRole role, const Flow& flow, const float tolerance)
{
    ...
    for (int i = 0; i < (int)lines.size(); ++i) {
        ...
        //QDS: has large difference in width
        if (thickness_delta > tolerance) {
        ...
        //QDS: just update the max and min width and continue
        else {
    ...
    //QDS: handle the remaining segment
