...
void AMSSetting::create()
{
    ...
    m_static_ams_settings = new wxStaticText(this, wxID_ANY, _L("BOX Settings"), wxDefaultPosition, wxDefaultSize, 0);
...
void AMSSetting::update_ams_img(MachineObject* obj_)
{
    ...
    if (auto ams_switch = obj_->GetFilaSystem()->GetAmsFirmwareSwitch().lock();
        ...
        ams_icon_str = "box_icon";// A series support AMS
    }
...
void AMSSettingTypePanel::CreateGui()
{
    ...
    Label* title = new Label(this, ::Label::Head_13, _L("Box Type"));
...
void AMSSettingTypePanel::OnAmsTypeChanged(wxCommandEvent& event)
{
    ...
    if (obj_) {
        ...
        MessageDialog dlg(this, _L("Box Type switching needs firmware update, taking about 30s. Switch now ?"), SLIC3R_APP_NAME + _L("Info"), wxOK | wxCANCEL | wxICON_INFORMATION);
        dlg.SetButtonLabel(wxID_OK, _L("Confirm"));
...
void AMSSettingArrangeAMSOrder::OnBtnRearrangeClicked(wxCommandEvent& event)
{
    auto part = m_ams_firmware_switch.lock();
    if (part)  {
        MessageDialog dlg(this, _L("Are you sure to reset the ID sequence of the connected BOX?"),
                                   _L("Reset BOX Sequence"), wxOK | wxCANCEL | wxICON_INFORMATION);
        int rtn = dlg.ShowModal();
        if (rtn == wxID_OK) {
