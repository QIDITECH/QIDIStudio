...
{
    if (code == QIDI_NETWORK_ERR_BIND_CREATE_SOCKET_FAILED)
        return _L("Failed to create socket");

    else if (code == QIDI_NETWORK_ERR_BIND_SOCKET_CONNECT_FAILED)
        return _L("Failed to connect socket");

    else if (code == QIDI_NETWORK_ERR_BIND_PUBLISH_LOGIN_REQUEST)
        return _L("Failed to publish login request");

    else if (code == QIDI_NETWORK_ERR_BIND_GET_PRINTER_TICKET_TIMEOUT)
        return _L("Get ticket from device timeout");

    else if (code == QIDI_NETWORK_ERR_BIND_GET_CLOUD_TICKET_TIMEOUT)
        return _L("Get ticket from server timeout");

    else if (code == QIDI_NETWORK_ERR_BIND_POST_TICKET_TO_CLOUD_FAILED)
    ...
    else if (code == QIDI_NETWORK_ERR_BIND_RECEIVE_LOGIN_REPORT_TIMEOUT)
    ...
    if (agent && agent->is_user_login() && ping_code.length() == PING_CODE_LENGTH) {
        ...
        if (result < 0) {
            MessageDialog msg_wingow(nullptr, _L("Login failed. Please check the following items:\n"
                                                 ...
                                                 "2. Confirm that the region settings of the printer and QIDI Studio are consistent."), "", wxICON_WARNING | wxOK);
...
PingCodeBindDialog::~PingCodeBindDialog() {
     ...
     : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY, _L("Log in printer"), wxDefaultPosition, wxDefaultSize, wxCAPTION)
 {
     ...
     m_link_Terms_title->SetForegroundColour(wxColour("#4479FB"));   // y96
     ...
     m_link_privacy_title->SetForegroundColour(wxColour("#4479FB"));
     m_link_privacy_title->Bind(wxEVT_LEFT_DOWN, [this](auto& e) {
         ...
         if (country_code == "CN") {
            // y6
             url = "https://www.qidi3d.cn/policies/privacy";
         }
         else{
             url = "https://www.qidi3d.com/policies/privacy";
         }
     ...
     m_link_notice_title->SetForegroundColour(wxColour("#4479FB"));
     ...
     m_status_bar = std::make_shared<QDTStatusBarBind>(m_simplebook);
