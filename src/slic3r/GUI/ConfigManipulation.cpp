...
//y58
void ConfigManipulation::check_box_temperature(DynamicPrintConfig* config)
{
    if (is_msg_dlg_already_exist)
        return;

    int temperature_range_low, temperature_range_high;
    if (!get_box_temperature_range(config, temperature_range_low, temperature_range_high)) return;

    if (config->has("box_temperature")) {
        if (config->opt_int_nullable("box_temperature", 0) < temperature_range_low || config->opt_int_nullable("box_temperature", 0) > temperature_range_high) {
            wxString msg_text = _(L("Box may be broken when the temperature is out of recommended range.\n"
                                    "Please confirm whether to use the temperature for printing.\n\n"));
            msg_text += wxString::Format(_L("Recommended Box temperature of this filament type is [%d, %d] degree centigrade"), temperature_range_low, temperature_range_high);
            MessageDialog dialog(m_msg_dlg_parent, msg_text, "", wxICON_WARNING | wxOK);
            is_msg_dlg_already_exist = true;
            dialog.ShowModal();
            is_msg_dlg_already_exist = false;
...
//y58
bool ConfigManipulation::get_box_temperature_range(DynamicPrintConfig *config, int &range_low, int &range_high)
{
    bool range_low_exist = false, range_high_exist = false;
    if (config->has("box_temperature_range_low")) {
        range_low       = config->opt_int("box_temperature_range_low", (unsigned int) 0);
        range_low_exist       = true;
    }
    if (config->has("box_temperature_range_high")) {
        range_high       = config->opt_int("box_temperature_range_high", (unsigned int) 0);
