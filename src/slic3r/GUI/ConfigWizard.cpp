...
bool Bundle::load(fs::path source_path, bool ais_in_resources, bool ais_qdt_bundle)
{
    this->preset_bundle = std::make_unique<PresetBundle>();
    this->is_in_resources = ais_in_resources;
    this->is_qdt_bundle = ais_qdt_bundle;

    std::string path_string = source_path.string();
    std::string parent_path = source_path.parent_path().string();
    //QDS: add json logic for vendor bundles
...
Bundle::Bundle(Bundle &&other)
    ...
    , is_qdt_bundle(other.is_qdt_bundle)
{
...
const Bundle& BundleMap::qdt_bundle() const
{
    return const_cast<BundleMap*>(this)->qdt_bundle();
}
...
ConfigWizardIndex::ConfigWizardIndex(wxWindow *parent)
    : wxPanel(parent)
    , bg(ScalableBitmap(parent, "QIDIStudio_192px_transparent.png", 192))
...
//QDS: change filament_vendor to strings for multiple filaments case requirements
...
bool ConfigWizard::priv::apply_config(AppConfig *app_config, PresetBundle *preset_bundle, const PresetUpdater *updater, bool& apply_keeped_changes)
{
    ...
    if (preferred_pt = get_preferred_printer_technology("QDT", bundles.qdt_bundle());
        preferred_pt == ptAny || (preferred_pt == ptSLA && suppress_sla_printer)) {
        for (const auto& bundle : bundles) {
            //QDS: set QDT as default
            if (bundle.second.is_qdt_bundle) { continue; }
    ...
    // Prusa printers are considered first, then 3rd party.
    if (preferred_model = get_preferred_printer_model("QDT", bundles.qdt_bundle(), preferred_variant);
    ...
    //QDS skip sla get_first_added_material_preset(AppConfig::SECTION_MATERIALS, first_added_sla_material);
    ...
    , p(new priv(this))
{
    ...
    p->custom_config.reset(DynamicPrintConfig::new_from_defaults_keys({
        //w12
        "gcode_flavor", "printable_area", "bed_exclude_area", "bed_custom_texture", "bed_custom_model", "nozzle_diameter", "filament_diameter","thumbnail_size"
