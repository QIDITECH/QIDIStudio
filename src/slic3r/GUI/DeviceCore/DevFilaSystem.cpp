...
bool DevFilaSystem::IsQDT_Filament(std::string tag_uid)
{
    if (tag_uid.empty())
    {
        return false;
    ...
    if (jj.contains("ams"))
    {
        if (jj["ams"].contains("ams"))
        {
            ...
            if (!key_field_only)
            {
                ...
                {
                    obj->tray_is_qdt_bits = stol(jj["ams"]["tray_is_qdt_bits"].get<std::string>(), nullptr, 16);
                }
