...
namespace Slic3r
{
            ...
            if (it != localMachineList.end()) {
                ...
                if (obj->get_dev_ip().compare(dev_ip) != 0) {
                    if ( connection_name.empty() ) {
                        BOOST_LOG_TRIVIAL(info) << "MachineObject IP changed from " << QDTCrossTalk::Crosstalk_DevIP(obj->get_dev_ip())
                                                << " to " << QDTCrossTalk::Crosstalk_DevIP(dev_ip);
                        obj->set_dev_ip(dev_ip);
                    }
                    else {
                        if ( obj->dev_connection_name.empty() || obj->dev_connection_name.compare(connection_name) == 0) {
                            BOOST_LOG_TRIVIAL(info) << "MachineObject IP changed from " << QDTCrossTalk::Crosstalk_DevIP(obj->get_dev_ip())
                                                    << " to " << QDTCrossTalk::Crosstalk_DevIP(dev_ip) << " connection_name is " << connection_name;
    ...
    void DeviceManager::restore_local_machines_from_user_access_config()
    {
        ...
        for (const auto& user_access_code : user_access_codes) {
            ...
            const auto dev_ip = QDTCrossTalk::Decode_DevIp(encoded_dev_ip, slicer_uuid);
            if (dev_ip.empty()) {
                BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": Decode_DevIp failed, dev_id=" << QDTCrossTalk::Crosstalk_DevId(dev_id);
                continue;
            }
            ...
            for (const auto& local_access_info : local_access_infos) {
                ...
                if (result < 0) {
                    BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": bind_detect failed code=" << result
                                             << ", dev_id=" << QDTCrossTalk::Crosstalk_DevId(local_access_info.dev_id);
                    continue;
                }
                ...
                if (detectData.dev_id != local_access_info.dev_id) {
                    BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": detected dev_id mismatch, config dev_id="
                                             << QDTCrossTalk::Crosstalk_DevId(local_access_info.dev_id)
                                             << ", detected dev_id=" << QDTCrossTalk::Crosstalk_DevId(detectData.dev_id);
                    continue;
                }
    ...
    bool DeviceManager::set_selected_machine(std::string dev_id)
    {
        ...
        if (it != my_machine_list.end())
        {
            if (selected_machine == dev_id)
            {
                ...
                {
                    BOOST_LOG_TRIVIAL(info) << "set_selected_machine: same cloud machine, dev_id =" << QDTCrossTalk::Crosstalk_DevId(dev_id)
                ...
                {
                    BOOST_LOG_TRIVIAL(info) << "set_selected_machine: same lan machine, dev_id =" << QDTCrossTalk::Crosstalk_DevId(dev_id)
            ...
            else
            {
                if (m_agent)
                {
                    ...
                    {
                        BOOST_LOG_TRIVIAL(info) << "set_selected_machine: select new lan machine, dev_id =" << QDTCrossTalk::Crosstalk_DevId(dev_id);
                        it->second->reset();
#if !QDT_RELEASE_TO_PUBLIC
                        it->second->connect(Slic3r::GUI::wxGetApp().app_config->get("enable_ssl_for_mqtt") == "true" ? true : false);
#else
...
#if !QDT_RELEASE_TO_PUBLIC
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ": " << QDTCrossTalk::Crosstalk_JsonLog(j);
#endif
