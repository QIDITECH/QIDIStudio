        ...
        for (int i = 0; i < distance_map.size(); i++) {
            ...
            for (int j = 0; j < distance_map[i].size(); j++) {
                ...
                if (distance_map[i][j].is_same_color
                    ...
                    else if (min_val == distance_map[i][j].distance && filaments[picked_src_idx].filament_id!= box_filament_infos[picked_src_idx].filament_id && filaments[i].filament_id == box_filament_infos[j].filament_id) {
                        picked_src_idx = i;
                        picked_tar_idx = j;
                    }
                }
...
#pragma endregion
                    ...
                    if (request_tab_from_qds) {
                        BOOST_LOG_TRIVIAL(info) << "qds extrusion_cali_get: " << str;
                                  ...
                                  wxString& info)
{
    ...
    {"PLA-Glow", _L("The rough surface of PLA Glow can accelerate wear on the Box system, particularly on the internal components of the Box Lite.")}
    };
