...
void MachineObject::record_user_access_dev_ip()
{
    ...
    if (GUI::wxGetApp().app_config) {
        if (!dev_ip.empty()) {
            const auto& dev_ip_str = QDTCrossTalk::Encode_DevIp(dev_ip, GUI::wxGetApp().app_config->get("slicer_uuid"));
...
{
    // BOOST_LOG_TRIVIAL(trace) << "reset reset_update_time, dev_id =" << QDTCrossTalk::Crosstalk_DevId(get_dev_id());
...
{
    // BOOST_LOG_TRIVIAL(trace) << "reset dev_id=" << QDTCrossTalk::Crosstalk_DevId(get_dev_id());
    last_update_time = std::chrono::system_clock::now();
    subscribe_counter = SUBSCRIBE_RETRY_COUNT;
    m_push_count = 0;
    m_full_msg_count = 0;
...
bool MachineObject::is_connected()
{
    ...
    if (diff.count() > DISCONNECT_TIMEOUT) {
        // BOOST_LOG_TRIVIAL(trace) << "machine_object: dev_id=" << QDTCrossTalk::Crosstalk_DevId(get_dev_id()) << ", diff count = " << diff.count();
        return false;
    }
...
bool MachineObject::is_info_ready(bool check_version) const
{
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__
        ...
        << ", dev_id=" << QDTCrossTalk::Crosstalk_DevId(get_dev_id());
    return false;
}
...
int MachineObject::parse_json(std::string tunnel, std::string payload, bool key_field_only)
{
    ...
    try {
                ...
                if (j_pre["print"]["command"].get<std::string>() == "push_status") {
                    if (j_pre["print"].contains("msg")) {
                        if (j_pre["print"]["msg"].get<int>() == 0) {           //all message
                            // BOOST_LOG_TRIVIAL(trace) << "static: get push_all msg, dev_id=" << QDTCrossTalk::Crosstalk_DevId(get_dev_id());
                            m_push_count++;
                            m_full_msg_count++;
                    ...
                    if (j_pre.contains("exclude_object") && j_pre["exclude_object"].is_object()) {
                        const auto& eo = j_pre["exclude_object"];
                        if (eo.contains("excluded_objects") && eo["excluded_objects"].is_array()) {
                            m_excluded_objects.clear();
                            for (const auto& obj : eo["excluded_objects"]) {
                                if (obj.is_string())
                                    m_excluded_objects.push_back(obj.get<std::string>());
                            }
                        }
                    }
                }
        ...
        // BOOST_LOG_TRIVIAL(trace) << "parse_json: dev_id=" << QDTCrossTalk::Crosstalk_DevId(get_dev_id()) << ", tunnel=" << tunnel << ", merged playload=" << QDTCrossTalk::Crosstalk_JsonLog(j);
#else
        if (Slic3r::get_logging_level() >= level_string_to_boost("trace")) {
            // BOOST_LOG_TRIVIAL(trace) << __FUNCTION__ << ": dev_id=" << QDTCrossTalk::Crosstalk_DevId(get_dev_id()) << ", origin playload=" << QDTCrossTalk::Crosstalk_JsonLog(j_pre);
        } else {
            if (j_pre.contains("print")) {
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ": dev_id=" << QDTCrossTalk::Crosstalk_DevId(get_dev_id()) << ", print playload=" << QDTCrossTalk::Crosstalk_JsonLog(j_pre["print"]);
            };
        }
#endif
...
#pragma endregion
        ...
        // BOOST_LOG_TRIVIAL(trace) << "parse_json failed! dev_id=" << QDTCrossTalk::Crosstalk_DevId(this->get_dev_id()) <<", ewhat = " << e.what();
    } catch (...) {
        // BOOST_LOG_TRIVIAL(trace) << "parse_json failed! dev_id=" << QDTCrossTalk::Crosstalk_DevId(this->get_dev_id());
    }
