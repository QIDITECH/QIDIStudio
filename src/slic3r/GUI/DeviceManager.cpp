    ...
    std::vector<FilamentInfo> filaments, std::vector<FilamentInfo> &result, std::vector<bool> map_opt, std::vector<int> exclude_id, bool nozzle_has_ams_then_ignore_ext)
{
    ...
    for (int i = 0; i < filaments.size(); i++) {
        ...
        for(int j = 0; j < box_filament_infos.size(); j++){
            ...
            val.tray_id = j;
            wxColour c = wxColour(filaments[i].color);
            wxColour tray_c = AmsTray::decode_color(box_filament_infos[j].color);
            val.distance = GUI::calc_color_distance(c, tray_c);
        ...
        for (int i = 0; i < distance_map.size(); i++) {
            ...
            for (int j = 0; j < distance_map[i].size(); j++) {
                ...
                if (distance_map[i][j].is_same_color
                    ...
                    //else if (min_val == distance_map[i][j].distance && filaments[i].filament_id == box_filament_infos[j].filament_id) {
                    //    picked_src_idx = i;
                    //    picked_tar_idx = j;
                    //}
                }
...
bool MachineObject::is_qdt_filament(std::string tag_uid)
{
    if (tag_uid.empty())
        return false;
...
int MachineObject::command_request_push_all(bool request_now)
{
    ...
    if (diff.count() < REQUEST_PUSH_MIN_TIME) {
        if (request_now) {
            BOOST_LOG_TRIVIAL(trace) << "static: command_request_push_all, dev_id=" << QDTCrossTalk::Crosstalk_DevId(dev_id);
            last_request_push = std::chrono::system_clock::now();
        }
        else {
            BOOST_LOG_TRIVIAL(trace) << "static: command_request_push_all: send request too fast, dev_id=" << QDTCrossTalk::Crosstalk_DevId(dev_id);
            return -1;
        }
    } else {
        BOOST_LOG_TRIVIAL(trace) << "static: command_request_push_all, dev_id=" << QDTCrossTalk::Crosstalk_DevId(dev_id);
...
int MachineObject::command_pushing(std::string cmd)
{
    ...
    if (diff.count() < REQUEST_START_MIN_TIME) {
        BOOST_LOG_TRIVIAL(trace) << "static: command_request_start: send request too fast, dev_id=" << QDTCrossTalk::Crosstalk_DevId(dev_id);
        return -1;
    }
    else {
        BOOST_LOG_TRIVIAL(trace) << "static: command_request_start, dev_id=" << QDTCrossTalk::Crosstalk_DevId(dev_id);
...
{
    BOOST_LOG_TRIVIAL(trace) << "reset reset_update_time, dev_id =" << QDTCrossTalk::Crosstalk_DevId(dev_id);
...
{
    BOOST_LOG_TRIVIAL(trace) << "reset dev_id=" << QDTCrossTalk::Crosstalk_DevId(dev_id);
    last_update_time = std::chrono::system_clock::now();
    subscribe_counter = SUBSCRIBE_RETRY_COUNT;
    m_push_count = 0;
    m_full_msg_count = 0;
...
bool MachineObject::is_connected()
{
    ...
    if (diff.count() > DISCONNECT_TIMEOUT) {
        BOOST_LOG_TRIVIAL(trace) << "machine_object: dev_id=" << QDTCrossTalk::Crosstalk_DevId(dev_id) <<", diff count = " << diff.count();
        return false;
    }
...
int MachineObject::parse_json(std::string tunnel, std::string payload, bool key_field_only)
{
    ...
    try {
                ...
                if (j_pre["print"]["command"].get<std::string>() == "push_status") {
                    if (j_pre["print"].contains("msg")) {
                        if (j_pre["print"]["msg"].get<int>() == 0) {           //all message
                            BOOST_LOG_TRIVIAL(trace) << "static: get push_all msg, dev_id=" << QDTCrossTalk::Crosstalk_DevId(dev_id);
                            m_push_count++;
                            m_full_msg_count++;
        ...
        if (Slic3r::get_logging_level() < level_string_to_boost("trace")) {
            BOOST_LOG_TRIVIAL(info) << "parse_json: dev_id=" << QDTCrossTalk::Crosstalk_DevId(dev_id) << ", origin playload=" << QDTCrossTalk::Crosstalk_JsonLog(j_pre);
        } else {
            BOOST_LOG_TRIVIAL(trace) << "parse_json: dev_id=" << QDTCrossTalk::Crosstalk_DevId(dev_id) << ", tunnel is=" << tunnel << ", merged playload=" << QDTCrossTalk::Crosstalk_JsonLog(j);
        }
...
#pragma region print_task
                    ...
                    if (jj.contains("project_id")
                        ...
                        QDTSubTask* curr_task = get_subtask();
                        if (curr_task) {
                        ...
                        if (jj.contains("upgrade_state")) {
                            ...
                            if (jj["upgrade_state"].contains("dis_state")) {
                                ...
                                if (upgrade_display_hold_count > 0)
                                    ...
                                    //QDS compatibility with old version
                    ...
                    if (jj.contains("ams")) {
                        if (jj["ams"].contains("ams")) {
                            ...
                            if (!key_field_only) {
                                ...
                                if (jj["ams"].contains("tray_is_qdt_bits")) {
                                    tray_is_qdt_bits = stol(jj["ams"]["tray_is_qdt_bits"].get<std::string>(), nullptr, 16);
                                }
                                ...
                                if (ams_exist_bits != last_ams_exist_bits
                                    ...
                                    || tray_is_qdt_bits != last_is_qdt_bits
...
#pragma endregion
                    ...
                    if (request_tab_from_qds) {
                        request_tab_from_qds = false;
                        reset_pa_cali_history_result();
        ...
        BOOST_LOG_TRIVIAL(trace) << "parse_json failed! dev_id=" << QDTCrossTalk::Crosstalk_DevId(this->dev_id) <<", ewhat = " << e.what();
    } catch (...) {
        BOOST_LOG_TRIVIAL(trace) << "parse_json failed! dev_id=" << QDTCrossTalk::Crosstalk_DevId(this->dev_id);
    }
...
QDTSubTask* MachineObject::get_subtask()
{
    if (!subtask_)
        subtask_ = new QDTSubTask(nullptr);
    return subtask_;
}

QDTModelTask* MachineObject::get_modeltask()
{
    return model_task;
}

void MachineObject::set_modeltask(QDTModelTask* task)
{
    model_task = task;
}
        ...
        for (const auto &[slot_id, curr_tray] : ams->trayList) {
            ...
            if (curr_tray->setting_id.size() == 8 && curr_tray->setting_id[0] == 'P' && filament_list.find(curr_tray->setting_id) == filament_list.end()) {
                if (checked_filament.find(curr_tray->setting_id) == checked_filament.end()) {
                    ...
                    try {
                        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " " << __LINE__ << " box settings_id is not exist in filament_list and reset, ams_id: " << ams_id << " tray_id"
            ...
            if (curr_tray->setting_id.size() == 8 && curr_tray->setting_id[0] == 'P' && curr_tray->nozzle_temp_min != "" && curr_tray->nozzle_temp_max != "") {
                if (checked_filament.find(curr_tray->setting_id) == checked_filament.end()) {
                    ...
                    try {
                        ...
                        if (!is_equation) {
                            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " " << __LINE__ << " box filament is not match min max temp and reset, ams_id: " << ams_id << " tray_id"
                                                    << slot_id << "filament_id: " << curr_tray->setting_id;
        ...
        if (it != userMachineList.end()) {
                ...
                it->second->bind_ssdp_version != ssdp_version)
            {
                ...
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " UpdateUserMachineInfo"
                                        << ", dev_id= " << QDTCrossTalk::Crosstalk_DevId(dev_id)
                                        << ", ip = "  << QDTCrossTalk::Crosstalk_DevIP(dev_ip)
                                        << ", printer_name= " << QDTCrossTalk::Crosstalk_DevName(dev_name)
        ...
        if (it != localMachineList.end()) {
            ...
            if (obj->dev_ip.compare(dev_ip) != 0) {
                if ( connection_name.empty() ) {
                    BOOST_LOG_TRIVIAL(info) << "MachineObject IP changed from " << QDTCrossTalk::Crosstalk_DevIP(obj->dev_ip) << " to " << QDTCrossTalk::Crosstalk_DevIP(dev_ip);
                    obj->dev_ip = dev_ip;
                }
                else {
                    if ( obj->dev_connection_name.empty() || obj->dev_connection_name.compare(connection_name) == 0) {
                        BOOST_LOG_TRIVIAL(info) << "MachineObject IP changed from " << QDTCrossTalk::Crosstalk_DevIP(obj->dev_ip) << " to " << QDTCrossTalk::Crosstalk_DevIP(dev_ip) << " connection_name is " << connection_name;
            ...
            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " New Machine, dev_id= " << QDTCrossTalk::Crosstalk_DevId(dev_id)
...
{
    BOOST_LOG_TRIVIAL(info) << "set_selected_machine=" << QDTCrossTalk::Crosstalk_DevId(dev_id);
                     ...
                     wxString& wiki_url)
{
    ...
    if (DeviceManager::filaments_blacklist.contains("blacklist")) {
        for (auto filament_item : DeviceManager::filaments_blacklist["blacklist"]) {
            ...
            if (!vendor.empty()) {
                if ((vendor == "qidi tech" && (tag_vendor == vendor)) ||
                    (vendor == "third party" && (tag_vendor != "qidi tech"))){
                    // Do nothing
                } else {
                    continue;
                }
