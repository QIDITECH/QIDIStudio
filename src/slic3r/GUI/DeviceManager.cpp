...
bool MachineObject::HasAms() const
{
    ...
    string machine_box_type = GUI::wxGetApp().plater()->sidebar().box_list_preset_name;
    string machine_ip = GUI::wxGetApp().plater()->sidebar().box_list_printer_ip;

    return !machine_box_type.empty() && (machine_box_type.find(preset_typename) != std::string::npos) && (dev_ip == machine_ip);
}
