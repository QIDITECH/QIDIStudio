...
import { QIDI_COLORS, formatTypeSeries } from './constants';
...
function isPresetColor(value: string): boolean {
  return QIDI_COLORS.some((c) => c.toUpperCase() === value.toUpperCase());
}
...
function normalizePresetFilamentName(name: string | undefined, vendor: string, type: string, series: string): string {
  ...
  if (vendor === 'QIDI') aliases.push('QIDI');
...
export function AddEditDialog({
  ...
  // STUDIO-17977 F1.3: QDT 官方耗材代码（如 "Q01B00" / "13903"），来自
  ...
  useEffect(() => {
    ...
    // STUDIO-17977 F1.3: QDT 官方代码不持久化，每次 open 都从空开始；
  ...
  // ids match either.  Surfacing the legacy QIDI 40-hex palette here keeps
  ...
  const qidiFallbackCandidates = useMemo<CandidateColor[]>(
    () =>
      QIDI_COLORS.map((hex): CandidateColor => {
  ...
  //     1. strict per-fila_id candidates (official QDT match)
  //     2. type-aggregated fallback (other vendors' same material_type)
  //     3. QIDI 40-colour legacy palette (only after material is chosen,
  ...
  const effectiveCandidates: CandidateColor[] = useMemo(() => {
    ...
    return materialType ? qidiFallbackCandidates : [];
  }, [
    candidates,
    fallbackFilaIds,
    candidatesByFilaId,
    materialType,
    qidiFallbackCandidates,
  ]);
  ...
  // between AMS RFID / QDT preset registry).
  ...
  useEffect(() => {
    ...
    if (!stillValid) {
      ...
      if (fallback) {
        // fallback.color_code is the QDT official code (e.g. "Q01B00"),
  ...
  useEffect(() => {
    ...
    // STUDIO-17977 F1.3: QDT official colour code follows the same
  ...
  const onPickCandidate = useCallback((c: CandidateColor) => {
    ...
    setColorName(c.name || '');
    // STUDIO-17977 F1.3: capture the QDT official colour code (e.g. "Q01B00"
    // / "13903") so the preview row can surface it next to the hex.
    //
    // QIDI legacy fallback and user-owned supplemental candidates carry the
    // primary hex in `color_code` purely as a unique key (testid / dedup);
    // those values aren't real QDT codes, so we suppress them here and leave
  ...
  const onPickCustomSwatch = useCallback((hex: string) => {
    ...
    // name; clear color_name and the QDT official colour code so the
  ...
  // QIDI_COLORS hex-membership check, which became stale once the palette
  ...
  const confirmCustomColor = useCallback(() => {
    const next = commitCustomColorSelection(draftColor, customColors, QIDI_COLORS);
    ...
    // or QDT official code; clear both so the preview row reflects the
  ...
  const seedFormFromTray = (unit: AmsUnit, tray: AmsTray) => {
    ...
    // tag_uid (QDT original spool) so users could not accidentally change
    ...
    //      mapping because it comes straight from the QIDI preset used to
    //      print the spool.
    ...
    // F4.8: color may arrive as #RRGGBBAA (QDT firmware appends alpha for
    ...
    // name and missing QDT fila code.  Honour the invariant: only push
            ...
            <div className="flex flex-col gap-[16px]">
              ...
              <div className="flex flex-col gap-[8px]">
                ...
                {/*
                  ...
                  badge driven by a stale QIDI_COLORS membership check.
                  The new layout, in left-to-right order:
                    [swatch] [colour name?] [QDT official code?] [hex list]
                  ...
                  - QDT official code (e.g. "Q01B00" / "13903") is the
