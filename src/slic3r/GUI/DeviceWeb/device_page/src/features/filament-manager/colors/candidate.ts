...
//       (1) official QDT candidates from FilamentColorCodeQuery,
...
/**
 * "Official" candidates have a real QDT fila code (e.g. "11400" / "Q01B00")
 ...
 * matching the AMS-vs-QDT hex-order mismatches observed on real machines.
 */
...
/**
 * Four-stage fallback for the list row tail "official colour name + QDT
 ...
 *      multiple QDT series (e.g. "11400" 柠檬黄 in PLA Basic / Matte /
...
export function resolveCandidateForSpool(
  ...
  // multicolor information even when AMS / QDT order differs.
  if (ownList && single) {
 ...
 * disagrees with the QDT preset registry on gradient-vs-multicolor for
...
export function candidateMatchesFormState(args: {
  ...
  // hexes in different orders across AMS / QDT preset registry.
