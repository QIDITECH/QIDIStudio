...
int Field::def_width_wider()	{ return 12; }  // y19
...
//                        wxMessageDialog dialog(m_parent, msg_text, _L("Parameter validation") + ": " + m_opt_id, wxICON_WARNING | wxYES | wxNO);
    ...
    case coFloatsOrPercents: {
        ...
        //w12
        if (m_opt.opt_key == "thumbnail_size") {
...
#ifdef __WXOSX__
    text_ctrl->OSXDisableAllSmartSubstitutions(); // QDS
#endif // __WXOSX__
...
bool TextCtrl::value_was_changed()
{
    ...
    wxString   ret_str = text_ctrl()->GetValue(); // QDS
    // update m_value!
...
void TextCtrl::propagate_value()
{
    ...
    if (!is_defined_input_value<wxTextCtrl>(text_ctrl(), m_opt.type)) { // QDS
        ...
        if (!m_value.empty()) // QDS: null value
            on_kill_focus();
...
void TextCtrl::set_value(const boost::any& value, bool change_event/* = false*/) {
    ...
    if (m_opt.nullable) {
        ...
        text_ctrl()->SetValue(m_is_na_val ? na_value() :
                                            boost::any_cast<wxString>(value)); // QDS
    }
    else
        text_ctrl()->SetValue(value.empty() ? "" : boost::any_cast<wxString>(value)); // QDS // QDS: null value
...
{
    text_ctrl()->SetValue(boost::any_cast<wxString>(m_last_meaningful_value)); // QDS
    propagate_value();
}

void TextCtrl::set_na_value()
{
    text_ctrl()->SetValue(na_value()); // QDS
    propagate_value();
}
...
void TextCtrl::msw_rescale()
{
    ...
    {
        wxTextCtrl *field = text_ctrl(); // QDS
...
{
    window->Enable();
    text_ctrl()->SetEditable(true); // QDS
}
void TextCtrl::disable()
{
    window->Disable();
    text_ctrl()->SetEditable(false); // QDS
}
...
{
	m_disable_change_event = !change_event;
    dynamic_cast<::CheckBox *>(window)->SetValue(value); // QDS
	m_disable_change_event = false;
}
...
void CheckBox::set_value(const boost::any& value, bool change_event)
{
    ...
    if (m_opt.nullable) {
        ...
        if (!m_is_na_val)
            m_last_meaningful_value = value;
        dynamic_cast<::CheckBox*>(window)->SetValue(m_is_na_val ? false : boost::any_cast<unsigned char>(value) != 0); // QDS
    }
    else if (!value.empty()) // QDS: null value
        dynamic_cast<::CheckBox*>(window)->SetValue(boost::any_cast<bool>(value)); // QDS
...
void CheckBox::set_last_meaningful_value()
{
    if (m_opt.nullable) {
        m_is_na_val = false;
        dynamic_cast<::CheckBox*>(window)->SetValue(boost::any_cast<unsigned char>(m_last_meaningful_value) != 0); // QDS
        on_change_field();
    }
}
...
void CheckBox::set_na_value()
{
    if (m_opt.nullable) {
        m_is_na_val = true;
        dynamic_cast<::CheckBox *>(window)->SetValue(false); // QDS
        on_change_field();
    }
}
...
{
// 	boost::any m_value;
	bool value = dynamic_cast<::CheckBox*>(window)->GetValue(); // QDS
...
#endif //__WXGTK3__
	// temp->SetFont(Slic3r::GUI::wxGetApp().normal_font()); // QDS
    if (!wxOSX) temp->SetBackgroundStyle(wxBG_STYLE_PAINT);
	wxGetApp().UpdateDarkUI(temp);
...
void SpinCtrl::propagate_value()
{
    ...
    if (tmp_value == UNDEF_VALUE) {
        ...
        if (m_opt.min > 0 && tmp_value < m_opt.min) {
            ...
            // spin->GetText()->SetInsertionPointEnd(); // QDS
        }
#endif
...
#ifdef __WXOSX__
using choice_ctrl = ::ComboBox; // QDS
#else
using choice_ctrl = ::ComboBox; // QDS
#endif // __WXOSX__
...
#ifdef UNDEIFNED__WXOSX__ // __WXOSX__ // QDS
...
void Choice::propagate_value()
{
    ...
    else if (!m_value.empty()) // QDS: null value
        on_kill_focus();
}
