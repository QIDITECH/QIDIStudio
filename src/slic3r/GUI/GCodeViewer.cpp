...
namespace GUI {
    //y55
    bool GCodeViewer::is_show_speed = false;
//QDS translation of EViewType
...
static std::vector<std::array<float, 4>> decode_colors(const std::vector<std::string> &colors)
{
    ...
    for (size_t i = 0; i < colors.size(); ++i) { output[i] = decode_color(colors[i]); }
    return output;
}
...
float GCodeViewer::Extrusions::Range::get_color_size() const {
...
//QDS: GUI refactor: add canvas size from parameters
void GCodeViewer::SequentialView::Marker::render(int canvas_width, int canvas_height, const EViewType& view_type) const
{
    ...
    //QDS: minus the plate offset when show tool position
...
//QDS: GUI refactor: move to right
...
//void GCodeViewer::SequentialView::GCodeWindow::render(float top, float bottom, uint64_t curr_line_id) const
{
    ...
    //B
    static const ImVec4 LINE_NUMBER_COLOR = { 68.0f/ 255.0f, 121.0f / 255.0f, 251.0f / 255.0f, 1.0f };
    static const ImVec4 SELECTION_RECT_COLOR = { 68.0f/ 255.0f, 121.0f / 255.0f, 251.0f / 255.0f, 1.0f };
...
Slic3r::GUI::GCodeViewer::SequentialView::SequentialView(const GCodeViewer &outer_viewer) : m_gcode_viewer(outer_viewer) {
...
//            }
    ...
    if (preset_bundle != nullptr) {
        ...
        if (curr->is_system)
            ...
            if (filename.empty()) {
                filename = preset_bundle->get_hotend_model_for_printer_model(PresetBundle::QDT_DEFAULT_PRINTER_MODEL);
            }
        }
    }
    ...
    // QDS initialzed view_type items
    m_user_mode = mode;
...
void GCodeViewer::update_by_mode(ConfigOptionMode mode)
{
    ...
    view_type_items.push_back(EViewType::FilamentId); // QDS for first layer inspection
    ...
    //}
    //QDS: seam is not real move and extrusion, put at last line
                ...
                const std::vector<BoundingBoxf3>& exclude_bounding_box, bool initialized, ConfigOptionMode mode, bool only_gcode)
{
    ...
    //QDS: move the id to the end of reset
    ...
    // QDS: data for rendering color arrangement recommendation
...
{
    //QDS: should also reset the result id
...
void GCodeViewer::load_toolpaths(const GCodeProcessorResult& gcode_result, const BuildVolume& build_volume, const std::vector<BoundingBoxf3>& exclude_bounding_box)
{
    ...
    auto add_vertices_as_line = [](const GCodeProcessorResult::MoveVertex& prev, const GCodeProcessorResult::MoveVertex& curr, VertexBuffer& vertices) {
        ...
        //QDS: Has modified a lot for this function to support arc move
    ...
    //QDS: modify a lot to support arc travel
    auto add_indices_as_line = [](const GCodeProcessorResult::MoveVertex& prev, const GCodeProcessorResult::MoveVertex& curr, TBuffer& buffer,
        size_t& vbuffer_size, unsigned int ibuffer_id, IndexBuffer& indices, size_t move_id) {
            ...
            size_t loop_num = curr.is_arc_move_with_interpolation_points() ? curr.interpolation_points.size() : 0;
            for (size_t i = 0; i < loop_num + 1; i++) {
                //QDS: add previous index
                indices.push_back(static_cast<IBufferType>(indices.size()));
                //QDS: add current index
                indices.push_back(static_cast<IBufferType>(indices.size()));
    ...
    // QDS: also merge the point on arc to bounding box
    for (const GCodeProcessorResult::MoveVertex& move : gcode_result.moves) {
    ...
    for (size_t i = 0; i < m_moves_count; ++i) {
        ...
        // QDS: get the point number and then judge whether the remaining buffer is enough
    ...
    //QDS: generate map from ssid to move id in advance to reduce computation
    m_ssid_to_moveid_map.clear();
    ...
    //QDS: smooth toolpaths corners for the given TBuffer using triangles
    auto smooth_triangle_toolpaths_corners = [&gcode_result, this](const TBuffer& t_buffer, MultiVertexBuffer& v_multibuffer) {
        ...
        //QDS: modify a lot of this function to support arc move
        ...
        for (const Path& path : t_buffer.paths) {
            //QDS: the two segments of the path sharing the current vertex may belong
            ...
            // QDS: modify a lot to support arc move which has internal points
            for (size_t j = 1; j < path_vertices_count; ++j) {
                ...
                //QDS: select the subpaths which contains the previous/next segments
                ...
                if (j == path_vertices_count - 1) {
                    if (!gcode_result.moves[move_id].is_arc_move_with_interpolation_points())
                        break;   // QDS: the last move has no internal point.
                    loop_num--;  //QDS: don't need to handle the endpoint of the last arc move of path
                ...
                // QDS: smooth triangle toolpaths corners including arc move which has internal interpolation point
                for (int k = 0; k <= loop_num; k++) {
                    ...
                    if (can_displace) {
                        ...
                        if (!is_sharp) {
                            //QDS: matches outer corner vertices
                            if (is_right_turn)
                                match_left_vertices_with_internal_point(prev_sub_path, next_sub_path, curr_s_id, is_internal_point, k, vertex_size_floats, displacement_vec);
                            else
                                match_right_vertices_with_internal_point(prev_sub_path, next_sub_path, curr_s_id, is_internal_point, k, vertex_size_floats, displacement_vec);
    ...
    for (size_t i = 0; i < m_moves_count; ++i) {
        ...
        // if adding the vertices for the current segment exceeds the threshold size of the current vertex buffer
        // create another index buffer
        // QDS: support multi points in one MoveVertice, should multiply point number
        size_t vertices_size_to_add = (t_buffer.render_primitive_type == TBuffer::ERenderPrimitiveType::BatchedModel) ? t_buffer.model.data.vertices_size_bytes() : points_num * t_buffer.max_vertices_per_segment_size_bytes();
...
void GCodeViewer::load_shells(const Print& print, bool initialized, bool force_previewing)
{
    ...
    if ((print.id().id == m_shells.print_id)&&(print.get_modified_count() == m_shells.print_modify_count)) {
        //QDS: update force previewing logic
        if (force_previewing)
    ...
    //QDS: move behind of reset_shell, to clear previous shell for empty plate
    ...
    for (const PrintObject* obj : print.objects()) {
        ...
        // QDS: object may be deleted when this method is called when deleting an object
        if (object_idx == -1)
            continue;

        std::vector<int> instance_ids(model_obj->instances.size());
        //QDS: only add the printable instance
    ...
    for (GLVolume* volume : m_shells.volumes.volumes) {
        ...
        //QDS: add shell bounding box logic
...
void GCodeViewer::refresh_render_paths(bool keep_sequential_current_first, bool keep_sequential_current_last) const
{
                    ...
                    if (sub_path_id != -1) {
                        ...
                        if (offset > 0) {
                            if (buffer.render_primitive_type == TBuffer::ERenderPrimitiveType::Line) {
                                ...
                                // QDS: modify to support moves which has internal point
                                for (size_t i = sub_path.first.s_id + 1; i < m_sequential_view.current.last + 1; i++) {
                                    size_t move_id = m_ssid_to_moveid_map[i];
                                    const GCodeProcessorResult::MoveVertex& curr = m_gcode_result->moves[move_id];
                                    if (curr.is_arc_move()) {
    ...
    for (const auto& [tbuffer_id, ibuffer_id, path_id, sub_path_id] : paths) {
        ...
        RenderPath key{ tbuffer_id, color.get_data(), static_cast<unsigned int>(ibuffer_id), path_id };
...
{
    //QDS: add shell previewing logic
...
void GCodeViewer::render_all_plates_stats(const std::vector<const GCodeProcessorResult*>& gcode_result_list, bool show /*= true*/) const {
    ...
    ImGui::PushStyleColor(ImGuiCol_Header, ImVec4(0.27f, 0.47f, 0.98f, 1.0f));  // y96 change color
    ImGui::PushStyleColor(ImGuiCol_HeaderHovered, ImVec4(0.27f, 0.47f, 0.98f, 1.0f));   // y96 change color
...
void GCodeViewer::render_legend_color_arr_recommen(float window_padding)
{
    ...
    auto link_text = [&](const std::string &label) {
        ...
        ImColor HyperColor = ImColor(68, 121, 251, 255).Value;
    ...
    ////QDS Color Arrangement Recommendation
    ...
    // ImGui::Dummy({window_padding, window_padding});
    ImGui::BeginChild("#BOX", ImVec2(0, AMS_container_height), true, ImGuiWindowFlags_AlwaysUseWindowPadding);
    {
        ...
        ImGui::BeginChild("#LeftBOX", ImVec2(half_width, ams_item_height), false, ImGuiWindowFlags_AlwaysUseWindowPadding);
        {
        ...
        child_begin_draw_list->AddRectFilled(cursor_pos, ImVec2(cursor_pos.x + half_width, cursor_pos.y + line_height), IM_COL32(0, 0, 0, 20));
        ImGui::BeginChild("#RightBOX", ImVec2(half_width, ams_item_height), false, ImGuiWindowFlags_AlwaysUseWindowPadding);
        {
...
void GCodeViewer::render_legend(float &legend_height, int canvas_width, int canvas_height, int right_margin)
{
        ...
        std::function<void()> callback = nullptr)
    {
        ...
        if (callback) {
            ...
            bool b_menu_item = ImGui::QDTMenuItem(("##" + columns_offsets[0].first).c_str(), nullptr, false, true, max_height);
            ImGui::PopStyleVar(2);
            ...
            if (checkbox) {
                ...
                //y
                ImGui::PushStyleColor(ImGuiCol_CheckMark, ImVec4(0.27f, 0.47f, 0.98f, 1.00f));  // change color
    ...
    // QDS: no ColorChange type, use ToolChange
    ...
    if (is_use_helio){
        ...
        // QDS Set the width of the 8 "ABCD" words minus the "sliced result" to the spacing between the buttons and the title
        float single_word_width = imgui.calc_text_size("ABCD").x;
        float title_width       = imgui.calc_text_size(title).x;
        float spacing           = 18.0f * m_scale;
    ...
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.0f, 0.47f, 0.98f, 1.00f));
    ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(0.0f, 0.47f, 0.98f, 0.78f));
    ...
    if (ImGui::QDTBeginCombo("", view_type_value, flags)) {
    ...
    case EViewType::FeatureType:
    {
        ...
        //QDS: get travel time and percent
        {
    ...
    case EViewType::Tool:
{
    ...
    auto append_option_item = [this, append_item](EMoveType type, std::vector<float> offsets) {
        ...
        if (type == EMoveType::Travel) {
            //QDS: only display travel time in FeatureType view
    ...
    case EViewType::FeatureType:
    {
        ...
        for(auto item : options_items) {
            if (item != EMoveType::Travel) {
                ...
                //QDS: show travel time in FeatureType view
    ...
    case EViewType::ColorPrint:
    {
        ...
        //QDS display filament change times
        ImGui::Dummy({window_padding, window_padding});
        ImGui::SameLine();
    ...
    if (show_estimated) {
        ...
        if (m_view_type == EViewType::FeatureType) {
            //QDS display filament cost
            ImGui::Dummy({ window_padding, window_padding });
            ImGui::SameLine();
            imgui.text(total_filament_str + ":");
            ImGui::SameLine(max_len);
            //QDS: use current plater's print statistics
            bool imperial_units = wxGetApp().app_config->get("use_inches") == "1";
            char buf[64];
        ...
        //QDS: start gcode is mostly same with prepeare time
        if (time_mode.prepare_time != 0.0f) {
            ...
            if (timelapse_time != 0.0f)
                //y68
                imgui.text(short_time(get_time_dhms(time_mode.prepare_time + 600.0f)) + " + " + short_time(get_time_dhms(timelapse_time)));
            else
                imgui.text(short_time(get_time_dhms(time_mode.prepare_time + 600.0f)));
        }
        ImGui::Dummy({ window_padding, window_padding });
        ImGui::SameLine();
        ...
        ImGui::SameLine(max_len);
        imgui.text(short_time(get_time_dhms(time_mode.time + 600.0f))); //y68
...
void GCodeViewer::push_combo_style()
{
    ...
    //y
    ImGui::PushStyleColor(ImGuiCol_BorderActive, ImVec4(0.27f, 0.47f, 0.98f, 1.00f));   // change color
    ImGui::PushStyleColor(ImGuiCol_HeaderHovered, ImVec4(0.27f, 0.47f, 0.98f, 0.0f));   // change color
    ImGui::PushStyleColor(ImGuiCol_HeaderActive, ImVec4(0.27f, 0.47f, 0.98f, 1.0f));    // change color
}
