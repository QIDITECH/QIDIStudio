...
bool GLCanvas3D::_init_assemble_view_toolbar()
{
    ...
    //QDS: assemble toolbar is at the top and right, we don't need the rounded-corner effect at the left side and the top side
...
void GLCanvas3D::_load_print_toolpaths(const BuildVolume &build_volume)
{
    ...
    const std::array<float, 4> color = { 0.5f, 0.5f, 1.0f, 1.0f }; // y96 greenish
                ...
                notification_manager.push_slicing_customize_error_notification(NotificationType::QDTMixUsePLAAndPETG, NotificationLevel::WarningNotificationLevel, text, _u8L("Click Wiki for help."),
                    [](wxEvtHandler*) {
                        ...
                        if (language.find("zh") == 0)
                        	region = L"zh";
                        wxGetApp().open_browser_with_warning_dialog(wxString::Format(L"https://wiki.qiditech.com/%s/filament-acc/filament/h2d-pla-and-petg-mutual-support", region));
                        return false;
                    });
            }
            else
