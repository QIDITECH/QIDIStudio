...
void GLCanvas3D::_render_overlays()
{
    ...
    // y7
    const float size = float(GLToolbar::Default_Icons_Size * wxGetApp().toolbar_icon_scale(/*true*/));
    const float gizmo_size = 0.9 * size;
...
void GLCanvas3D::_load_print_object_toolpaths(const PrintObject& print_object, const BuildVolume& build_volume, const std::vector<std::string>& str_tool_colors, const std::vector<CustomGCode::Item>& color_print_values)
{
    ...
    struct Ctxt
    {
        ...
        static const std::array<float, 4>& color_support() { static std::array<float, 4> color = { 0.5f, 0.5f, 1.0f, 1.f }; return color; } // greenish
