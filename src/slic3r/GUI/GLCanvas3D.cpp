...
void GLCanvas3D::LayersEditing::render_variable_layer_height_dialog(const GLCanvas3D& canvas) {
    ...
    imgui.qdt_slider_float_style("##adaptive_slider", &m_adaptive_quality, 0.0f, 1.f, "%.2f");
    ImGui::SameLine();
    ...
    ImGui::QDTDragFloat("##adaptive_input", &m_adaptive_quality, 0.05f, 0.0f, 0.0f, "%.2f");
    ...
    //y
    ImGui::PushStyleColor(ImGuiCol_SliderGrabActive, ImVec4(0.27f, 0.47f, 0.98f, 1.00f));   // y96 change color
    if(ImGui::QDTSliderScalar("##radius_slider", ImGuiDataType_S32, &radius, &v_min, &v_max)){
    ...
    ImGui::PushStyleColor(ImGuiCol_FrameBgHovered, ImVec4(0.27f, 0.47f, 0.98f, 0.00f)); // y96 change color
    ImGui::PushStyleColor(ImGuiCol_FrameBgActive, ImVec4(0.27f, 0.47f, 0.98f, 0.00f));  // y96 change color
    if (ImGui::QDTDragScalar("##radius_input", ImGuiDataType_S32, &radius, 1, &v_min, &v_max)) {
        radius = std::clamp(radius, 1, 10);
        m_smooth_params.radius = (unsigned int)radius;
    }
    ImGui::PopStyleColor(3);

    imgui.qdt_checkbox("##keep_min", m_smooth_params.keep_min);
    ImGui::SameLine();
    ImGui::AlignTextToFramePadding();
...
void GLCanvas3D::SequentialPrintClearance::set_polygons(const Polygons& polygons, const std::vector<std::pair<Polygon, float>>& height_polygons)
{
    ...
    //QDS: add the height limit compute logic
...
//QDS: add arrange and orient event
...
void GLCanvas3D::load_arrange_settings()
{
    ...
    //QDS: add specific arrange settings
...
bool GLCanvas3D::init()
{
    ...
    //QDS load render color for style editor
...
//QDS: get current plater's bounding box
...
{
    //QDS: if not initialized, return inside directly insteadof assert
    if (!m_initialized) {
...
BoundingBoxf3 GLCanvas3D::scene_bounding_box() const
{
    ...
    //QDS merge plate scene bounding box
    if (m_canvas_type == ECanvasType::CanvasView3D) {
...
void GLCanvas3D::render(bool only_init)
{
    ...
    if (m_picking_enabled) {
        ...
        //QDS: enable picking when no volumes for partplate logic
    ...
    /* assemble render*/
        ...
        //QDS: add outline logic insteadof selection under assemble view
        //_render_selection();
        // QDS: add outline logic
        _render_objects(m_volumes, GLVolumeCollection::ERenderType::Transparent, b_with_stencil_outline);
    }
...
void GLCanvas3D::reload_scene(bool refresh_immediately, bool force_full_scene_refresh)
{
    ...
    // QDS: support wipe tower for multi-plates
    ...
    // QDS: normalize painting data with current filament count
    ...
    //QDS clean hover_volume_idxs
    m_hover_volume_idxs.clear();

    if (m_reload_delayed)
        return;

    // QDS: do not check wipe tower changes
    ...
    // Update the toolbar
    //QDS: notify the PartPlateList to reload all objects
    ...
    if (m_canvas_type != ECanvasType::CanvasAssembleView) {
        ...
        if (!m_volumes.empty()) {
           ...
           // const bool objectLimited = (state == ModelInstanceEPrintVolumeState::ModelInstancePVS_Limited);
            ...
            //QDS: turn off the warning when fully outside
...
void GLCanvas3D::load_gcode_preview(const GCodeProcessorResult& gcode_result, const std::vector<std::string>& str_tool_colors, bool only_gcode)
{
    ...
    //QDS: init is called in GLCanvas3D.render()
    ...
    if (wxGetApp().is_editor()) {
        //QDS: always load shell at preview, do this in load_shells
        _update_slice_error_status();
    }
...
void GLCanvas3D::on_char(wxKeyEvent& evt)
{
    ...
    //QDS: add orient deactivate logic
                    ...
                    if (m_canvas_type == CanvasPreview) {
                        ...
                        if (evt.CmdDown() || evt.ShiftDown()) {
                            ...
                            if (keyCode == WXK_UP || keyCode == WXK_DOWN) {
                                ...
                                // QDS set as dirty, update in render_gcode()
...
//#endif
        ...
        if (layer_editing_object_idx != -1 && m_layers_editing.bar_rect_contains(*this, pos(0), pos(1))) {
            ...
            // QDS: define Alt key to enable volume selection mode
        ...
        // do not process the dragging if the left mouse was set down in another canvas
            ...
            if ((any_gizmo_active || m_hover_volume_idxs.empty()) && m_mouse.is_start_position_3D_defined()) {
                ...
                if (this->m_canvas_type == ECanvasType::CanvasAssembleView || m_gizmos.is_paint_gizmo()) {
                    ...
                    //QDS do not limit rotate in assemble view
                    ...
                    if (wxGetApp().app_config->get("use_free_camera") == "1")
                        ...
                        if (evt.ControlDown() || evt.CmdDown()) {
                            ...
                            //QDS rotate with current plate center
                            PartPlate* plate = wxGetApp().plater()->get_partplate_list().get_curr_plate();
                            if (plate)
        ...
        if (m_layers_editing.state != LayersEditing::Unknown) {
            ...
            //QDS: don't use alt as de-select
        ...
        //QDS Select plate in this 3D canvas.
...
void GLCanvas3D::do_move(const std::string &snapshot_type,bool force_volume_move)
{
    ...
    //QDS: notify instance updates to part plater list
    ...
    for (const std::pair<int, int>& i : done) {
        ...
        //QDS: don't call translate if the z is zero
    ...
    //QDS: nofity object list to update
...
void GLCanvas3D::handle_sidebar_focus_event(const std::string& opt_key, bool focus_on)
{
    ...
    //QDS: this event was sent from gizmo now, no need to clear gizmo
...
GLCanvas3D::WipeTowerInfo GLCanvas3D::get_wipe_tower_info(int plate_idx) const
{
    ...
    for (const GLVolume* vol : m_volumes.volumes) {
        if (vol->is_wipe_tower && vol->object_idx() - 1000 == plate_idx) {
            ...
            // QDS: the wipe tower pos might be outside bed
            PartPlate* plate = wxGetApp().plater()->get_partplate_list().get_plate(plate_idx);
...
bool GLCanvas3D::_render_orient_menu(float left, float right, float bottom, float top)
{
    ...
    auto canvas_w = float(get_canvas_size().get_width());
    auto canvas_h = float(get_canvas_size().get_height());
    //QDS: GUI refactor: move main toolbar to the right
...
bool GLCanvas3D::_render_arrange_menu(float left, float toolbar_height)
{
    ...
    bool b_Spacing = imgui->qdt_slider_float_style("##Spacing", &settings.distance, dist_min, 100.0f, "%5.2f") || dist_min > settings.distance;
    ImGui::SameLine(window_width - slider_icon_width + 1.3 * cursor_slider_left);
    ImGui::PushItemWidth(1.5 * slider_icon_width);
    bool b_spacing_input = ImGui::QDTDragFloat("##spacing_input", &settings.distance, 0.05f, 0.0f, 0.0f, "%.2f");
    ...
    ImGui::Separator();
#if !QDT_RELEASE_TO_PUBLIC
    if (imgui->qdt_checkbox(_L("Save SVG"), settings.save_svg)) {
        settings_out.save_svg = settings.save_svg;
        appcfg->set("arrange", save_svg_key.c_str(), settings_out.save_svg ? "1" : "0");
        settings_changed = true;
    }
#endif
    if (imgui->qdt_checkbox(_L("Auto rotate for arrangement"), settings.enable_rotation)) {
        settings_out.enable_rotation = settings.enable_rotation;
        appcfg->set("arrange", rot_key.c_str(), settings_out.enable_rotation? "1" : "0");
        settings_changed = true;
    }

    if (imgui->qdt_checkbox(_L("Allow multiple materials on same plate"), settings.allow_multi_materials_on_same_plate)) {
    ...
    if (op && op->getBool()) {
        if (imgui->qdt_checkbox(_L("Avoid extrusion calibration region"), settings.avoid_extrusion_cali_region)) {
    ...
    // Align to Y axis. Only enable this option when auto rotation not enabled
    {
        ...
        if (imgui->qdt_checkbox(_L("Align to Y axis"), settings.align_to_y_axis)) {
...
bool GLCanvas3D::_init_main_toolbar()
{
    ...
    //QDS: main toolbar is at the top and left, we don't need the rounded-corner effect at the right side and the top side
...
void GLCanvas3D::_picking_pass()
{
    if (m_picking_enabled && !m_mouse.dragging && m_mouse.position != Vec2d(DBL_MAX, DBL_MAX)) {
        ...
        //QDS: only render plate in view 3D
        ...
        //QDS: remove the bed picking logic
...
void GLCanvas3D::_render_background() const
{
    ...
    if (wxGetApp().is_editor()) {
        ...
        if (!m_volumes.empty())
            ...
            //QDS: use current plater's bounding box
...
//QDS: GUI refactor: GLToolbar adjust
...
void GLCanvas3D::_render_imgui_select_plate_toolbar()
{
    ...
    if(m_sel_plate_toolbar.show_stats_item)
    {
        ...
        {
            text_clr = ImVec4(68.0f/ 255.0f, 121.0f / 255.0f, 251.0f / 255.0f, 0.2f);
...
void GLCanvas3D::_render_assembly_view_thumbnail_toolbar()
{
    auto close_assembly_notifications = []() {
        ...
        if (notify_mgr) {
            notify_mgr->close_notification_of_type(NotificationType::QDTIsolatedVolumeInfo);
            notify_mgr->close_notification_of_type(NotificationType::QDTAssemblyFarFromOrigin);
            notify_mgr->close_notification_of_type(NotificationType::QDTIntersectsVolumeInfo);
        }
    };
    ...
    if (ImGui::ImageButton3(help_id, help_id, button_icon_size, ImVec2(0, 0), ImVec2(1, 1), -1, ImVec4(0, 0, 0, 0), ImVec4(1, 1, 1, 1), ImVec2(0, 0))) {
        ...
        if (is_zh) {
            wxLaunchDefaultBrowser("https://wiki.qidi3d.com/zh/software/qidi-studio/toolbar/assembly-view-guide");
        } else {
            wxLaunchDefaultBrowser("https://wiki.qidi3d.com/en/software/qidi-studio/toolbar/assembly-view-guide");
        }
    }
...
void GLCanvas3D::_load_print_toolpaths(const BuildVolume &build_volume)
{
    ...
    for (size_t i = 0; i < skirt_height; ++ i) {
        ...
        //QDS: usage of m_brim are deleted
...
void GLCanvas3D::_load_wipe_tower_toolpaths(const BuildVolume& build_volume, const std::vector<std::string>& str_tool_colors)
{
    ...
    ctxt.print = print;
    ctxt.tool_colors = tool_colors.empty() ? nullptr : &tool_colors;
    //QDS: has no single_extruder_multi_material_priming
...
void GLCanvas3D::_render_thumbnail_internal(ThumbnailData& thumbnail_data, const ThumbnailsParams& thumbnail_params,
    ...
    const ExtraThumbData&              extra_thumb_data)
{
    //QDS modify visible calc function
    ...
    //QDS modify scene box to plate scene bounding box
    ...
    if (camera_view_angle_type == Camera::ViewAngleType::Top_Plate) {
        ...
        //QDS: use original iso view for thumbnail
    ...
    if (for_picking) {
        ...
        for (GLVolume* vol : visible_volumes) {
            //QDS set render color for thumbnails
...
void GLCanvas3D::_show_isolated_volumes_notification()
{
    ...
    notify_mgr->push_notification(NotificationType::QDTIsolatedVolumeInfo,
...
void GLCanvas3D::_check_assembly_far_from_origin()
{
    ...
    if (is_far) {
        ...
        notify_mgr->push_notification(NotificationType::QDTAssemblyFarFromOrigin,
                                      NotificationManager::NotificationLevel::ImportantNotificationLevel,
    ...
    const ExtraThumbData&              extra_thumb_data)
{
    ...
    //DQS modify visible calc function
    static std::array<float, 4>       curr_color;
    static const std::array<float, 4> orange = { 0.923f, 0.504f, 0.264f, 1.0f };
    static const std::array<float, 4> gray = { 0.64f, 0.64f, 0.64f, 1.0f };
    GLVolumePtrs                      visible_volumes;
...
void GLCanvas3D::_set_warning_notification(EWarning warning, bool state)
{
    ...
    case PLATER_WARNING:
        if (warning == EWarning::MixUsePLAAndPETG) {
            if (state) {
                notification_manager.push_slicing_customize_error_notification(NotificationType::QDTMixUsePLAAndPETG, NotificationLevel::WarningNotificationLevel, text, _u8L("Click Wiki for help."),
                    [](wxEvtHandler*) {
                        ...
                        if (language.find("zh") == 0)
                        	region = L"zh";
                        //y68
                        wxGetApp().open_browser_with_warning_dialog(wxString::Format(L"https://wiki.qidi3d.com/%s/Memo/Filament-Guide", region));
                        return false;
                    });
            }
            else
                notification_manager.close_slicing_customize_error_notification(NotificationType::QDTMixUsePLAAndPETG, NotificationLevel::WarningNotificationLevel);
        } else if (warning == EWarning::MultiFilaNoWipeTower) {
            if (state) {
                notification_manager.push_notification(NotificationType::QDTMultiFilaNoWipeTower, NotificationLevel::HintNotificationLevel, text, _u8L("Jump to: Prime tower"),
                                                       [](wxEvtHandler *) {
                                                           wxGetApp().sidebar().jump_to_option("enable_prime_tower", Preset::TYPE_PRINT, L"");
                                                           return false;
                                                       });
            } else {
                notification_manager.close_notification_of_type(NotificationType::QDTMultiFilaNoWipeTower);
            }
        }
        else if (warning == EWarning::NozzleFilamentIncompatible){
            if(state){
                notification_manager.push_slicing_customize_error_notification(NotificationType::QDTNozzleFilamentIncompatible, NotificationLevel::WarningNotificationLevel, text);
            }
            else{
                notification_manager.close_slicing_customize_error_notification(NotificationType::QDTNozzleFilamentIncompatible, NotificationLevel::WarningNotificationLevel);
            }
        }
        else if (warning == EWarning::TpuNozzleMultipleFilaments) {
            if (state) {
                notification_manager.push_slicing_customize_error_notification(NotificationType::QDTTpuNozzleHasMultiFilament, NotificationLevel::WarningNotificationLevel, text);
            } else {
            ...
            if (state) {
                notification_manager.push_slicing_customize_error_notification(NotificationType::QDTHighTempNeedWrappingDetection, NotificationLevel::WarningNotificationLevel, text,
            ...
            if (state) {
                notification_manager.push_slicing_customize_error_notification(
                    NotificationType::QDTSingleExtruderMixedFilamentRisk,
        ...
        if (warning == EWarning::LeftExtruderPrintableError) {
            if (state) {
                if (auto left_str = get_left_extruder_unprintable_text(); !left_str.empty())
                    notification_manager.qdt_show_filament_map_invalid_notification_before_slice(NotificationType::LeftExtruderUnprintableError, left_str);
            }
            else {
                notification_manager.qdt_close_filament_map_invalid_notification_before_slice(NotificationType::LeftExtruderUnprintableError);
            }
        }
            ...
            if (state) {
                if (auto right_str = get_right_extruder_unprintable_text(); !right_str.empty())
                    notification_manager.qdt_show_filament_map_invalid_notification_before_slice(NotificationType::RightExtruderUnprintableError, right_str);
            }
            else {
                notification_manager.qdt_close_filament_map_invalid_notification_before_slice(NotificationType::RightExtruderUnprintableError);
            }
        }
    ...
    case SLICING_ERROR:
        ...
        if (warning == EWarning::FilamentPrintableError) {
            if (state){
                ...
                notification_manager.push_slicing_customize_error_notification(NotificationType::QDTFilamentPrintableError, NotificationLevel::ErrorNotificationLevel, text,  _u8L("Click here to regroup"), callback);
            }
            else
                notification_manager.close_slicing_customize_error_notification(NotificationType::QDTFilamentPrintableError, NotificationLevel::ErrorNotificationLevel);
        }
            ...
            if (state) {
                ...
                notification_manager.push_flushing_volume_error_notification(NotificationType::QDTFlushingVolumeZero, NotificationLevel::WarningNotificationLevel, text, _u8L("Flushing Volume"), callback);
            } else
                notification_manager.close_flushing_volume_error_notification(NotificationType::QDTFlushingVolumeZero, NotificationLevel::WarningNotificationLevel);
        }
        else if (warning == EWarning::FilamentNozzleFlowIncompatible) {
            if (state) {
                notification_manager.push_slicing_customize_error_notification(NotificationType::QDTNozzleFilamentIncompatible, NotificationLevel::ErrorNotificationLevel, text);
            } else {
                notification_manager.close_slicing_customize_error_notification(NotificationType::QDTNozzleFilamentIncompatible, NotificationLevel::ErrorNotificationLevel);
            }
        }
        else {
            if (state)
    ...
    case SLICING_LIMIT_ERROR:
        if (state)
            notification_manager.qdt_show_filament_map_invalid_notification_after_slice(NotificationType::QDTSliceLimitError, text);
        else
            notification_manager.qdt_close_filament_map_invalid_notification_after_slice(NotificationType::QDTSliceLimitError);
        break;
    case SLICING_HEIGHT_OUTSIDE:
        if (state)
            notification_manager.push_slicing_customize_error_notification(NotificationType::QDTSliceMultiExtruderHeightOutside, NotificationLevel::ErrorNotificationLevel, text);
        else
            notification_manager.close_slicing_customize_error_notification(NotificationType::QDTSliceMultiExtruderHeightOutside, NotificationLevel::ErrorNotificationLevel);
        break;
...
//QDS: add deactivate orient menu
...
//QDS: add deactivate layersediting menu
