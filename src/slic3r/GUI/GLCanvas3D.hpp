...
//QDS: declare EVT_GLCANVAS_PLATE_SELECT
...
//        float distance_sla       = 6.;
    ...
    //QDS: add canvas type for assemble view usage
    enum ECanvasType
    {
        CanvasView3D = 0,
        CanvasPreview = 1,
...
private:
    ...
    bool m_dirty;
    bool m_initialized;
    //QDS: add flag to controll rendering
    ...
    //QDS if explosion_ratio is changed, need to update volume bounding box
    mutable float m_explosion_ratio = 1.0;
    mutable Vec3d m_rotation_center{ 0.0, 0.0, 0.0};
    //QDS store camera view
    Camera camera;
    ...
    Bed3D::Axes m_axes;
    //QDS:record key botton frequency
...
public:
    ...
    //QDS use gcoder viewer render calibration thumbnails
    ...
    // QDS: get empty cells to put new object
    ...
    // QDS: get the nearest empty cell
...
private:
    ...
    //QDS: add partplate print volume get function
    ...
    //QDS: add deactivate_orient_menu
    bool _deactivate_orient_menu();
    //QDS: add _deactivate_layersediting_menu
