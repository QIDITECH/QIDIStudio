...
{
    //QDS AssembleView canvas has its own selection
    ...
    wxDataViewCtrl(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxDV_MULTIPLE)
{
    ...
    //QDS: add part plate related event
    ...
    Bind(wxEVT_DATAVIEW_SELECTION_CHANGED, [this](wxDataViewEvent& event) {
        ...
        if (wxGetKeyState(WXK_SHIFT)) {
            ...
            // QDS: use wxDataViewCtrl's internal mechanism
#if 0
#ifdef __WXMSW__
			// Workaround for entering the column editing mode on Windows. Simulate keyboard enter when another column of the active line is selected.
		    wxDataViewItem    item;
...
//    Bind(wxEVT_KEY_DOWN, &ObjectList::OnChar, this);
    ...
    // QDS: dont need to do extra setting for a deleted object
...
void ObjectList::update_name_column_width() const
{
    ...
    // QDS: walkaround for wxDataViewCtrl::HasScrollbar() does not return correct status
    int others_width = 0;
    for (int cn = colName; cn < colCount; cn++) {
        if (cn != colName) {
...
void ObjectList::paste_volumes_into_list(int obj_idx, const ModelVolumePtrs& volumes)
{
    ...
    //QDS: notify partplate the modify
...
{
    // QDS Disable menu popup if current canvas is Preview
...
void ObjectList::load_generic_subobject(const std::string& type_name, const ModelVolumeType type)
{
    ...
    //QDS Switch to Objects List after add a modifier
...
void ObjectList::load_shape_object(const std::string &type_name)
{
    ...
    static const std::unordered_set<std::string> cooling_orientation_blacklist = {"QIDI Cube", "QIDI Cube V2", "ksr FDMTest"};
    bool skip_cooling_orientation = cooling_orientation_blacklist.find(type_name) != cooling_orientation_blacklist.end();
    //y75
    if (!skip_cooling_orientation && full_config.has("fan_direction") && full_config.has("auxiliary_fan") && full_config.option<ConfigOptionBool>("seal")->value)
    {
...
void ObjectList::del_instances_from_object(const int obj_idx)
{
    ...
    // QDS: remove snapshot name "Delete All Instances from Object"
    take_snapshot("");
...
bool ObjectList::del_subobject_from_object(const int obj_idx, const int idx, const int type)
{
    ...
    if (type == itVolume) {
        ...
        if (object->instances.size() == 1) {
            // QDS: remove snapshot name "Last instance of an object cannot be deleted."
            Slic3r::GUI::show_error(nullptr, "");
            return false;
        }

        // QDS: remove snapshot name "Delete Instance"
        take_snapshot("");
    ...
    if (to_multipart_object) {
        ...
        for (int obj_idx : obj_idxs) {
            ...
            for(int volume_idx = 0; volume_idx < object->volumes.size(); volume_idx++) {
                ...
                //QDS: add config from old objects
        ...
        //QDS init assemble transformation based on non-selected objects' assembly bounding box
...
void ObjectList::layers_editing()
{
    ...
    if (!layers_item.IsOk())
    {
        ...
        if (ranges.empty()) {
            // QDS: remove snapshot name "Add layers"
...
// QDS: merge parts of a single object into one volume, similar to export_stl, but no need to export and then import
void ObjectList::boolean()
{
    ...
    // QDS: ensure on bed but no need to ensure locate in the center around origin
    new_object->ensure_on_bed();
    new_object->center_around_origin();
    new_object->translate_instances(-new_object->origin_translation);
    new_object->origin_translation = Vec3d::Zero();

    // QDS: notify it before move
    notify_instance_updated(m_objects->size() - 1);
...
{
    auto model_object = (*m_objects)[obj_idx];
    //QDS start add obj_idx for debug
    PartPlateList& list = wxGetApp().plater()->get_partplate_list();
    if (notify_partplate) {
...
void ObjectList::add_layer_range_after_current(const t_layer_height_range current_range)
{
    ...
    if (++ it_next_range == ranges.end())
    {
        ...
        // QDS: remove snapshot name "Add Height Range"
        take_snapshot("");
        changed = true;
...
wxString ObjectList::can_add_new_range_after_current(const t_layer_height_range current_range)
{
    ...
    // QDS: remove all layer range message
...
bool ObjectList::edit_layer_range(const t_layer_height_range& range, const t_layer_height_range& new_range, bool dont_update_ui)
{
    ...
    // QDS: remove snapeshot name "Edit Height Range"
    take_snapshot("");
...
bool ObjectList::check_last_selection(wxString& msg_str)
{
        ...
        )
    {
        ...
        // QDS: change "Object or Instance" to "Object"
...
{
    // QDS: object (obj_idx calc in obj list) is already removed from m_objects in Plater::priv::remove().
#if 0
...
void GUI::ObjectList::smooth_mesh()
{
    ...
    auto show_warning_dlg = [this](int cur_face_count,std::string name,bool is_part) {
        ...
        if (cur_face_count > limit_face_count) {
            ...
            WarningDialog dlg(static_cast<wxWindow *>(wxGetApp().mainframe), (is_part ? _L("Part") : _L("Object")) + " " + content, _L("QIDIStudio warning"), wxYES_NO);
    ...
    auto show_smooth_mesh_error_dlg = [this](std::string name) {
        ...
        WarningDialog dlg(static_cast<wxWindow *>(wxGetApp().mainframe), content, _L("QIDIStudio warning"), wxOK);
        dlg.ShowModal();
    };
...
void ObjectList::set_extruder_for_selected_items(const int extruder)
{
    ...
    for (const wxDataViewItem& sel_item : sels)
    {
        ...
        // QDS: handle extruder 0 for part, use it's parent extruder
    ...
    // QDS: update extruder/filament column
    Refresh();
}
...
//QDS: notify partplate the instance added/updated
...
void ObjectList::update_after_undo_redo()
{
    ...
    //QDS: undo/redo will rebuild all the plates before
...
void ObjectList::toggle_printable_state()
{
    ...
    // QDS: remove snapshot name "Set Printable group", "Set Unprintable group", "Set Printable"...
    take_snapshot("");
