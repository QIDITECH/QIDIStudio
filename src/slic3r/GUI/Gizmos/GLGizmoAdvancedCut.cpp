...
void GLGizmoAdvancedCut::render_connectors()
{
    ...
    for (size_t i = 0; i < connectors.size(); ++i) {
        ...
        deal_connector_pos_by_type(pos, height, connector.attribs.type, connector.attribs.style, looking_forward, m_connectors_editing, m_clp_normal); // QDT
...
{
    if (m_part_selection && m_part_selection->valid() && show_in_3d == false && (m_is_dragging == false || m_connectors_editing)) // QDT
...
void GLGizmoAdvancedCut::render_cut_plane_input_window(float x, float y, float bottom_limit)
{
    ...
    ImGui::QDTInputDouble("##cut_rotation_x", &m_buffered_rotation[0], 0.0f, 0.0f, "%.2f");
    ImGui::SameLine(caption_size + 1 * unit_size + 2 * space_size);
    ImGui::PushItemWidth(unit_size);
    ImGui::QDTInputDouble("##cut_rotation_y", &m_buffered_rotation[1], 0.0f, 0.0f, "%.2f");
    ImGui::SameLine(caption_size + 2 * unit_size + 3 * space_size);
    ImGui::PushItemWidth(unit_size);
    ImGui::QDTInputDouble("##cut_rotation_z", &m_buffered_rotation[2], 0.0f, 0.0f, "%.2f");
    ...
    ImGui::QDTInputDouble("##cut_movement", &m_buffered_movement, 0.0f, 0.0f, "%.2f");
    ...
    ImGui::SameLine(caption_size + 1 * space_size);
    ImGui::PushItemWidth(3 * unit_size + 2 * space_size);
    ImGui::QDTInputDouble("##cut_height", &m_buffered_height, 0.0f, 0.0f, "%.2f");
    ...
    auto render_part_action_line = [this, space_size,has_connectors](const wxString &label, const wxString &suffix, bool &keep_part, bool &place_on_cut_part, bool &rotate_part) {
        ...
        m_imgui->qdt_checkbox(text, has_connectors ? keep : keep_part);
        ...
        if (m_imgui->qdt_checkbox(_L("Keep orientation") + suffix, is_keep)){
            rotate_part       = false;
            place_on_cut_part = false;
        }
        ImGui::SameLine();
        if (m_imgui->qdt_checkbox(_L("Place on cut") + suffix, place_on_cut_part)) {
            // in qdt_checkbox have done//place_on_cut_part = !place_on_cut_part;
            rotate_part       = false;
        }
        ImGui::SameLine();
        if (m_imgui->qdt_checkbox(_L("Flip") + suffix, rotate_part)) {
            //in qdt_checkbox have done://rotate_part       = !rotate_part;
            place_on_cut_part = false;
        }
        m_imgui->disabled_end();
    };
    ...
    m_imgui->qdt_checkbox(_L("Cut to parts"), m_cut_to_parts);
    if (m_cut_to_parts) {
...
void GLGizmoAdvancedCut::render_connectors_input_window(float x, float y, float bottom_limit)
{
    ...
    if (render_slider_double_input(_u8L("Size"), m_connector_size, m_connector_size_tolerance))
        ...
        m_imgui->qdt_checkbox(_L("Middle of geometry"), m_auto_center_connector);
        // ----------------------------------------
...
bool GLGizmoAdvancedCut::render_slider_double_input(const std::string &label, float &value_in, float &tolerance_in)
{
    ...
    m_imgui->qdt_slider_float_style(("##" + label).c_str(), &value, min_size, mean_size, format.c_str());

    ImGui::SameLine(left_width);
    ImGui::PushItemWidth(first_input_width);
    ImGui::QDTDragFloat(("##input_" + label).c_str(), &value, 0.05f, min_size, mean_size, format.c_str());
    ...
    m_imgui->qdt_slider_float_style(("##tolerance_" + label).c_str(), &tolerance, min_tolerance, 2.f, format.c_str(), 1.f, true, _L("Tolerance"));

    left_width += (slider_with + item_in_gap);
    ImGui::SameLine(left_width);
    ImGui::PushItemWidth(second_input_width);
    ImGui::QDTDragFloat(("##tolerance_input_" + label).c_str(), &tolerance, 0.05f, min_tolerance, 2.f, format.c_str());
...
bool GLGizmoAdvancedCut::render_slider_double_input_by_format(const std::string &label, float &value_in, float value_min, float value_max, DoubleShowType show_type)
{
    ...
    if (m_imgui->qdt_slider_float_style(("##" + label).c_str(), &value, value_min, value_max, format.c_str())) {
        if (show_type == DoubleShowType::PERCENTAGE) {
    ...
    ImGui::SameLine(left_width);
    ImGui::PushItemWidth(first_input_width);
    if (ImGui::QDTDragFloat(("##input_" + label).c_str(), &value, 0.05f, value_min, value_max, format.c_str())) {
        if (show_type == DoubleShowType::PERCENTAGE) {
            value_in = value * 0.01f;
        } else if (show_type == DoubleShowType::DEGREE) {
            value_in = Geometry::deg2rad(value);
