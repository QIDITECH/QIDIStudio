...
void GLGizmoMeasure::render_debug_dialog()
{
    auto add_feature_data = [this](const SelectedFeatures::Item& item) {
        ...
        add_strings_row_to_table(*m_imgui, "Type", ImGuiWrapper::COL_QIDI, text, ImGui::GetStyleColorVec4(ImGuiCol_Text));
        switch (item.feature->get_type())
        {
        case Measure::SurfaceFeatureType::Point:
        {
            add_strings_row_to_table(*m_imgui, "m_pt1", ImGuiWrapper::COL_QIDI, format_vec3(item.feature->get_point()), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            break;
        }
        case Measure::SurfaceFeatureType::Edge:
        {
            auto [from, to] = item.feature->get_edge();
            add_strings_row_to_table(*m_imgui, "m_pt1", ImGuiWrapper::COL_QIDI, format_vec3(from), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            add_strings_row_to_table(*m_imgui, "m_pt2", ImGuiWrapper::COL_QIDI, format_vec3(to), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            break;
        }
        case Measure::SurfaceFeatureType::Plane:
        {
            auto [idx, normal, origin] = item.feature->get_plane();
            add_strings_row_to_table(*m_imgui, "m_pt1", ImGuiWrapper::COL_QIDI, format_vec3(normal), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            add_strings_row_to_table(*m_imgui, "m_pt2", ImGuiWrapper::COL_QIDI, format_vec3(origin), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            add_strings_row_to_table(*m_imgui, "m_value", ImGuiWrapper::COL_QIDI, format_double(idx), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            break;
        }
        case Measure::SurfaceFeatureType::Circle:
        {
            ...
            add_strings_row_to_table(*m_imgui, "m_pt1", ImGuiWrapper::COL_QIDI, format_vec3(center), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            add_strings_row_to_table(*m_imgui, "m_pt2", ImGuiWrapper::COL_QIDI, format_vec3(normal), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            add_strings_row_to_table(*m_imgui, "m_value", ImGuiWrapper::COL_QIDI, format_double(radius), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            break;
        }
        }
        std::optional<Vec3d> extra_point = item.feature->get_extra_point();
        if (extra_point.has_value())
            add_strings_row_to_table(*m_imgui, "m_pt3", ImGuiWrapper::COL_QIDI, format_vec3(*extra_point), ImGui::GetStyleColorVec4(ImGuiCol_Text));
    };
    ...
    if (ImGui::BeginTable("Mode", 2)) {
        ...
        add_strings_row_to_table(*m_imgui, "Mode", ImGuiWrapper::COL_QIDI, txt, ImGui::GetStyleColorVec4(ImGuiCol_Text));
        ImGui::EndTable();
    }

    ImGui::Separator();
    if (ImGui::BeginTable("Hover", 2)) {
        add_strings_row_to_table(*m_imgui, "Hover id", ImGuiWrapper::COL_QIDI, std::to_string(m_hover_id), ImGui::GetStyleColorVec4(ImGuiCol_Text));
        const std::string txt = m_curr_feature.has_value() ? surface_feature_type_as_string(m_curr_feature->get_type()) : "None";
        add_strings_row_to_table(*m_imgui, "Current feature", ImGuiWrapper::COL_QIDI, txt, ImGui::GetStyleColorVec4(ImGuiCol_Text));
        ImGui::EndTable();
    }
    ...
    if (!m_selected_features.first.feature.has_value() && !m_selected_features.second.feature.has_value())
        ...
        if (m_selected_features.first.feature.has_value()) {
            m_imgui->text_colored(ImGuiWrapper::COL_QIDI, "Selection 1");
        ...
        if (m_selected_features.second.feature.has_value()) {
            m_imgui->text_colored(ImGuiWrapper::COL_QIDI, "Selection 2");
...
void GLGizmoMeasure::show_distance_xyz_ui()
{
    ...
    auto add_edit_distance_xyz_box = [this](Vec3d &distance) {
        m_imgui->disabled_begin(m_hit_different_volumes.size() == 1);
        {
            ...
            ImGui::QDTInputDouble("##measure_distance_x", &m_buffered_distance[0], 0.0f, 0.0f, "%.2f");

            ImGui::TableNextRow();
            ImGui::TableSetColumnIndex(0);
            m_imgui->text_colored(ImGuiWrapper::COL_GREEN, "Y:");
            ImGui::TableSetColumnIndex(1);
            ImGui::QDTInputDouble("##measure_distance_y", &m_buffered_distance[1], 0.0f, 0.0f, "%.2f");
            m_imgui->disabled_end();
            ...
            ImGui::TableSetColumnIndex(1);
            ImGui::QDTInputDouble("##measure_distance_z", &m_buffered_distance[2], 0.0f, 0.0f, "%.2f");
            m_imgui->disabled_end();
        }
        m_imgui->disabled_end();
    ...
    if (ImGui::BeginTable("Measure", 4)) {
        if (m_selected_features.second.feature.has_value()) {
            ...
            if (measure.angle.has_value() && m_measure_mode == EMeasureMode::ONLY_MEASURE)
                ...
                add_measure_row_to_table(_u8L("Angle"), ImGuiWrapper::COL_QIDI, format_double(Geometry::rad2deg(measure.angle->angle)),"°",
            ...
            if (measure.distance_infinite.has_value() && m_measure_mode == EMeasureMode::ONLY_MEASURE) {
                ...
                add_measure_row_to_table(show_strict ? _u8L("Perpendicular distance") : _u8L("Distance"), ImGuiWrapper::COL_QIDI, format_double(distance), m_units,
                                            ImGui::GetStyleColorVec4(ImGuiCol_Text));
                ++measure_row_count;
                ImGui::PopID();
            }
            if (show_strict &&
                ...
                add_measure_row_to_table(_u8L("Direct distance"), ImGuiWrapper::COL_QIDI, format_double(distance), m_units, ImGui::GetStyleColorVec4(ImGuiCol_Text));
                ++measure_row_count;
                ImGui::PopID();
            }
            if (measure.distance_xyz.has_value() && m_measure_mode == EMeasureMode::ONLY_MEASURE) {
                ...
                if (measure.distance_xyz->norm() > EPSILON) {
                    ...
                    add_measure_row_to_table(_u8L("Distance XYZ"), ImGuiWrapper::COL_QIDI, format_vec3(distance), m_units, ImGui::GetStyleColorVec4(ImGuiCol_Text));
                    ++measure_row_count;
                    ImGui::PopID();
                }
            }
        ...
        /*for (unsigned int i = measure_row_count; i < max_measure_row_count; ++i) {
            add_strings_row_to_table(*m_imgui, " ", ImGuiWrapper::COL_QIDI, " ", ImGui::GetStyleColorVec4(ImGuiCol_Text));
        }*/
        ImGui::EndTable();
    }
}
...
void GLGizmoMeasure::show_face_face_assembly_common() {
    if (m_measure_mode == EMeasureMode::ONLY_ASSEMBLY && m_hit_different_volumes.size() == 2 &&
        ...
        m_imgui->disabled_begin(!(action.can_set_to_center_coincidence));
        {
            ...
            ImGui::PushStyleColor(ImGuiCol_Button, m_is_dark_mode ? ImVec4(68 / 255.0, 121 / 255.0, 251 / 255.0, 1.0) : ImVec4(68 / 255.0, 121 / 255.0, 251 / 255.0, 1.0));
...
void GLGizmoMeasure::show_face_face_assembly_senior()
{
    if (m_measure_mode == EMeasureMode::ONLY_ASSEMBLY && m_hit_different_volumes.size() == 2 &&
        ...
        if (m_imgui->qdt_checkbox(_L("Flip by Face 2"), m_flip_volume_2)) {
        ...
        if (action.has_parallel_distance) {
            ...
            ImGui::PushItemWidth(m_input_size_max);
            ImGui::QDTInputDouble("##parallel_distance_z", &m_buffered_parallel_distance, 0.0f, 0.0f, "%.2f");
        ...
        if (action.can_around_center_of_faces) {
            ...
            ImGui::PushItemWidth(m_input_size_max);
            ImGui::QDTInputDouble("##rotate_around_center", &m_buffered_around_center, 0.0f, 0.0f, "%.2f");
