...
//QDS: use the global one in 3DScene.cpp
...
void GLGizmoMmuSegmentation::render_triangles(const Selection &selection) const
{
    ...
    //QDS: to improve the random white pixel issue
...
void GLGizmoMmuSegmentation::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    }
    {
        ...
        ImGui::PushStyleColor(ImGuiCol_Border, ImVec4(0.27f, 0.47f, 0.98f, 1.00f)); // y96 change color
    ...
    for (int i = 0; i < tool_ids.size(); i++) {
        ...
        if (m_current_tool == tool_ids[i]) {
            ...
            ImGui::PushStyleColor(ImGuiCol_ButtonActive, m_is_dark_mode ? ImVec4(43 / 255.0f, 64 / 255.0f, 54 / 255.0f, 1.00f) : ImVec4(0.86f, 0.99f, 0.91f, 1.00f));
            ImGui::PushStyleColor(ImGuiCol_Border, ImVec4(0.27f, 0.47f, 0.98f, 1.00f));
    ...
    if (m_current_tool == ImGui::CircleButtonIcon || m_current_tool == ImGui::SphereButtonIcon) {
        ...
        m_imgui->qdt_slider_float_style("##cursor_radius", &m_cursor_radius, CursorRadiusMin, CursorRadiusMax, "%.2f", 1.0f, true);
        ImGui::SameLine(drag_left_width + circle_max_width);
        ImGui::PushItemWidth(1.5 * slider_icon_width);
        ImGui::QDTDragFloat("##cursor_radius_input", &m_cursor_radius, 0.05f, 0.0f, 0.0f, "%.2f");
        ...
        ImGui::SameLine(circle_max_width);
        ImGui::PushItemWidth(sliders_width);
        bool slider_clp_dist = m_imgui->qdt_slider_float_style("##clp_dist", &clp_dist, 0.f, 1.f, "%.2f", 1.0f, true);
        ImGui::SameLine(drag_left_width + circle_max_width);
        ImGui::PushItemWidth(1.5 * slider_icon_width);
        bool b_clp_dist_input = ImGui::QDTDragFloat("##clp_dist_input", &clp_dist, 0.05f, 0.0f, 0.0f, "%.2f");
        ...
        m_cursor_type = TriangleSelector::CursorType::POINTER;
        m_imgui->qdt_checkbox(m_desc["edge_detection"], m_detect_geometry_edge);
        ...
        if (m_detect_geometry_edge) {
            ...
            ImGui::PushItemWidth(sliders_width);
            if (m_imgui->qdt_slider_float_style("##smart_fill_angle", &m_smart_fill_angle, SmartFillAngleMin, SmartFillAngleMax, format_str.data(), 1.0f, true))
                for (auto &triangle_selector : m_triangle_selectors) {
                    triangle_selector->seed_fill_unselect_all_triangles();
                    triangle_selector->request_update_render_data();
                }
            ...
            ImGui::PushItemWidth(1.5 * slider_icon_width);
            ImGui::QDTDragFloat("##smart_fill_angle_input", &m_smart_fill_angle, 0.05f, 0.0f, 0.0f, "%.2f");
        } else {
        ...
        m_imgui->qdt_slider_float_style("##cursor_height", &m_cursor_height, CursorHeightMin, CursorHeightMax, format_str.data(), 1.0f, true);
        ImGui::SameLine(drag_left_width + height_max_width);
        ImGui::PushItemWidth(1.5 * slider_icon_width);
        ImGui::QDTDragFloat("##cursor_height_input", &m_cursor_height, 0.05f, 0.0f, 0.0f, "%.2f");

        m_imgui->qdt_checkbox(_L("Place input box of bottom near mouse"), m_lock_x_for_height_bottom);
        ImGui::Separator();
        if (m_c->object_clipper()->get_position() == 0.f) {
            ImGui::AlignTextToFramePadding();
            m_imgui->text(m_desc.at("clipping_of_view"));
        ...
        m_imgui->qdt_slider_float_style("##gap_area", &TriangleSelectorPatch::gap_area, TriangleSelectorPatch::GapAreaMin, TriangleSelectorPatch::GapAreaMax, format_str.data(), 1.0f, true);
        ImGui::SameLine(drag_left_width + gap_area_slider_left);
        ImGui::PushItemWidth(1.5 * slider_icon_width);
        ImGui::QDTDragFloat("##gap_area_input", &TriangleSelectorPatch::gap_area, 0.05f, 0.0f, 0.0f, "%.2f");
    }
    ImGui::Separator();
    if (m_current_tool == ImGui::CircleButtonIcon || m_current_tool == ImGui::SphereButtonIcon) {
        ...
        if (m_imgui->qdt_checkbox(_L("Vertical"), vertical_only)) {
        ...
        if (m_imgui->qdt_checkbox(_L("Horizontal"), horizontal_only)) {
        ...
        m_imgui->qdt_checkbox(_L("View: keep horizontal"), is_front_view);
...
void GLGizmoMmuSegmentation::init_model_triangle_selectors()
{
    ...
    // QDS: Don't continue when model object is null
    if (mo == nullptr)
        return;
