...
Emboss::CreateVolumeParams GLGizmoSVG::create_input(GLCanvas3D &canvas, ModelVolumeType volume_type)
{
    ...
    const GLVolume *gl_volume = canvas.get_selection().get_first_volume();//modify by qds //get_first_hovered_gl_volume(canvas);
...
void GLGizmoSVG::on_set_state() {
   ...
   // m_parent.set_raycaster_gizmos_on_top(GLGizmoBase::m_state == GLGizmoBase::On);
    ...
    if (GLGizmoBase::m_state == GLGizmoBase::Off) {
        ...
        m_parent.enable_moving(true); // modify by qds
        reset_volume();
...
void GLGizmoSVG::on_render()
{
    ...
    if (!m_volume->is_the_only_one_part()) {
        m_parent.enable_moving(false); // modify by qds
...
void GLGizmoSVG::draw_use_surface()
{
    ...
    if (m_imgui->qdt_checkbox("##useSurface", m_volume_shape.projection.use_surface)) {
        process_job();
    }
}
