...
void GLGizmoSimplify::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    // QDS: somehow the calculated utf8 width is too narrow, have to add 35 here
    ...
    if (m_imgui->qdt_radio_button("##use_error", !m_configuration.use_count)) {
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.27f, 0.47f, 0.98f, 1.00f));   // y96
    ImGui::PushStyleColor(ImGuiCol_SliderGrab, ImVec4(0.27f, 0.47f, 0.98f, 1.00f));

    if (m_imgui->qdt_sliderin("##ReductionLevel", &reduction, 0, 4, reduce_captions[reduction].c_str())) {
    ...
    if (m_imgui->qdt_radio_button("##use_count", m_configuration.use_count)) {
        m_configuration.use_count = !m_configuration.use_count;
        start_process = true;
    }
    ...
    if (m_imgui->qdt_slider_float_style("##decimate_ratio", &m_configuration.decimate_ratio, 0.f, 100.f, format)) {
    ...
    if (ImGui::QDTDragFloat("##decimate_ratio_input", &m_configuration.decimate_ratio, 0.05f, 0.0f, 0.0f, "%.2f%%")) {
        if (m_configuration.decimate_ratio < 0.f)
            m_configuration.decimate_ratio = 0.f;
        if (m_configuration.decimate_ratio > 100.f)
            m_configuration.decimate_ratio = 100.f;
    ...
    m_imgui->qdt_checkbox(_L("Show wireframe").c_str(), m_show_wireframe);
    ...
    if (is_worker_running) { // apply or preview
        ...
        ImGui::QDTProgressBar2(progress / 100., progress_size);
        ImGui::SameLine();
    ...
    if (m_imgui->qdt_button(_L("Apply"))) {
        apply_simplify();
    }
    ...
    if (m_imgui->qdt_button(_L("Cancel"))) {
        close();
    }
