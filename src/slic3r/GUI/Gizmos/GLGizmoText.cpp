...
void GLGizmoText::draw_style_list(float caption_size)
{
    ...
    if (m_style_manager.exist_stored_style())
        ...
        ImGui::TextColored(ImGuiWrapper::COL_QIDI, "%s", title.c_str());
    ...
    if (ImGui::QDTBeginCombo("##style_selector", add_text_modify(trunc_name).c_str())) {
        ...
        for (const StyleManager::Style &style : styles) {
            ...
            if (ImGui::QDTSelectable(style.truncated_name.c_str(), is_selected, flags, select_size)) {
...
void GLGizmoText::draw_style_save_as_popup()
{
    ...
    std::string&  new_name     = m_style_new_name; // text_volume->get_text_configuration()->style.name;//QDS modify
...
void GLGizmoText::draw_rotation(int caption_size, int slider_width, int drag_left_width, int slider_icon_width)
{
    ...
    if (m_imgui->qdt_slider_float_style("##angle", &m_rotate_angle, -180.f, 180.f, "%.2f", 1.0f, true ,_L("Rotate the text counterclockwise."))) {
        update_style_angle(text_volume, angle_degree, m_rotate_angle);
    }
    ImGui::SameLine(drag_left_width);
    ImGui::PushItemWidth(1.5 * slider_icon_width);
    bool set_rotate_angle_flag = false;
    if (ImGui::QDTDragFloat("##angle_input", &m_rotate_angle, 0.05f, -180.f, 180.f, "%.2f")) {
    ...
    const std::string &text, Emboss::StyleManager &style_manager, const Selection &selection, ModelVolumeType type, std::shared_ptr<std::atomic<bool>> &cancel)
{
    ...
    style.projection.depth = m_thickness; // QDS add
    style.projection.embeded_depth = m_embeded_depth; // QDS add
...
void GLGizmoText::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    ImGui::SameLine(caption_size);
    ImGui::PushItemWidth(slider_width);
    if (m_imgui->qdt_slider_float_style("##text_gap", &m_text_gap, -10.f, 100.f, "%.2f", 1.0f, true))
        m_need_update_text = true;

    ImGui::SameLine(drag_left_width);
    ImGui::PushItemWidth(1.5 * slider_icon_width);
    if (ImGui::QDTDragFloat("##text_gap_input", &m_text_gap, 0.05f, -10.f, 100.f, "%.2f")) {
        bool need_deal = false;
...
void GLGizmoText::draw_text_input(int caption_width)
{
    ...
    if (!exist_font) {
        ...
        if (is_text_empty(m_text))  {//QDS modify
       ...
       /* if (prop.skew.has_value())//QDS modify
...
void GLGizmoText::draw_font_list()
{
    ...
    if (m_imgui->qdt_combo_with_filter("##Combo_Font", selected, m_face_names->faces_names, &filtered_items_idx, &is_filtered, m_imgui->scaled(32.f / 15.f))) {
        ...
        for (int i = 0; i < show_items_count; i++) {
            ...
            if (ImGui::QDTSelectable(face.name_truncated.c_str(), is_selected, flags, selectable_size)) {
...
bool GLGizmoText::revertible(const std::string &name, T &value, const T *default_value, const std::string &undo_tooltip, float undo_offset, Draw draw) const
{
    ...
    if (changed || default_value == nullptr)
        ImGuiWrapper::text_colored(ImGuiWrapper::COL_QIDI_CHANGE, name);
    else
...
void GLGizmoText::draw_advanced(float caption_size, float slider_width, float slider_icon_width, float drag_left_width)
{
   ...
   /* bool exist_change = false;
    ...
    ImGui::PushItemWidth(slider_width);
    if (m_imgui->qdt_slider_float_style("##text_boldness", &m_custom_boldness, min_boldness, max_boldness, "%.0f", 1.0f, true)) {
        set_default_boldness(boldness);
    ...
    ImGui::PushItemWidth(1.5 * slider_icon_width);
    if (ImGui::QDTDragFloat("##text_boldness_input", &m_custom_boldness, 1, min_boldness, max_boldness, "%.0f")) {
        bool need_deal = false;
    ...
    ImGui::SameLine(caption_size + ad_space_size);
    ImGui::PushItemWidth(slider_width);
    if (m_imgui->qdt_slider_float_style("##text_skew", &m_custom_skew, min_skew, max_skew, "%.1f", 1.0f, true)) {
        skew = m_custom_skew;
    }
    ...
    ImGui::SameLine(drag_left_width + ad_space_size);
    ImGui::PushItemWidth(1.5 * slider_icon_width);
    if (ImGui::QDTDragFloat("##text_skew_input", &m_custom_skew, 1, min_skew, max_skew, "%.1f")) {
        bool need_deal = false;
...
void init_face_names(CurFacenames &face_names)
{
    ...
    wxFontEnumerator::InvalidateCache();
    std::vector<wxString> delete_bad_font_list;//QDS
#ifdef _WIN32
    ...
    if (face_names.hash == hash) {
        if (face_names.faces_names.size() == 0) {//FIX by qds
