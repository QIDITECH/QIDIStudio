...
namespace Slic3r {
namespace GUI {
//QDS: GUI refactor: to support top layout
...
GLGizmosManager::GLGizmosManager(GLCanvas3D& parent)
    ...
    , m_tooltip("")
    , m_serializing(false)
    //QDS: GUI refactor: add object manipulation in gizmo
...
//QDS: GUI refactor: GLToolbar&&Gizmo adjust
...
void GLGizmosManager::update_data()
{
    ...
    // QDS: when select multiple objects, uniform scale can be deselected, display the 0-5 grabbers
    ...
    else if (!evt.HasModifiers())
    {
        ...
        //{
           /* if (m_current == Scale)
            ...
            //break;
        //}
        // QDS: Skip all keys when in gizmo. This is necessary for 3D text tool.
        default:
        {
...
void GLGizmosManager::render_background(float left, float top, float right, float bottom, float border) const
{
    ...
    {
        //QDS: GUI refactor: remove the corners of gizmo
...
void GLGizmosManager::do_render_overlay() const
{
    ...
    // QDS simplify gizmo is not a selected gizmo and need to render input window
...
bool GLGizmosManager::is_in_editing_mode(bool error_notification) const
{
    ...
    // QDS: remove SLA editing notification
    return true;
}
