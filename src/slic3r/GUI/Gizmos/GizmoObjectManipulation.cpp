 ...
 void GizmoObjectManipulation::do_render_move_window(ImGuiWrapper *imgui_wrapper, std::string window_name, float x, float y, float bottom_limit)
{
    ...
    ImGui::PushItemWidth(unit_size);
    ImGui::QDTInputDouble(label_values[0][0], &display_position[0], 0.0f, 0.0f, "%.2f", 0, true);
    ImGui::SameLine(caption_max + unit_size + (++index) * space_size + intput_box_space_size);
    ImGui::PushItemWidth(unit_size);
    ImGui::QDTInputDouble(label_values[0][1], &display_position[1], 0.0f, 0.0f, "%.2f", 0, true);
    ImGui::SameLine(caption_max + (++index_unit) * unit_size + (++index) * space_size + intput_box_space_size + space_size *0.75f);
    ImGui::PushItemWidth(unit_size);
    ImGui::QDTInputDouble(label_values[0][2], &display_position[2], 0.0f, 0.0f, "%.2f", 0, true);
