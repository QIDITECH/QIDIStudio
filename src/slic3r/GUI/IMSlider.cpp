...
static const ImU32 BRAND_COLOR            = IM_COL32(68, 121, 251, 255);
...
IMSlider::IMSlider(int lowerValue, int higherValue, int minValue, int maxValue, long style)
{
    ...
    // QDS set to none style by default
...
{
    //QDT only support MultiExtruder
...
void IMSlider::draw_ticks(const ImRect& slideable_region) {
    ...
    const ImU32 tick_hover_box_clr = m_is_dark ? IM_COL32(65, 65, 71, 255) : IM_COL32(10, 255, 255, 255);   // y96
...
bool IMSlider::render(int canvas_width, int canvas_height)
{
    ...
    ImGui::PushStyleColor(ImGuiCol_::ImGuiCol_Text, ImVec4(0.27f, 0.47f, 0.98f, 1.0f)); // change color
...
void IMSlider::render_input_custom_gcode(std::string custom_gcode)
{
    ...
    if (ImGui::BeginPopupModal((_u8L("Custom G-code")).c_str(), NULL, windows_flag))
    {
        ...
        if (imgui.qdt_button(_L("Cancel")) || ImGui::IsKeyDown(ImGui::GetKeyIndex(ImGuiKey_Escape))) {
            m_show_custom_gcode_window = false;
            ImGui::CloseCurrentPopup();
            set_focus = true;
        }
...
void IMSlider::render_go_to_layer_dialog()
{
    ...
    if (ImGui::BeginPopupModal((_u8L("Jump to Layer")).c_str(), NULL, windows_flag))
    {
        ...
        if (imgui.qdt_button(_L("OK")) || (!disable_button && ImGui::IsKeyDown(ImGui::GetKeyIndex(ImGuiKey_Enter)))) {
...
void IMSlider::render_add_menu()
{
    ...
    if (ImGui::BeginPopup("slider_add_menu_popup")) {
        ...
        //QDS render this menu item only when extruder_num > 1
        if (extruder_num > 1) {
...
{
    // QDS: This function is useless in QDS
    return false;
