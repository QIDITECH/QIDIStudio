...
static const std::map<const wchar_t, std::string> font_icons_extra_large = {
    //QDS do not use notification_clippy
...
void ImGuiWrapper::new_frame()
{
    ...
    // QDT: we should render the new frame first, than reset keys' status

    // QDT: copy & paste form prusa github repo (https://github.com/prusa3d/PrusaSlicer/blob/master/src/slic3r/GUI/ImGuiWrapper.cpp#L375C5-L402C6)
...
/* QDT style widgets */
bool ImGuiWrapper::qdt_combo_with_filter(const char* label, const std::string& preview_value, const std::vector<std::string>& all_items, std::vector<int>* filtered_items_idx, bool* is_filtered, float item_height)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    ...
    ImGui::PopStyleVar();
    ImGui::PopStyleColor();
    ImGui::QDTRenderArrow(window->DrawList, arrow_bb.Min + ImVec2(ImMax(0.0f, (arrow_size.x - g.FontSize) * 0.5f), ImMax(0.0f, (arrow_size.y - g.FontSize) * 0.5f)), ImGui::GetColorU32(ImGuiCol_Text), ImGuiDir_Down);
    ...
    if (ImGui::BeginPopup(label))
    {
        ...
        ImGui::InputText("##qdt_combo_with_filter_inputText", pattern_buffer, sizeof(pattern_buffer));
        ImGui::PopItemWidth();
        ImGui::PopStyleVar();
        ...
        *is_filtered = is_filtering;
        ...
        if (ImGui::BeginListBox("##qdt_combo_with_filter_listBox", { item_rect_width, item_rect_height * 7.75f})) {
            ImGui::PopStyleVar(2);
            return true;
        }
        else
...
bool ImGuiWrapper::qdt_input_double(const wxString& label, const double& value, const std::string& format)
{
    //return ImGui::InputDouble(label.c_str(), const_cast<double *>(&value), 0.0f, 0.0f, format.c_str(), ImGuiInputTextFlags_CharsDecimal);
    return ImGui::InputDouble(label.c_str(), const_cast<double *>(&value), 0.0f, 0.0f, format.c_str(), ImGuiInputTextFlags_CharsDecimal);
}

bool ImGuiWrapper::qdt_slider_float_style(const std::string &label, float *v, float v_min, float v_max, const char *format, float power, bool clamp, const wxString &tooltip)
{
    ...
    bool ret = qdt_slider_float(label, v, v_min,v_max, format, power, clamp,tooltip);
...
bool ImGuiWrapper::qdt_slider_float(const std::string& label, float* v, float v_min, float v_max, const char* format, float power, bool clamp, const wxString& tooltip)
{
    ...
    bool ret = ImGui::QDTSliderFloat(str_label.c_str(), v, v_min, v_max, format, power);
...
bool ImGuiWrapper::qdt_button(const wxString &label)
{
    auto label_utf8 = into_u8(label);
    return ImGui::QDTButton(label_utf8.c_str());
}
...
bool ImGuiWrapper::qdt_checkbox(const wxString &label, bool &value, bool enabled, bool b_dark_mode)
{
    bool result;
    bool b_value = value;
    if (b_value) {
        ImGui::PushStyleColor(ImGuiCol_FrameBg, ImVec4(0.27f, 0.47f, 0.98f, 1.00f));    // y96 change color
...
bool ImGuiWrapper::qdt_radio_button(const char *label, bool active)
{
    bool result;
    bool b_value = active;
    if (b_value) {
    ...
    result = ImGui::QDTRadioButton(label,active);
    if (b_value) { ImGui::PopStyleColor(3); }
    return result;
}

bool ImGuiWrapper::qdt_sliderin(const char *label, int *v, int v_min, int v_max, const char *format, ImGuiSliderFlags flags)
{
    return ImGui::QDTSliderScalarIn(label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags);
}
...
void ImGuiWrapper::push_menu_style(const float scale)
{
    if (m_is_dark_mode) {
        ...
        ImGui::PushStyleColor(ImGuiCol_HeaderHovered, ImVec4(0.27f, 0.47f, 0.98f, 1.0f));
...
void ImGuiWrapper::push_common_window_style(const float scale) {
    if (m_is_dark_mode) {
        ...
        ImGui::PushStyleColor(ImGuiCol_Separator, ImGuiWrapper::COL_SEPARATOR_DARK);                             // 5
        ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(1.00f, 1.00f, 1.00f, 1.00f));                              // 6
        ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.27f, 0.47f, 0.98f, 1.00f));                       // 7
        ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(0.27f, 0.47f, 0.98f, 1.00f));                        // 8
        ImGui::PushStyleColor(ImGuiCol_FrameBgHovered, ImVec4(84 / 255.0f, 84 / 255.0f, 90 / 255.0f, 1.00f));    // 9
        ImGui::PushStyleColor(ImGuiCol_FrameBgActive, ImVec4(62 / 255.0f, 62 / 255.0f, 69 / 255.0f, 1.00f));     // 10
        ImGui::PushStyleColor(ImGuiCol_FrameBg, ImVec4(238 / 255.0f, 238 / 255.0f, 238 / 255.0f, 0.00f));        // 11
        ...
        ImGui::PushStyleColor(ImGuiCol_PlotHistogram, ImVec4(0.27f, 0.47f, 0.98f, 1.00f));                       // 14
    }
    else {
        ImGui::PushStyleVar(ImGuiStyleVar_FrameBorderSize, 1.0f * scale);
        ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(20.0f, 10.0f) * scale);
...
void ImGuiWrapper::push_combo_style(const float scale)
{
    if (m_is_dark_mode) {
        ...
        ImGui::PushStyleColor(ImGuiCol_PopupBg, ImGuiWrapper::COL_WINDOW_BG);
        ImGui::PushStyleColor(ImGuiCol_BorderActive, ImVec4(0.27f, 0.47f, 0.98f, 1.00f));
        ImGui::PushStyleColor(ImGuiCol_HeaderHovered, ImVec4(0.27f, 0.47f, 0.98f, 0.5f));
        ImGui::PushStyleColor(ImGuiCol_HeaderActive, ImVec4(0.27f, 0.47f, 0.98f, 1.0f));
        ImGui::PushStyleColor(ImGuiCol_Header, ImVec4(0.27f, 0.47f, 0.98f, 1.0f));
...
void ImGuiWrapper::filament_group(const std::string& filament_type, const char* hex_color, unsigned char filament_id, float align_width)
{
    ...
    static ImTextureID transparent;
    ImVec2             text_size = ImGui::CalcTextSize(filament_type.c_str());
    // QDS image sizing based on text width (DPI scaling)
