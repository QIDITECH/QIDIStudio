...
namespace instance_check_internal
{
	...
	static HWND l_qidi_studio_hwnd;
	static BOOL CALLBACK EnumWindowsProc(_In_ HWND   hwnd, _In_ LPARAM lParam)
	{
		...
		if (wndTextString.find(L"QIDIStudio") != std::wstring::npos && classNameString == L"wxWindowNR") {
	...
	static bool send_message(const std::string& message, const std::string &version)
	{
		if (!EnumWindows(EnumWindowsProc, 0)) {
			...
			SendMessage(l_qidi_studio_hwnd, WM_COPYDATA, 0, (LPARAM)&data_to_send);
			return true;
		}
	    return false;
	}
...
namespace MessageHandlerDBusInternal
{
	...
	//method AnotherInstance receives message from another QIDIStudio instance
...
void OtherInstanceMessageHandler::listen()
{
	...
	if (DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER != name_req_val) {
		BOOST_LOG_TRIVIAL(error) << "Not primary owner of DBus name - probably another QIDIStudio instance is running.";
	    BOOST_LOG_TRIVIAL(error) << "Dbus Messages listening terminating.";
	    dbus_connection_unref(conn);
	    return;
	}
