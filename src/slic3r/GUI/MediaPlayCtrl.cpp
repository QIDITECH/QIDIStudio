...
//y76
MediaPlayCtrl::MediaPlayCtrl(wxWindow *parent, VideoPanel *media_ctrl, const wxPoint &pos, const wxSize &size)
...
void MediaPlayCtrl::Play()
{
    ...
    if (m_lan_proto > MachineObject::LVL_Disable && (m_lan_mode || !m_remote_proto) && !m_disable_lan && !m_lan_ip.empty()) {
        ...
        if (m_lan_proto == MachineObject::LVL_Local)
            url = "qidi:///local/" + m_lan_ip + ".?port=6000&user=" + m_lan_user + "&passwd=" + m_lan_passwd;
        else if (m_lan_proto == MachineObject::LVL_Rtsps)
            url = "qidi:///rtsps___" + m_lan_user + ":" + m_lan_passwd + "@" + m_lan_ip + "/streaming/live/1?proto=rtsps";
            ...
            if (url.find("agora") != std::string::npos) {
                ...
                if (wxGetApp().app_config->get("not_show_agora_tips") != "1") {
                    MessageDialog msg_dlg(this->GetParent(),
                                        _L("The live streaming feature relies on QIDI Studio to run,and the stream will end some time after the app is closed."),
...
void MediaPlayCtrl::RequestFileSystemUrl(std::function<void(std::string url)> cb, bool lan_mode)
{
    ...
    if (lan_mode && !m_lan_ip.empty() && !m_lan_passwd.empty()) {
        std::string url = "qidi:///local/" + m_lan_ip + "?port=6000&user=" + m_lan_user + "&passwd=" + m_lan_passwd;
        cb(url);
        return;
    }
...
void MediaPlayCtrl::SetStatus(wxString const &msg2, bool hyperlink)
{
    ...
    if (m_failed_code != 0) {
        int state2 = m_last_state >= MEDIASTATE_IDLE ? m_last_state - MEDIASTATE_IDLE :
                                                       m_last_state + MEDIASTATE_IDLE - MEDIASTATE_IDLE;
...
bool MediaPlayCtrl::start_stream_service(bool *need_install)
{
    ...
    wxString file_url2 = L"qidi:///camera/" + from_u8(file_url);
    file_url2.Replace("\\", "/");
...
{
#ifdef __WIN32__
    HANDLE shm = ::OpenFileMapping(FILE_MAP_READ, FALSE, L"qidi_stream_url");
