...
void MultiTaskItem::onPause()
{
    if (get_obj() && !get_obj()->can_resume()) {
        BOOST_LOG_TRIVIAL(info) << "MultiTask: pause current print task dev_id =" << QDTCrossTalk::Crosstalk_DevId(get_obj()->dev_id);
...
{
    if (get_obj() && get_obj()->can_resume()) {
        BOOST_LOG_TRIVIAL(info) << "MultiTask: resume current print task dev_id =" << QDTCrossTalk::Crosstalk_DevId(get_obj()->dev_id);
...
{
    if (get_obj()) {
        BOOST_LOG_TRIVIAL(info) << "MultiTask: abort current print task dev_id =" << QDTCrossTalk::Crosstalk_DevId(get_obj()->dev_id);
