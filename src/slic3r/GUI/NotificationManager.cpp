	...
	, m_notification_start  (GLCanvas3D::timestamp_now())
{
    ...
    m_NormalColor = ImVec4(0.3, 0.18, 0.91, 1);
...
void NotificationManager::PopNotification::use_qdt_theme()
{
 ...
 //   OldStyle.Colors[ImGuiCol_Text]     = m_TextColor;
	...
	m_HyperTextColor = m_is_dark ? ImVec4(0.3, 0.18, 0.91, 1) : ImVec4(0.3, 0.18, 0.91, 1);
...
void NotificationManager::PopNotification::render(GLCanvas3D& canvas, float initial_y, bool move_from_overlay, float overlay_width, float right_margin)
{
	...
	if (imgui.begin(name, window_flags)) {
		...
		qdt_render_left_sign(imgui, win_size.x, win_size.y, win_pos.x, win_pos.y);
...
void NotificationManager::PopNotification::qdt_render_block_notification(GLCanvas3D &canvas, float initial_y, bool move_from_overlay, float overlay_width, float right_margin)
{
	if (m_state == EState::Unknown)
		init();
	...
	if (imgui.begin(name, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse)) {
		...
		//render left icon
		qdt_render_block_notif_left_sign(imgui, win_size.x, win_size.y, win_pos.x, win_pos.y);
		qdt_render_block_notif_text(imgui, win_size.x, win_size.y, win_pos.x, win_pos.y);
		qdt_render_block_notif_buttons(imgui, win_size, win_pos);
		//render_close_button(imgui, win_size.x, win_size.y, win_pos.x, win_pos.y);
		//m_minimize_b_visible = false;
		//if (m_multiline && m_lines_count > 3)
		//	render_minimize_button(imgui, win_pos.x, win_pos.y);
...
void NotificationManager::PopNotification::qdt_render_block_notif_text(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
...
void NotificationManager::PopNotification::qdt_render_block_notif_buttons(ImGuiWrapper& imgui, ImVec2 win_size, ImVec2 win_pos)
{
	// close button
    ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(.0f, .0f, .0f, .0f));
	ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(.0f, .0f, .0f, .0f));
...
//{
//	if (m_data.type == NotificationType::QDTObjectInfo)
//	{
//		set_Multiline(true);
//	}
//}

void NotificationManager::PopNotification::qdt_render_block_notif_left_sign(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
...
{
	push_notification_data({ NotificationType::QDTGeneralError, NotificationLevel::ErrorNotificationLevel, 0,  _u8L("Error:") + "\n" + text }, 0);
}

void NotificationManager::close_general_error_notification(const std::string& text)
{
	for (std::unique_ptr<PopNotification> &notification : m_pop_notifications) {
		if (notification->get_type() == NotificationType::QDTGeneralError && notification->compare_text(_u8L("Error:") + "\n" + text)) {
			notification->close();
		}
	}
}
...
{
    NotificationData data{ NotificationType::AssemblyInfo, NotificationLevel::PrintInfoNotificationLevel, QDT_NOTICE_MAX_INTERVAL, text, "", nullptr};
	...
	for (const auto& notification : m_pop_notifications) {
        if (m_canvas_type == GLCanvas3D::ECanvasType::CanvasAssembleView) {
            if (notification->get_type() != NotificationType::AssemblyInfo
                ...
                && notification->get_type() != NotificationType::QDTIsolatedVolumeInfo
                && notification->get_type() != NotificationType::QDTAssemblyFarFromOrigin
                && notification->get_type() != NotificationType::QDTIntersectsVolumeInfo) {
                continue;
            }
        }
        else {
        ...
        if (notification->get_data().level == NotificationLevel::ErrorNotificationLevel || notification->get_data().level == NotificationLevel::SeriousWarningNotificationLevel) {
            notification->qdt_render_block_notification(canvas, bottom_up_last_y, m_move_from_overlay && (m_canvas_type == GLCanvas3D::ECanvasType::CanvasView3D), overlay_width * m_scale, right_margin * m_scale);
...
void NotificationManager::set_canvas_type(GLCanvas3D::ECanvasType t_canvas_type)
{
    ...
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications) {
        ...
        if (notification->get_type() == NotificationType::QDTPlateInfo)
            notification->hide(m_canvas_type != GLCanvas3D::ECanvasType::CanvasView3D);
        if (notification->get_type() == NotificationType::SignDetected)
            notification->hide(m_canvas_type == GLCanvas3D::ECanvasType::CanvasView3D);
        if (notification->get_type() == NotificationType::QDTObjectInfo)
            notification->hide(m_canvas_type != GLCanvas3D::ECanvasType::CanvasView3D);
        if (notification->get_type() == NotificationType::QDTSeqPrintInfo)
            notification->hide(m_canvas_type != GLCanvas3D::ECanvasType::CanvasView3D);
...
void NotificationManager::qdt_show_plateinfo_notification(const std::string &text)
{
    NotificationData data{NotificationType::QDTPlateInfo, NotificationLevel::PrintInfoNotificationLevel, QDT_NOTICE_MAX_INTERVAL, text};
...
void NotificationManager::qdt_close_3mf_warn_notification()
{
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications)
        if (notification->get_type() == NotificationType::QDT3MFInfo) {
            notification->close();
        }
}

void NotificationManager::qdt_show_3mf_warn_notification(const std::string &text)
{
    NotificationData data{NotificationType::QDT3MFInfo, NotificationLevel::ErrorNotificationLevel, QDT_NOTICE_MAX_INTERVAL, text};
...
void NotificationManager::qdt_close_plateinfo_notification()
{
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications)
        if (notification->get_type() == NotificationType::QDTPlateInfo) {
            notification->close();
...
void NotificationManager::qdt_show_preview_only_notification(const std::string &text)
{
    NotificationData data{NotificationType::QDTPreviewOnlyMode, NotificationLevel::WarningNotificationLevel, 0, text};

    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications) {
        if (notification->get_type() == NotificationType::QDTPreviewOnlyMode) {
            notification->reinit();
            notification->update(data);
            return;
        }
...
void NotificationManager::qdt_close_preview_only_notification()
{
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications)
        if (notification->get_type() == NotificationType::QDTPreviewOnlyMode) {
            notification->close();
...
void NotificationManager::qdt_close_objectsinfo_notification()
{
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications)
        if (notification->get_type() == NotificationType::QDTObjectInfo) { notification->close(); }
}

void NotificationManager::qdt_show_seqprintinfo_notification(const std::string &text, const std::string &link_text, std::function<bool(wxEvtHandler*)> callback, const std::string &second_link_text, std::function<bool(wxEvtHandler*)> second_callback,bool has_error)
{
    NotificationData data{NotificationType::QDTSeqPrintInfo,
                          has_error ? NotificationLevel::WarningNotificationLevel : NotificationLevel::PrintInfoNotificationLevel,
                          QDT_NOTICE_MAX_INTERVAL,
                          text,
                          link_text,
...
void NotificationManager::qdt_close_seqprintinfo_notification()
{
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications)
        if (notification->get_type() == NotificationType::QDTSeqPrintInfo) { notification->close(); }
}

void NotificationManager::qdt_show_plugin_install_notification(const std::string &text)
{
    ...
    NotificationData data{NotificationType::QDTPluginInstallHint, NotificationLevel::WarningNotificationLevel, 0, text, hyper_text, callback};

    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications) {
        if (notification->get_type() == NotificationType::QDTPluginInstallHint) {
            notification->reinit();
            notification->update(data);
            return;
        }
...
void NotificationManager::qdt_close_plugin_install_notification()
{
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications)
        if (notification->get_type() == NotificationType::QDTPluginInstallHint) { notification->close(); }
}

void NotificationManager::qdt_show_slice_emptylayer_notification(const std::string &text, bool bOverride)
{
    NotificationData data{NotificationType::QDTSliceEmptyLayer, NotificationLevel::WarningNotificationLevel, 0, _u8L("WARNING:") + "\n" + text};

    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications) {
        if (notification->get_type() == NotificationType::QDTSliceEmptyLayer) {
            if (bOverride)
                notification->update(data);
            else {
                NotificationData sdata{NotificationType::QDTSliceEmptyLayer, NotificationLevel::WarningNotificationLevel, 0, notification->get_text1() + "\n" + text};
...
void NotificationManager::qdt_show_app_newversion_notification()
{
	push_notification(NotificationType::NewAppAvailable);
}

void NotificationManager::qdt_show_need_support_on_notification()
{
    NotificationData data{NotificationType::QDTNeedSupportON, NotificationLevel::WarningNotificationLevel, 0,_u8L("Warning:") + "\n" + _u8L("Your model needs support ! Please make support material enable.")};

    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications) {
        if (notification->get_type() == NotificationType::QDTNeedSupportON) {
            notification->reinit();
            notification->update(data);
            return;
        }
...
void NotificationManager::qdt_close_need_support_on_notification()
{
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications)
        if (notification->get_type() == NotificationType::QDTNeedSupportON) { notification->close(); }
}


void NotificationManager::qdt_show_gcode_overlap_notification()
{
    NotificationData data{NotificationType::QDTGcodeOverlap, NotificationLevel::WarningNotificationLevel, 0,
                          _u8L("Warning:") + "\n" + _u8L("Gcode path overlap")};

    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications) {
        if (notification->get_type() == NotificationType::QDTGcodeOverlap) {
            notification->reinit();
            notification->update(data);
            return;
        }
...
void NotificationManager::qdt_close_gcode_overlap_notification()
{
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications)
...
void NotificationManager::qdt_show_filament_map_invalid_notification_before_slice(const NotificationType type,const std::string& text)
{
    auto callback = [](wxEvtHandler*) {
...
void NotificationManager::qdt_close_filament_map_invalid_notification_before_slice(const NotificationType type)
{
    close_notification_of_type(type);
}

void NotificationManager::qdt_show_filament_map_invalid_notification_after_slice(const NotificationType type, const std::string& text)
{
    auto callback = [](wxEvtHandler*) {
        auto plater = wxGetApp().plater();
...
void NotificationManager::qdt_close_filament_map_invalid_notification_after_slice(const NotificationType type)
{
    close_notification_of_type(type);
}
...
void NotificationManager::qdt_show_sole_text_notification(NotificationType sType, const std::string &text, bool bOverride, int level, bool autohide) {
    ...
    default:
		...
		if (autohide == false) nHideTime = QDT_NOTICE_MAX_INTERVAL;
        break;
    }
...
void NotificationManager::qdt_chose_sole_text_notification(NotificationType sType)
{
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications)
