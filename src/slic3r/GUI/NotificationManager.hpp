...
#define QDT_NOTICE_PLATEINFO_OBJID      1024+1
#define QDT_NOTICE_OBJECTS_OBJID        1024+2

#define QDT_NOTICE_MAX_INTERVAL         86400 * 10
...
//	LoadingFailed,
	...
	ExportOngoing,
	// QDS: Short meesage to fill space between start and finish of arranging
	ArrangeOngoing,
	// QDT: Plate Info ,Design For @YangLeDuo
	QDTPlateInfo,
	// QDT: 3MF warnings
	QDT3MFInfo,
	// QDT: Some Objects Info, Design For @YangLeDuo
	QDTObjectInfo,
	// QDT: Objects have empty layer when Slicing
	QDTSliceEmptyLayer,
	// QDT: model need support
	QDTNeedSupportON,
	// QDT: Gcode overlap
	QDTGcodeOverlap,
	//QDT: sequence print info
	QDTSeqPrintInfo,
	...
	QDTMixUsePLAAndPETG,
	QDTMixedFilamentBroken,
    QDTMultiFilaNoWipeTower,
	QDTNozzleFilamentIncompatible,
	QDTTpuNozzleHasMultiFilament,
    QDTHighTempNeedWrappingDetection,
	QDTPrintedWeightOverLimitWarn,
    QDTHighShrinkageFilament,
    QDTSingleExtruderMixedFilamentRisk,
    AssemblyWarning,
    AssemblyInfo,
    QDTIsolatedVolumeInfo,
    QDTAssemblyFarFromOrigin,
    QDTIntersectsVolumeInfo,
	QDTArcFittingInfo,
    QDTCalibExtruderMismatch,
...
//	void set_slicing_warning_gray(const std::string& text, bool g);
	...
	//QDS--plateinfo
    void qdt_show_plateinfo_notification(const std::string &text);
    void qdt_close_plateinfo_notification();

    //QDS-- 3mf warning
    void qdt_show_3mf_warn_notification(const std::string &text);
    void qdt_close_3mf_warn_notification();

    //QDS--preview only mode
    void qdt_show_preview_only_notification(const std::string &text);
    void qdt_close_preview_only_notification();

    //QDS--PluginInstallHint
    void qdt_show_plugin_install_notification(const std::string &text);
    ...
    void qdt_show_seqprintinfo_notification(const std::string &text, const std::string &link_text = "", std::function<bool(wxEvtHandler*)> callback = nullptr, const std::string &second_link_text = "", std::function<bool(wxEvtHandler*)> second_callback = nullptr,bool has_error = false);
    void qdt_close_seqprintinfo_notification();

	//QDS--EmptyLayer
	void qdt_show_slice_emptylayer_notification(const std::string &text, bool bOverride = true);

	//QDS--App has NewVersion
	void qdt_show_app_newversion_notification();

	//QDS--model need support on
	void qdt_show_need_support_on_notification();
    void qdt_close_need_support_on_notification();
...
private:
	...
	public:
        ...
        virtual void qdt_render_block_notification(GLCanvas3D &canvas, float initial_y, bool move_from_overlay, float overlay_width, float right_margin);
	...
	protected:
		...
		virtual void qdt_render_block_notif_text(ImGuiWrapper& imgui,
			const float win_size_x, const float win_size_y,
			const float win_pos_x, const float win_pos_y);
		virtual void qdt_render_block_notif_buttons(ImGuiWrapper& imgui,
			ImVec2 win_size,
			ImVec2 win_pos);
        virtual void qdt_render_block_notif_left_sign(ImGuiWrapper &imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y);
		// Left sign could be error or warning sign
        virtual void qdt_render_left_sign(ImGuiWrapper &imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y);
	...
	const std::vector<NotificationData> basic_notifications = {
        ...
        NotificationData{NotificationType::PresetUpdateAvailable, NotificationLevel::ImportantNotificationLevel, QDT_NOTICE_MAX_INTERVAL, _u8L("Configuration can update now."), _u8L("Detail."),
        ...
        NotificationData{NotificationType::QDTPluginUpdateAvailable, NotificationLevel::ImportantNotificationLevel, QDT_NOTICE_MAX_INTERVAL,
			_u8L("New network plug-in available."),
			_u8L("Details"),
                         [](wxEvtHandler* evnthndlr) {
                //QDS set feishu release page by default
                 wxCommandEvent* evt = new wxCommandEvent(EVT_UPDATE_PLUGINS_WHEN_LAUNCH);
				 wxQueueEvent(wxGetApp().plater(), evt);
				 return true;
             }},

        NotificationData{NotificationType::QDTPrinterConfigUpdateAvailable, NotificationLevel::ImportantNotificationLevel, QDT_NOTICE_MAX_INTERVAL,
        ...
        NotificationData{NotificationType::QDTUserPresetExceedLimit, NotificationLevel::WarningNotificationLevel, QDT_NOTICE_MAX_INTERVAL,
