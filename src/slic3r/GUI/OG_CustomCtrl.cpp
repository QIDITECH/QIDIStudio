...
#  endif
    ...
    for (CtrlLine& ctrl_line : ctrl_lines) {
        if (&ctrl_line.og_line == &line)
        {
            ...
            if (opt_group->option_label_at_right) // QDS: position buttons at right
            ...
            for (auto opt : option_set) {
                ...
                if (!opt_group->option_label_at_right) { // QDS: position option label at right
                ...
                if (opt_group->split_multi_line) {// QDS
...
void OG_CustomCtrl::CtrlLine::render(wxDC& dc, wxCoord h_pos, wxCoord v_pos)
{
    ...
    wxColour blink_color = StateColor::darkModeColorFor("#4479FB");
    ...
    for (const Option& opt : option_set) {
        ...
        if (ctrl->opt_group->option_label_at_right) // QDS
            add_field_width(field);
        // add label if any
        if (is_multioption_line && !option.label.empty()) {
            //!            To correct translation by context have to use wxGETTEXT_IN_CONTEXT macro from wxWidget 3.1.1
...
wxCoord OG_CustomCtrl::CtrlLine::draw_text(wxDC &dc, wxPoint pos, const wxString &text, const wxColour *color, int width, bool is_url/* = false*/, bool is_main/* = false*/)
{
    ...
    if (!text.IsEmpty()) {
        ...
        if (ctrl->opt_group->split_multi_line && !is_main) { // QDS
            const std::vector<Option> &option_set = og_line.get_options();
