...
#define QDS_GL_EXTENSION_FUNC(_func) (OpenGLManager::get_framebuffers_type() == OpenGLManager::EFramebufferType::Ext ? _func ## EXT : _func)
#define QDS_GL_EXTENSION_PARAMETER(_param) OpenGLManager::get_framebuffers_type() == OpenGLManager::EFramebufferType::Ext ? _param ## _EXT : _param
...
void OpenGLManager::_bind_frame_buffer(const std::string& name, EMSAAType msaa_type, uint32_t t_width, uint32_t t_height)
{
    if (OpenGLManager::s_back_frame == name) {
        ...
        glsafe(QDS_GL_EXTENSION_FUNC(::glBindFramebuffer)(QDS_GL_EXTENSION_PARAMETER(GL_FRAMEBUFFER), 0));
        return;
    }
...
void OpenGLManager::blit_framebuffer(const std::string& source, const std::string& target)
{
    ...
    if (s_back_frame == source) {
        glsafe(QDS_GL_EXTENSION_FUNC(::glBindFramebuffer)(QDS_GL_EXTENSION_PARAMETER(GL_READ_FRAMEBUFFER), 0));
    }
    else
    {
        ...
        glsafe(QDS_GL_EXTENSION_FUNC(::glBindFramebuffer)(QDS_GL_EXTENSION_PARAMETER(GL_READ_FRAMEBUFFER), source_id));
    }

    if (s_back_frame == target) {
        glsafe(QDS_GL_EXTENSION_FUNC(::glBindFramebuffer)(QDS_GL_EXTENSION_PARAMETER(GL_DRAW_FRAMEBUFFER), 0));
    }
    else
    {
        ...
        glsafe(QDS_GL_EXTENSION_FUNC(::glBindFramebuffer)(QDS_GL_EXTENSION_PARAMETER(GL_DRAW_FRAMEBUFFER), target_id));
    }
...
wxGLCanvas* OpenGLManager::create_wxglcanvas(wxWindow& parent, EMSAAType msaa_type)
{
    ...
    int attribList[] = {
        ...
        WX_GL_MIN_ALPHA, 		8,
        WX_GL_DEPTH_SIZE, 		24,
        //QDS: turn on stencil buffer for outline
...
FrameBuffer::~FrameBuffer()
{
    ...
    {
        glsafe(QDS_GL_EXTENSION_FUNC(::glDeleteFramebuffers)(1, &m_gl_id));
        m_gl_id = UINT32_MAX;
    }
    ...
    {
        glsafe(QDS_GL_EXTENSION_FUNC(::glDeleteRenderbuffers)(1, &m_depth_rbo_id));
        m_depth_rbo_id = UINT32_MAX;
    }

    if (UINT32_MAX != m_gl_id_for_back_fbo)
    {
        glsafe(QDS_GL_EXTENSION_FUNC(::glDeleteFramebuffers)(1, &m_gl_id_for_back_fbo));
        m_gl_id_for_back_fbo = UINT32_MAX;

        glsafe(QDS_GL_EXTENSION_FUNC(::glDeleteRenderbuffers)(2, m_msaa_back_buffer_rbos));
...
void FrameBuffer::bind()
{
    ...
    glsafe(QDS_GL_EXTENSION_FUNC(::glBindFramebuffer)(QDS_GL_EXTENSION_PARAMETER(GL_FRAMEBUFFER), (UINT32_MAX == m_gl_id_for_back_fbo ? m_gl_id : m_gl_id_for_back_fbo)));
}
...
void FrameBuffer::read_pixel(uint32_t x, uint32_t y, uint32_t width, uint32_t height, EPixelFormat format, EPixelDataType type, void* pixels)
{
    ...
    glsafe(QDS_GL_EXTENSION_FUNC(::glBindFramebuffer)(QDS_GL_EXTENSION_PARAMETER(GL_FRAMEBUFFER), m_gl_id));
    glsafe(::glReadPixels(x, y, width, height, gl_format, gl_type, pixels));
    glsafe(QDS_GL_EXTENSION_FUNC(::glBindFramebuffer)(QDS_GL_EXTENSION_PARAMETER(GL_FRAMEBUFFER), 0));
}
...
void FrameBuffer::create_no_msaa_fbo(bool with_depth)
{
    ...
    glsafe(QDS_GL_EXTENSION_FUNC(::glGenFramebuffers)(1, &m_gl_id));
    ...
    glsafe(QDS_GL_EXTENSION_FUNC(::glFramebufferTexture2D)(QDS_GL_EXTENSION_PARAMETER(GL_FRAMEBUFFER), QDS_GL_EXTENSION_PARAMETER(GL_COLOR_ATTACHMENT0), GL_TEXTURE_2D, m_color_texture_id, 0));

    if (with_depth)
    {
        glsafe(QDS_GL_EXTENSION_FUNC(::glGenRenderbuffers)(1, &m_depth_rbo_id));
        glsafe(QDS_GL_EXTENSION_FUNC(::glBindRenderbuffer)(QDS_GL_EXTENSION_PARAMETER(GL_RENDERBUFFER), m_depth_rbo_id));

        glsafe(QDS_GL_EXTENSION_FUNC(::glRenderbufferStorage)(QDS_GL_EXTENSION_PARAMETER(GL_RENDERBUFFER), GL_DEPTH24_STENCIL8, m_width, m_height));
...
{
    glsafe(QDS_GL_EXTENSION_FUNC(::glGenFramebuffers)(1, &m_gl_id_for_back_fbo));

    glsafe(QDS_GL_EXTENSION_FUNC(::glBindFramebuffer)(QDS_GL_EXTENSION_PARAMETER(GL_FRAMEBUFFER), m_gl_id_for_back_fbo));

    // use renderbuffer instead of texture to avoid the need to use glTexImage2DMultisample which is available only since OpenGL 3.2
    glsafe(QDS_GL_EXTENSION_FUNC(::glGenRenderbuffers)(2, m_msaa_back_buffer_rbos));

    glsafe(QDS_GL_EXTENSION_FUNC(::glBindRenderbuffer)(QDS_GL_EXTENSION_PARAMETER(GL_RENDERBUFFER), m_msaa_back_buffer_rbos[0]));
    glsafe(QDS_GL_EXTENSION_FUNC(::glRenderbufferStorageMultisample)(QDS_GL_EXTENSION_PARAMETER(GL_RENDERBUFFER), m_msaa, GL_RGBA8, m_width, m_height));

    glsafe(QDS_GL_EXTENSION_FUNC(::glBindRenderbuffer)(QDS_GL_EXTENSION_PARAMETER(GL_RENDERBUFFER), m_msaa_back_buffer_rbos[1]));
    glsafe(::glRenderbufferStorageMultisample(QDS_GL_EXTENSION_PARAMETER(GL_RENDERBUFFER), m_msaa, GL_DEPTH24_STENCIL8, m_width, m_height));

    glsafe(QDS_GL_EXTENSION_FUNC(::glFramebufferRenderbuffer)(QDS_GL_EXTENSION_PARAMETER(GL_FRAMEBUFFER), QDS_GL_EXTENSION_PARAMETER(GL_COLOR_ATTACHMENT0), QDS_GL_EXTENSION_PARAMETER(GL_RENDERBUFFER), m_msaa_back_buffer_rbos[0]));
    ...
    if (gl_info.is_version_greater_or_equal_to(3, 0)) {
        ...
        glsafe(QDS_GL_EXTENSION_FUNC(::glFramebufferRenderbuffer)(QDS_GL_EXTENSION_PARAMETER(GL_FRAMEBUFFER), QDS_GL_EXTENSION_PARAMETER(GL_DEPTH_ATTACHMENT), QDS_GL_EXTENSION_PARAMETER(GL_RENDERBUFFER), m_msaa_back_buffer_rbos[1]));
        glsafe(QDS_GL_EXTENSION_FUNC(::glFramebufferRenderbuffer)(QDS_GL_EXTENSION_PARAMETER(GL_FRAMEBUFFER), QDS_GL_EXTENSION_PARAMETER(GL_STENCIL_ATTACHMENT), QDS_GL_EXTENSION_PARAMETER(GL_RENDERBUFFER), m_msaa_back_buffer_rbos[1]));
    }
}
...
bool FrameBuffer::check_frame_buffer_status() const
{
    ...
    if (OpenGLManager::EFramebufferType::Ext == framebuffer_type) {
        if (::glCheckFramebufferStatusEXT(QDS_GL_EXTENSION_PARAMETER(GL_FRAMEBUFFER)) != GL_FRAMEBUFFER_COMPLETE_EXT)
        {
...
void FrameBuffer::resolve()
{
    ...
    glsafe(QDS_GL_EXTENSION_FUNC(::glBindFramebuffer)(QDS_GL_EXTENSION_PARAMETER(GL_READ_FRAMEBUFFER), m_gl_id_for_back_fbo));
    glsafe(QDS_GL_EXTENSION_FUNC(::glBindFramebuffer)(QDS_GL_EXTENSION_PARAMETER(GL_DRAW_FRAMEBUFFER), m_gl_id));
