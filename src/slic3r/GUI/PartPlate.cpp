...
std::array<unsigned char, 4>  PlateTextureForeground = {0x44, 0x79, 0xfb, 0xff};	// y96 change plateNum color
...
void PartPlate::render_logo(bool bottom, bool render_cali)
{
	...
	if (m_partplate_list->m_logo_texture_filename.empty()) {
        ...
        if (calc_bed_3d_boundingbox(box_in_plate_origin)) {
            if ((m_cur_bed_boundingbox.center() - box_in_plate_origin.center()).norm() > 0.5f) {
    ...
    if (!wxGetApp().plater()->is_printer_configed_by_QDT()) { // for Third party printer
        return;
    }
...
void PartPlate::generate_logo_polygon(ExPolygon &logo_polygon)
{
    ...
    if (cur_shape.size() == 4) { // rectangle case
		for (int i = 0; i < 4; i++){
			...
			if ((i  == 0) || (i  == 1)) {
				logo_polygon.contour.append({ scale_(p(0)), scale_(p(1) - 14.f)});
			}
			else {
...
void PartPlate::update_slice_context(BackgroundSlicingProcess & process)
{
	auto statuscb = [this](const Slic3r::PrintBase::SlicingStatus& status) {
		...
		//QDS: GUI refactor: add plate info befor message
...
// QDS: delay calc gcode path in backup dir
...
int PartPlate::load_gcode_from_file(const std::string& filename)
{
	...
	//QDS: need to apply two times, for after the first apply, the m_print got its object,
	//which will affect the config when new_full_config.normalize_fdm(used_filaments);
	m_print->apply(*m_model, full_config, false);

	// QDS: use backup path to save temp gcode
...
void PartPlateList::set_default_wipe_tower_pos_for_plate(int plate_idx, bool init_pos)
{
	...
	//y74 y75
	if( x < plate_bbox_x_max_local_coord * 0.6)
		x = plate_bbox_x_max_local_coord * 0.6;
...
int PartPlateList::select_plate(int index)
{
	...
	// QDS: erase unnecessary snapshot
	...
	for (unsigned int i = 0; i < (unsigned int)m_plate_list.size(); ++i)
	{
		...
		if (with_slice_info) {
			if (m_plate_list[i]->get_slice_result() && m_plate_list[i]->is_slice_result_valid()) {
				// QDS only include current palte_idx
...
{
    BedTextureInfo::TexturePart st_part1(10, 52, 8.393f, 192, "qdt_bed_st_left.svg");
    BedTextureInfo::TexturePart st_part2(74, -10, 148, 12, "qdt_bed_st_bottom.svg");
    BedTextureInfo::TexturePart pc_part1(10, 52, 8.393f, 192, "qdt_bed_pc_left.svg");
    BedTextureInfo::TexturePart pc_part2(74, -10, 148, 12, "qdt_bed_pc_bottom.svg");
    BedTextureInfo::TexturePart ep_part1(10, 52, 8.393f, 192, "qdt_bed_ep_left.svg");
    BedTextureInfo::TexturePart ep_part2(74, -10, 148, 12, "qdt_bed_ep_bottom.svg");
    BedTextureInfo::TexturePart pei_part1(10, 52, 8.393f, 192, "qdt_bed_pei_left.svg");
    ...
    if (use_double_extruder_texture) {
        ...
        if (middle_rect[2] > 0.f) {
            pei_part1 = BedTextureInfo::TexturePart(middle_rect[0], middle_rect[1], middle_rect[2], middle_rect[3], "qdt_bed_pei_middle.svg");
        }
        ...
        st_part1 = BedTextureInfo::TexturePart(57, 300, 236.12f, 10.f, "qdt_bed_st_middle.svg");
        if (middle_rect[2] > 0.f) {
            st_part1 = BedTextureInfo::TexturePart(middle_rect[0], middle_rect[1], middle_rect[2], middle_rect[3], "qdt_bed_st_middle.svg");
        }
        st_part2 = BedTextureInfo::TexturePart(45, -14.5, 260, 8, "qdt_bed_st_left_bottom.svg");
        if (bottom_texture_end_name.size() > 0 && bottom_rect[2] > 0.f) {
            std::string st_part2_name = "qdt_bed_st_bottom_" + bottom_texture_end_name + ".svg";
        ...
        ep_part1 = BedTextureInfo::TexturePart(57, 300, 236.12f, 10.f, "qdt_bed_ep_middle.svg");
        if (middle_rect[2] > 0.f) {
            ep_part1 = BedTextureInfo::TexturePart(middle_rect[0], middle_rect[1], middle_rect[2], middle_rect[3], "qdt_bed_ep_middle.svg");
        }
        ep_part2 = BedTextureInfo::TexturePart(45, -14.5, 260, 8, "qdt_bed_ep_left_bottom.svg");
        if (bottom_texture_end_name.size() > 0 && bottom_rect[2] > 0.f) {
            std::string ep_part2_name = "qdt_bed_ep_bottom_" + bottom_texture_end_name + ".svg";
        ...
        pc_part1 = BedTextureInfo::TexturePart(57, 300, 236.12f, 10.f, "qdt_bed_pc_middle.svg");
        if (middle_rect[2] > 0.f) {
            pc_part1 = BedTextureInfo::TexturePart(middle_rect[0], middle_rect[1], middle_rect[2], middle_rect[3], "qdt_bed_pc_middle.svg"); }
        pc_part2 = BedTextureInfo::TexturePart(45, -14.5, 70, 8, "qdt_bed_pc_left_bottom.svg");
        if (bottom_texture_end_name.size() > 0 && bottom_rect[2] > 0.f) {
            std::string pc_part2_name = "qdt_bed_pc_bottom_" + bottom_texture_end_name + ".svg";
        ...
        if (bottom_texture_end_name.size() > 0) {
            st_part2.update_file("qdt_bed_st_bottom_" + bottom_texture_end_name + ".svg");
            pc_part2.update_file("qdt_bed_pc_bottom_" + bottom_texture_end_name + ".svg");
            ep_part2.update_file("qdt_bed_ep_bottom_" + bottom_texture_end_name + ".svg");
            pei_part2.update_file("qdt_bed_pei_bottom_" + bottom_texture_end_name + ".svg");
            pte_part2.update_file("qdt_bed_pte_bottom_" + bottom_texture_end_name + ".svg");
        }
    }
...
{
	BedTextureInfo::TexturePart cali_line(18, 2, 224, 16, "qdt_cali_lines.svg");
