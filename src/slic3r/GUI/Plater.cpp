...
#define PRINTER_PANEL_SIZE (wxSize(FromDIP(96), FromDIP(96)))
...
void Sidebar::priv::layout_printer(bool is_support_box, bool isDual)
{
    ...
    vsizer_printer->GetItem(3)->Show(isQDT && !isDual);
}
...
public:
    ...
    {
        SetBackgroundColour(*wxWHITE);
        auto msg  = new wxStaticText(this, wxID_ANY, _L("Set the number of BOX installed on the nozzle."));
        ...
        auto txt4 = new wxStaticText(box, wxID_ANY, _L("BOX(4 slots)"));
        txt4->SetFont(Label::Body_14);
        txt4->SetBackgroundColour(0xF8F8F8);
        txt4->SetForegroundColour("#262E30");
        auto txt1 = new wxStaticText(box, wxID_ANY, _L("BOX(1 slot)"));
    ...
    : StaticGroup(parent, wxID_ANY)
{
    ...
    // BOX
    wxStaticText *label_ams  = new wxStaticText(this, wxID_ANY, _L("BOX"));
...
void ExtruderGroup::sync_ams(MachineObject const *obj, std::vector<DevAms *> const &ams4, std::vector<DevAms *> const &ams1)
{
    ...
    auto sync = [obj](std::vector<AMSinfo> &infos, std::vector<DevAms *> const &ams) -> bool {
        ...
        for (auto a : ams) {
            AMSinfo ams_info;
            //y71
            ams_info.parse_ams_info(const_cast<MachineObject*>(obj), a, false, obj->is_support_ams_humidity);
...
void Sidebar::update_sync_ams_btn_enable(wxUpdateUIEvent &e)
 {
     if (m_last_slice_state != p->plater->is_background_process_slicing()) {
         ...
         fila_sync_btn->Enable(!m_last_slice_state);
         Refresh();
     }
 }
    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(42 * wxGetApp().em_unit(), -1)), p(new priv(parent))
{
    ...
    // add printer
    {
        ...
        wiki_bed->Bind(wxEVT_BUTTON, [this](wxCommandEvent) {
            ...
            if (is_zh) {
                wxLaunchDefaultBrowser("https://wiki.qidi3d.com/zh/software/qidi-studio/print-settings/plate-type");
            } else {
    ...
    StateColor flush_bg_col(std::pair<wxColour, int>(wxColour(116, 168, 255), StateColor::Pressed),
    ...
    // y96
    StateColor flush_bd_col(std::pair<wxColour, int>(wxColour(68, 121, 251), StateColor::Pressed),
    ...
    checkbox->Bind(wxEVT_TOGGLEBUTTON, [this, checkbox](wxCommandEvent& evt) {
...
void Sidebar::update_all_preset_comboboxes()
{
    ...
    // y16
    if (true) {
        //only show connection button for not-QDT printer

        // 5
        // p->btn_connect_printer->Hide();

        //only show sync-ams button for QDT printer
        // p->m_bpButton_ams_filament->Show();
        //update print button default value for qdt or third-party printer
    ...
    if (cfg.opt_bool("support_multi_bed_types")) {
        ...
        p->combo_printer_bed->SetAble(false);
    }
    if (print_tech == ptFFF) {
...
void Sidebar::update_presets(Preset::Type preset_type)
{
    ...
    case Preset::TYPE_PRINTER:
    {
        ...
        //B y71
        wxGetApp().mainframe->show_calibration_button(false, true);
...
bool Sidebar::use_default_bed_type(bool is_qdt_preset)
{
...
void Sidebar::msw_rescale()
{
    ...
    // QDS TODO: add msw_rescale for newly added windows
    // QDS
...
void Sidebar::sys_color_changed()
{
    ...
    p->m_bpButton_del_filament->msw_rescale();
    //y59
    p->m_bpButton_box_filament->msw_rescale();
    p->m_bpButton_set_filament->msw_rescale();
    p->m_flushing_volume_btn->Rescale();
    p->m_purge_mode_btn->Rescale();
    ...
    //QDS: remove print related combos
#if 0
    for (PlaterPresetComboBox* combo : std::vector<PlaterPresetComboBox*>{  p->combo_print,
                                                                p->combo_sla_print,
                                                                p->combo_sla_material,
...
// QDS. Move logic from Plater::on_extruders_change() to Sidebar::on_filament_count_change().
...
void Sidebar::add_filament() {
    // QDS: limit filament choices to ExtruderMax
...
void Sidebar::get_small_btn_sync_pos_size(wxPoint &pt, wxSize &size) {
    size = fila_sync_btn->GetSize();
    pt   = fila_sync_btn->GetScreenPosition();
}
...
//w42
//w42
std::map<int, DynamicPrintConfig> Sidebar::build_filament_box_list(std::vector<std::string> id, std::vector<std::string> color, std::vector<int> slot_state, std::vector<int> slot_id, std::vector<std::string> type)
{
    std::map<int, DynamicPrintConfig> filament_ams_list;
    ...
    for (int i = 0; i < 16; i++) {
        ...
        tray_config.set_key_value("filament_type", new ConfigOptionStrings{type[i]}); //clear
        tray_config.set_key_value("slot_state", new ConfigOptionStrings{std::to_string(slot_state[i])});
        tray_config.set_key_value("slot_id", new ConfigOptionStrings{ std::to_string(slot_id[i]) });
        int group = i / 4 + 1;
        char suffix = 'A' + (i % 4); 
...
{
    box_filament_id = wxGetApp().plater()->box_msg.filament_id;
    box_filament_colors = wxGetApp().plater()->box_msg.filament_colors;
    box_slot_state = wxGetApp().plater()->box_msg.slot_state;
    box_slot_id = wxGetApp().plater()->box_msg.slot_id;
    box_list_preset_name = wxGetApp().plater()->box_msg.box_list_preset_name;
    filament_type = wxGetApp().plater()->box_msg.filament_type;
    std::map<int, DynamicPrintConfig> filament_ams_list = build_filament_box_list(box_filament_id, box_filament_colors, box_slot_state, box_slot_id, filament_type);
    std::string current_preset_name = wxGetApp().get_tab(Preset::TYPE_PRINTER)->get_presets()->get_edited_preset().name;
    if (wxGetApp().plater()->box_msg.box_list_preset_name != current_preset_name) {
        wxGetApp().plater()->box_msg.box_list_preset_name = current_preset_name;
    }
...
void Sidebar::set_flushing_volume_warning(const bool flushing_volume_modify)
{
    if (flushing_volume_modify){
        ...
        StateColor flush_bd_col(std::pair<wxColour, int>(wxColour(0, 66, 255), StateColor::Pressed),
...
void Sidebar::update_printer_thumbnail()
{
    ...
    //y61
    try{
        std::string machine_icon_path = Slic3r::resources_dir() + "/" + "profiles" + "/" + "thumbnail" + "/" + printer_type + "_thumbnail" + ".png"; 
        if(std::filesystem::exists(machine_icon_path)){
            p->image_printer->SetBitmap(create_scaled_bitmap(machine_icon_path, this, 48));
        }
        else {
            p->image_printer->SetBitmap(create_scaled_bitmap("printer_placeholder", this, 48));
        }
...
public:
    ...
    //QDS: add a flag to ignore cancel event
    ...
    // QDS
    std::shared_ptr<QDTStatusBar> statusbar();
    ...
    bool delete_object_from_model(size_t obj_idx, bool refresh_immediately = true); //QDS
    ...
    //y58
    void export_gcode(fs::path output_path, bool output_path_on_removable_media, bool has_notification = true);
    ...
    void on_action_send_multi_gcode(SimpleEvent&);  // y11
    ...
    //QDS: add plate_id for thumbnail
    ...
    //QDS store machine_sn and 3mf_path for PrintJob
    ...
    //QDS: add print project related logic
...
const std::regex Plater::priv::pattern_prusa(".*qdt", std::regex::icase);
    ...
    , partplate_list(this->q, &model)
{
    ...
    // QDS: to be checked. Not follow patch.
    background_process.set_thumbnail_cb([this](const ThumbnailsParams& params) { return this->generate_thumbnails(params, Camera::EType::Ortho); });
    background_process.set_slicing_completed_event(EVT_SLICING_COMPLETED);
    background_process.set_finished_event(EVT_PROCESS_COMPLETED);
    background_process.set_export_began_event(EVT_EXPORT_BEGAN);
    ...
    if (wxGetApp().is_editor()) {
        ...
        // QDS: should bind BACKGROUND_PROCESS event to plater
    ...
    if (wxGetApp().is_editor()) {
        ...
        view3D_canvas->Bind(EVT_GLCANVAS_ARRANGE, [this](SimpleEvent& evt) {
            //QDS arrage from EVT set default state.
        ...
        view3D_canvas->Bind(EVT_GLCANVAS_ORIENT, [this](SimpleEvent& evt) {
            //QDS oriant from EVT set default state.
            this->q->set_prepare_state(Job::PREPARE_STATE_DEFAULT);
            this->q->orient(); });
...
//        view3D_canvas->Bind(EVT_GLTOOLBAR_DELETE_ALL, [q](SimpleEvent&) { q->reset_with_confirm(); });
    ...
    if (wxGetApp().is_editor()) {
        ...
        q->Bind(EVT_GLTOOLBAR_SEND_MULTI_GCODE, &priv::on_action_send_multi_gcode, this);   // y11
    ...
    if (wxGetApp().is_editor()) {
        ...
        /* QDS do not handle removeable driver event */
...
void Plater::priv::select_view_3D(const std::string& name, bool no_slice)
{
    if (name == "3D") {
        ...
        BOOST_LOG_TRIVIAL(info) << "select preview";
        //QDS update extruder params and speed table before slicing
...
// QDS
std::shared_ptr<QDTStatusBar> Plater::priv::statusbar()
{
    return nullptr;
}
            ...
            if (type_3mf) {
                ...
                En3mfType          en_3mf_file_type = En3mfType::From_QDS;
                {
                    ...
                    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__
                                            << boost::format(", plate_data.size %1%, project_preset.size %2%, is_qds_3mf %3%, file_version %4% \n") % plate_data.size() %
                                                   project_presets.size() % (en_3mf_file_type == En3mfType::From_QDS) % file_version.to_string();

                    // 1. add extruder for prusa model if the number of existing extruders is not enough
                    // 2. add extruder for QDS or Other model if only import geometry
                ...
                // // else if (load_config && (file_version > app_version)) {
                    ...
                    //}
                        ...
                        load_config = false;
                        if (wxGetApp().app_config->get_bool("skip_non_qidi_3mf_warning") != true) {
                            TipsDialog dlg(q, _L("Load 3mf"),
                                           file_version.maj() == 0 && file_version.min() == 0 && file_version.patch() == 0
                                               ? _L("The 3mf is not from QIDI Tech, load geometry data only.")
                                               : _L("The 3mf is generated by old QIDI Studio, load geometry data only."),
                                           "skip_non_qidi_3mf_warning", wxOK);
                            dlg.ShowModal();
                        }
                    }
                    else if (!load_config) {
                    ...
                    if (plate_data.size() > 0) {
                        ...
                        if ((en_3mf_file_type == En3mfType::From_QDS) && (file_version < old_version) && load_model && load_config && !config_loaded.empty()) {
                            translate_old = true;
                    ...
                    if (load_config && !config_loaded.empty()) {
                        ...
                        if (!validity.empty()) {
                            ...
                            notify_manager->qdt_show_3mf_warn_notification(error_message);
                        }
                    }
...
//y75
                has_different_settings_to_system = config.option("different_settings_to_system") ? true : false;
//y75
                ...
                if (load_config) {
                    if (!config.empty()) {
                        ...
                        {
                            // QDS: modify the prime tower params for old version file
                            Semver old_version3(2, 0, 0);
                            if (en_3mf_file_type == En3mfType::From_QDS && file_version < old_version3) {
                        ...
                        if (choise.empty() || choise != "true") {
                            // QDS: first validate the printer
                        ...
                        //always load config
                        {
                            // QDS: save the wipe tower pos in file here, will be used later
                            ...
                            // QDS: do not change extruder nozzle stat when loading 3mf
                            ...
                            // QDS: moved this logic to presetcollection
                            //{
                            ...
                            // QDS: add preset combo box re-active logic
                            // currently found only needs re-active here
//y75
                            _3mf_type = en_3mf_file_type;
//y75
                            ...
                            // do some post process after loading config
                            {
                                ...
                                //QDS: rewrite wipe tower pos stored in 3mf file , the code above should be seriously reconsidered
        ...
        if (load_model) {
            ...
            // QDS: add load_old_project logic
            if ((!is_project_file) && (!load_old_project)) {
                ...
                if (imperial_units)
                    ...
                    // QDS do not handle look like in meters
                    MessageDialog dlg(q,
                ...
                // else if (model.looks_like_imperial_units()) {
                // QDS do not handle look like in imperial
    ...
    //QDS: add gcode loading logic in the end
    ...
    if (!obj_idxs.empty() && view3D != nullptr) {
        ...
        // QDS: update object list selection
...
std::vector<size_t> Plater::priv::load_model_objects(const ModelObjectPtrs& model_objects, bool allow_negative_z, bool split_object)
{
    ...
    for (ModelObject *model_object : model_objects) {
        ...
        //QDS: when the object is too large, let the user choose whether to scale it down
        ...
        if (!split_object) {
            ...
            for (ModelObject* model_object : model.objects) {
                //QDS initialize assemble transformation
    ...
    // QDS: get wipe tower of current plate
...
#else
    // QDS: find an empty cell to put the copied object
    ...
    //QDS: remove the auto scaled_down logic when load models
    //if (scaled_down) {
    //    GUI::show_info(q,
...
fs::path Plater::priv::get_export_file_path(GUI::FileType file_type)
{
    ...
    //qds  name the project using the part name
...
void Plater::priv::remove(size_t obj_idx)
{
    ...
    //QDS: notify partplate the instance removed
...
void Plater::priv::reset(bool apply_presets_change)
{
    ...
    reset_gcode_toolpaths();
    //QDS: update gcode to current partplate's
    ...
    //QDS: clear the partplate list's object before object cleared
    ...
    //QDS: reset all project embedded presets
...
void Plater::priv::split_object()
{
    ...
    else
    {
        ...
        //QDS: set split_object to true to avoid re-compute assemble matrix
...
{
    //QDS: use the plate's bounding box instead of the bed's
...
void Plater::priv::process_validation_warning(StringObjectException const &warning) const
{
    if (warning.string.empty())
        ...
        if (warning.hypetext == "filament_mix_print") {
            ...
            //     bool is_zh = wxGetApp().app_config->get("language") == "zh_CN";
            //     if (is_zh) {
            //         wxLaunchDefaultBrowser("https://wiki.qidi3d.com/zh/filament-acc/filament/h2d-filament-config-limit");
            //     } else {
            //         wxLaunchDefaultBrowser("https://wiki.qidi3d.com/en/filament-acc/filament/h2d-filament-config-limit");
            //     }
            //     return false;
            // };
            ;
...
unsigned int Plater::priv::update_background_process(bool force_validation, bool postpone_error_messages, bool switch_print)
{
    ...
    //QDS: add the switch print logic before Print::Apply
    ...
    if (switch_print)
    {
        // Update the "out of print bed" state of ModelInstances.
        this->update_print_volume_state();
        //QDS: update the current print to the current plate
    ...
    if ((invalidated != Print::APPLY_STATUS_UNCHANGED || force_validation) && ! background_process.empty()) {
        ...
        //QDS: add is_warning logic
        StringObjectException warning;
        //QDS: refine seq-print logic
        Polygons polygons;
        ...
        if (err.string.empty()) {
            ...
            notification_manager->qdt_close_3mf_warn_notification();
...
//y58
void Plater::priv::export_gcode(fs::path output_path, bool output_path_on_removable_media, bool has_notification)
{
...
unsigned int Plater::priv::update_restart_background_process(bool force_update_scene, bool force_update_preview)
{
    ...
    //QDS: judge whether can switch print or not
...
void Plater::priv::update_fff_scene()
{
    ...
    //QDS: add assemble view related logic
            ...
            if (has_source || has_name) {
                ...
                if (!match_found && has_name) {
                    ...
                    for (size_t o = 0; o < new_model.objects.size(); ++o) {
                        ...
                        if (found) break;
                        // QDS: step model,object loaded as a volume. GUI_ObfectList.cpp load_modifier()
...
void Plater::priv::set_current_panel(wxPanel* panel, bool no_slice)
{
    ...
    //QDS: add slice logic when switch to preview page
    auto do_reslice = [this, no_slice]() {
            ...
            if (!no_slice && !this->model.objects.empty() && !export_in_progress && model_fits && current_has_print_instances)
            {
                ...
                //QDS: add more judge for slicing
            ...
            //QDS: process empty plate, reset previous toolpath
            else
            {
                //if (!this->m_slice_all)
    ...
    //QDS: wish to reset all plates stats item selected state when back to View3D Tab
    preview->get_canvas3d()->reset_select_plate_toolbar_selection();

    wxPanel* old_panel = current_panel;
//#if QDT_HAS_FIRST_PAGE
    if (!old_panel) {
        //QDS: only switch to the first panel when visible
        panel->Show();
    ...
    if (keys_map) {
        ...
        if (new_bed_type != btCount) {
            ...
            if (old_bed_type != new_bed_type) {
                ...
                // only update curr_bed_type to config when preset is qdt printers
                // bool is_qdt_preset = preset_bundle.printers.get_edited_preset().is_qdt_vendor_preset(&preset_bundle);
                bool is_qdt_preset = true;
                if (is_qdt_preset) {
                    // update app_config
                    AppConfig* app_config = wxGetApp().app_config;
...
void Plater::priv::on_select_preset(wxCommandEvent &evt)
{
    ...
    // QDS: log modify of filament selection
    Slic3r::put_other_changes();
...
void Plater::priv::on_slicing_update(SlicingStatusEvent &evt)
{
    ...
    if (evt.status.flags & (PrintBase::SlicingStatus::RELOAD_SCENE | PrintBase::SlicingStatus::RELOAD_SLA_SUPPORT_POINTS)) {
        ...
        case ptFFF:
            //QDS: add slice project logic, only display shells at the beginning
    ...
    if (evt.status.flags & (PrintBase::SlicingStatus::UPDATE_PRINT_STEP_WARNINGS | PrintBase::SlicingStatus::UPDATE_PRINT_OBJECT_STEP_WARNINGS)) {
        ...
        //QDS: add partplate related logic, use the print in background process
...
{
#if 0
    //QDS: also export 3mf to the same directory for debugging
    std::string gcode_path_str(evt.GetString().ToUTF8().data());
    fs::path gcode_path(gcode_path_str);

    if (q) {
        q->export_3mf(gcode_path.replace_extension(".3mf"), SaveStrategy::Silence); // QDS: silence
    }
#endif
}
...
void Plater::priv::on_process_completed(SlicingProcessCompletedEvent &evt)
{
    ...
    //QDS:ignore cancel event for some special case
    if (m_ignore_event)
    {
    ...
    //QDS: slice .gcode.3mf file related logic, assign is_finished again
    bool only_has_gcode_need_preview = false;
    ...
    //QDS: set the current plater's slice result to valid
    if (!this->background_process.empty())
        this->background_process.get_current_plate()->update_slice_result_valid_state(evt.success());

    //QDS: update the action button according to the current plate's status
    ...
    //QDS: remove this update here, will be updated in update_fff_scene later
    ...
    if (evt.cancelled()) {
        ...
        // QDS, Generate calibration thumbnail for current plate
        if (!has_error && preview) {
            // generate calibration data
            /* QDS generate calibration data by printer
            preview->reload_print();
    ...
    // QDS stop publishing if error occur
    //if (m_is_publishing) {
...
void Plater::priv::on_action_add(SimpleEvent&)
{
    if (q != nullptr) {
        //q->add_model();
        //QDS open file in toolbar add
        q->add_file();
    }
}

//QDS: add plate from toolbar
...
//QDS: GUI refactor: slice with helio
...
void Plater::priv::on_action_slice_all(SimpleEvent&)
{
    if (q != nullptr) {
        ...
        if (!m_is_publishing)
            q->select_view_3D("Preview");
        //QDS: wish to select all plates stats item
...
void Plater::priv::on_action_send_to_multi_machine(SimpleEvent&)
{
    ...
    //if (!m_send_multi_dlg)
    //y61
    SendMultiMachinePage*    dlg = new SendMultiMachinePage(q);
    //y52
    dlg->prepare(partplate_list.get_curr_plate_index());
    ...
    if (dlg->ShowModal() == wxID_YES) 
    {
        ...
        std::map<std::string, std::vector<std::string>> send_machine_info = dlg->get_selected_machine_info();
...
void Plater::priv::on_action_send_gcode(SimpleEvent&)
{
    if (q != nullptr) {
        BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << ":received export gcode event\n" ;
        // q->send_gcode_legacy();
        q->send_gcode_to_printer();
    }
}

// y11
void Plater::priv::on_action_send_multi_gcode(SimpleEvent&)
{
    if (q != nullptr) {
        BOOST_LOG_TRIVIAL(debug) << __FUNCTION__ << ":received export gcode event\n" ;
        // q->send_gcode_legacy();
        q->send_gcode_to_printers();
    }
}
...
void Plater::priv::on_action_export_to_sdcard(SimpleEvent&)
{
	...
	// }
    //y40
    SendToPrinterDialog* dlg = new SendToPrinterDialog(q);
    //y52
    dlg->prepare(partplate_list.get_curr_plate_index());
    dlg->ShowModal();
}
...
//QDS: add slice button status update logic
...
//QDS: repair model through netfabb
...
{
    notification_manager->qdt_show_plugin_install_notification(into_u8(_L("Network Plug-in is not detected. Network related features are unavailable.")));
}

void Plater::priv::show_preview_only_hint(wxCommandEvent &event)
{
    notification_manager->qdt_show_preview_only_notification(into_u8(_L("Preview only mode:\nThe loaded file contains gcode only, Can not enter the Prepare page")));
}
...
{
    // QDS: GUI refactor: move sidebar to the left
    int x, y;
...
//QDS: add plate id for thumbnail generate param
...
void Plater::priv::set_project_name(const wxString& project_name)
{
    ...
    //update topbar title
#ifdef __WINDOWS__
    wxGetApp().mainframe->SetTitle(m_project_name + " - QIDIStudio");
...
//QDS: add job state related functions
...
GLCanvas3D* Plater::priv::get_current_canvas3D(bool exclude_preview)
{
    ...
    else //QDS default set to view3D
        return view3D->get_canvas3d();
            ...
            struct SyncInfoDialog : MessageDialog
            {
                SyncInfoDialog(wxWindow *parent)
                    : MessageDialog(parent,
                                    _L("The nozzle type and BOX quantity information has not been synced from the connected printer.\n"
...
void Plater::priv::take_snapshot(const std::string& snapshot_name, const UndoRedo::SnapshotType snapshot_type)
{
    ...
    //QDS: add PartPlateList as the paremeter for take_snapshot
...
void Plater::priv::undo()
{
    ...
    // QDS: undo-redo until modify record
...
void Plater::priv::undo_redo_to(std::vector<UndoRedo::Snapshot>::const_iterator it_snapshot)
{
    ...
    if (it_snapshot->timestamp < this->undo_redo_stack().active_snapshot_time() ?
        ...
        if (this->printer_technology == ptFFF) {
            ...
            // QDS: don't support wipe tower rotation
...
void Plater::priv::bring_instance_forward() const
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "QIDI Studio window going forward";
...
//y58
int Plater::getUploadCount() { return p->UploadCount; }
void Plater::setUploadCount(int uploadcount) { p->UploadCount = uploadcount; }
int Plater::getInterval() { return p->m_sending_interval; }
void Plater::setInterval(int interval) { p->m_sending_interval = interval; }
...
// QDS: FIXME, missing resotre logic
    ...
    wxString const& originfile)
{
    ...
    // QDS set default 3D view and direction after loading project
    //p->select_view_3D("3D");
    if (!m_exported_file) {
        p->select_view("topfront");
...
void Plater::import_model_id(wxString download_info)
{
            ...
            http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                ...
                .on_error([&msg, &cont, &retry_count, max_retries](std::string body, std::string error, unsigned http_status) {
                    ...
                    if (retry_count == max_retries) {
                        msg = _L("Importing to QIDI Studio failed. Please download the file and manually import it.");
                        cont = false;
                    }
                })
    ...
    if (download_ok) {
        ...
        /*QDS set project info after load project, project info is reset in load project */
...
void Plater::_calib_pa_line(const Calib_Params& params)
{
    ...
    normal_paths.push_back(std::move(input_file));
    ...
    double EndPA = params.end;
    double StartPA = params.start;
    double PAStep = params.step;
    ...
    const double external_perimeter_acceleration = print_config->opt_float_nullable(("outer_wall_acceleration"), extruder_idx);
    double       pa_first_layer_height = print_config->option<ConfigOptionFloat>("initial_layer_print_height")->value;
    double       pa_layer_height = print_config->get_abs_value("layer_height");
    double       nozzle_diameter = printer_config->option<ConfigOptionFloatsNullable>("nozzle_diameter")->values[0];
    double       pa_line_width = print_config->get_abs_value("outer_wall_line_width") == 0 ?
        print_config->get_abs_value("line_width") == 0 ?
        nozzle_diameter * 1.125 :
        print_config->get_abs_value("line_width") :
        print_config->get_abs_value("outer_wall_line_width");
    double       pa_travel_speed = print_config->option<ConfigOptionFloatsNullable>("initial_layer_speed")->get_at(extruder_idx) * 60;
    double retract_length = (filament_config->option("filament_retraction_length")->is_nil()) ?
        double(printer_config->opt_float("retraction_length", 0)) :
        double(filament_config->opt_float("filament_retraction_length", 0));
    double retract_speed = (filament_config->option("filament_retraction_speed")->is_nil()) ?
        double(printer_config->opt_float("retraction_speed", 0)) * 60 :
        double(filament_config->opt_float("filament_retraction_speed", 0)) * 60;
    ...
    const double print_flow_ratio = double(filament_config->opt_float("filament_flow_ratio", 0));
    ...
    CalibPressureAdvancePattern pa_pattern(params, full_config, true, model(), plate_origin);
    gcode << pa_pattern.generate_custom_nums_gcodes(full_config, true, model(), plate_origin, StartPA + PAStep, plate_center.x() - 57.5, start_y +  step_spacing + step_spacing / 2).rdbuf();
    for (int i = 1; i < (count + 1) / 2; i++) {
        double num_double = StartPA + (1 + i * 2) * PAStep;
    ...
    auto pa_end_gcode = printer_config->opt_string("machine_end_gcode");
    gcode << pa_end_gcode;
    pa_end_gcode = gcode.str();

    new_config.set_key_value("wall_generator", new ConfigOptionEnum<PerimeterGeneratorType>(PerimeterGeneratorType::Classic));
    new_config.set_key_value("machine_end_gcode", new ConfigOptionString(pa_end_gcode));
    ...
    giz_obj_manip.on_change("size", 1, count * step_spacing + pa_line_width);
    giz_obj_manip.on_change("size", 2, pa_first_layer_height);
...
void Plater::_calib_pa_pattern(const Calib_Params &params)
{
    ...
    const DynamicPrintConfig    full_config    = wxGetApp().preset_bundle->full_config();
    PresetBundle *              preset_bundle  = wxGetApp().preset_bundle;
    const bool                  is_qdt_machine = preset_bundle->printers.get_edited_preset().has_lidar(preset_bundle);
    const Vec3d                 plate_origin   = get_partplate_list().get_current_plate_origin();
    CalibPressureAdvancePattern pa_pattern(params, full_config, is_qdt_machine, model(), plate_origin);
    ...
    pa_pattern.generate_custom_gcodes(full_config, is_qdt_machine, model(), plate_origin);
    ...
    */
    ...
    int extruder_idx = 0;
    double pa_travel_speed = print_config->option<ConfigOptionFloatsNullable>("travel_speed")->values[0] * 60;
    ...
    const double speed_perimeter = print_config->opt_float_nullable(("inner_wall_speed"), extruder_idx);
    const double speed_fast = print_config->opt_float_nullable(("outer_wall_speed"), extruder_idx) * 60;
    const double speed_first_layer = print_config->option<ConfigOptionFloatsNullable>("initial_layer_speed")->get_at(extruder_idx) * 60;
    ...
    double retract_speed = (filament_config->option("filament_retraction_speed")->is_nil()) ?
        double(printer_config->opt_float("retraction_speed", 0)) * 60 :
        double(filament_config->opt_float("filament_retraction_speed", 0)) * 60;
    double retract_lift = (filament_config->option("filament_z_hop")->is_nil()) ?
        double(printer_config->opt_float("z_hop", 0)) :
        double(filament_config->opt_float("filament_z_hop", 0));

    select_all();
    wxGetApp().plater()->get_camera().select_view("top");
    double pa_first_layer_height = print_config->get_abs_value("initial_layer_print_height");

    std::stringstream gcode;
    const DynamicPrintConfig    full_config = wxGetApp().preset_bundle->full_config();
    const Vec3d                 plate_origin = get_partplate_list().get_current_plate_origin();
    CalibPressureAdvancePattern pa_pattern(params, full_config, true, model(), plate_origin);
    gcode << pa_pattern.generate_custom_nums_gcodes(full_config, true, model(), plate_origin, StartPA + PAStep, plate_center.x() - 40, start_y - step_spacing - line_spacing_xy).rdbuf();
    for (int i = 1; i < (count + 1) / 2; i++) {
        double num_double = StartPA + (1 + i * 2) * PAStep;
        std::stringstream gcode2 = pa_pattern.generate_custom_nums_gcodes(full_config, true, model(), plate_origin, num_double, plate_center.x() - 40, start_y - (1 + i * 2) * step_spacing -  line_spacing_xy);
        gcode << gcode2.rdbuf();
    }
    ...
    const int max_fan_speed = round(filament_config->opt_int("fan_max_speed", 0) * 2.55);
    gcode << "\nM106 S" << max_fan_speed;

    const bool seal = wxGetApp().app_config->get_seal();
    if (seal)
    {
        const int auxiliary_fan = round(filament_config->opt_int("additional_cooling_fan_speed", 0) * 2.55);
        gcode << "\nM106 P2 S" << auxiliary_fan;
    }
    else
    {
        const int auxiliary_fan_unseal = round(filament_config->opt_int("additional_cooling_fan_speed_unseal", 0) * 2.55);
        gcode << "\nM106 P2 S" << auxiliary_fan_unseal;
    }

    const int volume_fan_speed = round(filament_config->opt_int("during_print_exhaust_fan_speed", 0) * 2.55);
    ...
    giz_obj_manip.set_uniform_scaling(false);
    giz_obj_manip.on_change("size", 0, 25);
    giz_obj_manip.on_change("size", 1, pa_wall_width + line_spacing);
    giz_obj_manip.on_change("size", 2, pa_first_layer_height);
    giz_obj_manip.set_uniform_scaling(true);
...
void Plater::_calib_pa_tower(const Calib_Params &params)
{
    ...
    DynamicPrintConfig  new_config;
    DynamicPrintConfig* printer_config = &wxGetApp().preset_bundle->printers.get_edited_preset().config;
    auto                max_print_height = build_volume().printable_height();
    auto                pa_end_gcode = printer_config->opt_string("before_layer_change_gcode");
    ...
    select_all();
    giz_obj_manip.set_uniform_scaling(false);
    giz_obj_manip.on_change("size", 2, (count + 1) * 5);
    giz_obj_manip.set_uniform_scaling(true);
    pa_end_gcode = "M900 K{int(layer_z / 5) * " + double_to_str(PAStep) + " + " + double_to_str(StartPA) + "}\n" + pa_end_gcode;
    new_config.set_key_value("before_layer_change_gcode", new ConfigOptionString(pa_end_gcode));
    tab_printer->load_config(new_config);
...
//w29
void Plater::calib_flowrate(int pass,double input_value)
{
   ...
   // else
    ...
    //w29
    //print_config->set_key_value("resonance_avoidance", new ConfigOptionBool(false));
    if(pass == 1)
        filament_config->set_key_value("filament_flow_ratio", new ConfigOptionFloatsNullable{ 1.0 });
    else
        filament_config->set_key_value("filament_flow_ratio", new ConfigOptionFloatsNullable{ input_value });
...
void Plater::calib_max_vol_speed(const Calib_Params &params)
{
    ...
    */
    ...
    wxGetApp().mainframe->select_tab(size_t(MainFrame::tp3DEditor));
    std::string input_file;
    input_file = Slic3r::resources_dir() + "/calib/volumetric_speed/SpeedTestStructure.stl";
    std::vector<fs::path> normal_paths;
    normal_paths.push_back(std::move(input_file));
    load_files(normal_paths, LoadStrategy::LoadModel);
    orient();
    ...
    print_config->set_key_value("spiral_mode", new ConfigOptionBool(true));
    obj_cfg.set_key_value("outer_wall_line_width", new ConfigOptionFloat(line_width));
    //print_config->set_key_value("initial_layer_print_height", new ConfigOptionFloat(layer_height));
    obj_cfg.set_key_value("layer_height", new ConfigOptionFloat(layer_height));
    print_config->set_key_value("resonance_avoidance", new ConfigOptionBools({false}));
...
void Plater::load_gcode(const wxString& filename)
{
    ...
    //QDS:already reset in new_project
    //current_result->reset();
    ...
    //p->notification_manager->qdt_show_plateinfo_notification(into_u8(_L("Preview only mode for gcode file.")));
    ...
    *current_result = std::move(processor.extract_result());
    ...
    //QDS: add cost info when drag in gcode
...
std::vector<size_t> Plater::load_files(const std::vector<fs::path>& input_files, LoadStrategy strategy, bool ask_multi) {
    //QDS: wish to reset state when load a new file
    p->m_slice_all_only_has_gcode = false;
    //QDS: wish to reset all plates stats item selected state when load a new file
...
bool Plater::load_files(const wxArrayString& filenames)
{
    ...
    //    ////QDS: only 3mf will be treated as project file
    //    if (open_3mf_file((*it)))
    //        return true;
    //}
...
//QDS: trigger a restore project event
...
void Plater::remove_selected()
{
    ...
    // QDS: check before deleting object
    if (!p->can_delete())
        return;
...
// QDS: replace z with plane_points
...
//y58
void Plater::export_gcode(const boost::filesystem::path& output_path)
{
...
void Plater::export_gcode_3mf(bool export_all)
{
    ...
    if (!output_path.empty()) {
        //QDS do not set to removable media path
        bool path_on_removable_media = false;
        p->notification_manager->new_export_began(path_on_removable_media);
        p->exporting_status = path_on_removable_media ? ExportingStatus::EXPORTING_TO_REMOVABLE : ExportingStatus::EXPORTING_TO_LOCAL;
        //QDS do not save last output path
        p->last_output_path = output_path.string();
        p->last_output_dir_path = output_path.parent_path().string();
        int plate_idx = get_partplate_list().get_curr_plate_index();
        if (export_all)
            plate_idx = PLATE_ALL_IDX;
        export_3mf(output_path, SaveStrategy::Silence | SaveStrategy::SplitModel | SaveStrategy::WithGcode | SaveStrategy::SkipModel, plate_idx); // QDS: silence
...
// y11 
void Plater::send_gcode_to_printer()
{
    ...
    PrintHostSendQIDIDialog dlg(this, Plater::get_partplate_list().get_current_fff_print().print_statistics(), default_output_file);
    if(dlg.ShowModal() == wxID_OK)
    {
        std::string send_ip = dlg.send_ip;
        std::string show_ip = dlg.show_ip;
        PrintHostJob upload_job(send_ip, show_ip);
...
void Plater::send_gcode_to_printers()
{
    DynamicPrintConfig* physical_printer_config = &Slic3r::GUI::wxGetApp().preset_bundle->printers.get_edited_preset().config;
    if (! physical_printer_config || p->model.objects.empty())
        return;
    ...
    wxArrayString groups;
    {
     ...
     PrintHostSendQIDIDialog dlg(default_output_file, upload_job.printhost->get_post_upload_actions(), groups, storage_paths, storage_names,
                             this, Plater::get_partplate_list().get_current_fff_print().print_statistics());
     if (dlg.ShowModal() == wxID_OK) {
        //B53 //B62 //B64
         auto pppd = dlg.pppd();
...
// QDS export with/without boolean, however, stil merge mesh
...
int Plater::export_3mf(const boost::filesystem::path& output_path, SaveStrategy strategy, int export_plate_idx, Export3mfProgressFn proFn)
{
    ...
    //QDS: add plate logic for thumbnail generate
    ...
    if (!(strategy & SaveStrategy::Backup)) {
        ...
        if (p->partplate_list.get_curr_plate()->is_slice_result_valid()) {
            //QDS generate QDS calibration thumbnails
    ...
    bool store_result = Slic3r::store_qds_3mf(store_params);
    // reset designed info
    if (!has_design_info)
...
void Plater::reslice()
{
    ...
    if (model().calib_pa_pattern) {
        ...
        model().calib_pa_pattern->generate_custom_gcodes(
            ...
            preset_bundle->printers.get_edited_preset().is_qdt_vendor_preset(preset_bundle),
            model(),
    ...
    if (p->background_process.running())
    {
        ...
        //QDS: add reset logic for empty plate
...
//QDS: add project slicing related logic
    ...
    if (key == std::string("print_sequence")) {
        ...
        if (seq_print && view3d_canvas && view3d_canvas->is_initialized() && view3d_canvas->is_rendering_enabled()) {
            ...
            if (seq_print->value == PrintSequence::ByObject) {
                ...
                notify_manager->qdt_show_seqprintinfo_notification(info_text);
            }
            else
                notify_manager->qdt_close_seqprintinfo_notification();
        }
    }
...
void Plater::on_config_change(const DynamicPrintConfig &config)
{
    ...
    for (auto opt_key : diff_keys) {
        ...
        if (opt_key == "printer_technology") {
            ...
            //QDS: invalid all the slice results
...
void Plater::set_bed_shape() const
{
    ...
    if (bundle != nullptr) {
        ...
        if (curr->is_system) {
            ...
            bool is_configed_by_QDT = PresetUtils::system_printer_bed_model(*curr).size() > 0;
            if (is_configed_by_QDT) {
                bool is_qdt_preset = bundle->printers.get_selected_preset().is_qdt_vendor_preset(bundle);
                if (wxGetApp().app_config->has_section("user_qdt_svg_list")) {
                    auto cur_preset_name   = bundle->printers.get_edited_preset().name;
                    auto user_qdt_svg_list = wxGetApp().app_config->get_section("user_qdt_svg_list");
                    if (user_qdt_svg_list.size() > 0 && user_qdt_svg_list[cur_preset_name].size() > 0) {
                        texture_filename = user_qdt_svg_list[cur_preset_name];
                    } else {
                        if (is_qdt_preset) {
                            texture_filename = "";
                        }
                    }
                } else if (is_qdt_preset) {
                    texture_filename = "";
                }
            }
        }
...
{
//QDS: filament_color logic has been moved out of filament setting
#if 0
    bool update_scheduled = false;
...
void Plater::force_print_bed_update()
{
    ...
    p->config->opt_string("printer_model", true) = "qdt_empty";
}
...
{
    // QDS modify view3D->get_canvas3d() to current canvas
...
bool Plater::is_printer_configed_by_QDT() {
    auto  curr_preset        = wxGetApp().preset_bundle->printers.get_edited_preset();
    bool  is_configed_by_QDT = PresetUtils::system_printer_bed_model(curr_preset).size() > 0;
...
//QDS: partplate list related functions
...
void Plater::validate_current_plate(bool& model_fits, bool& validate_error)
{
    ...
    if (p->printer_technology == ptFFF) {
        ...
        if (err.string.empty()) {
            ...
            p->notification_manager->qdt_close_3mf_warn_notification();
        ...
        {
            p->notification_manager->qdt_show_filament_map_invalid_notification_before_slice(NotificationType::LeftExtruderUnprintableError, left_unprintable_text);
        }
        else {
            p->notification_manager->qdt_close_filament_map_invalid_notification_before_slice(NotificationType::LeftExtruderUnprintableError);
        }

        if (!right_unprintable_text.empty())
        {
            p->notification_manager->qdt_show_filament_map_invalid_notification_before_slice(NotificationType::RightExtruderUnprintableError,right_unprintable_text);
        }
        else {
            p->notification_manager->qdt_close_filament_map_invalid_notification_before_slice(NotificationType::RightExtruderUnprintableError);
        }
...
//QDS: delete the plate, index= -1 means the current plate
...
//QDS: set bed positions
void Plater::set_bed_position(Vec2d& pos)
{
    p->bed.set_position(pos);
}

//QDS: is the background process slicing currently
...
void Plater::show_object_info()
{
    ...
    if (selection.is_wipe_tower()) {
        ...
        notify_manager->qdt_show_objectsinfo_notification(info_text, false, !(p->current_panel == p->view3D), _u8L("View Wiki for more information"), [](wxEvtHandler*) {
            std::string language = wxGetApp().app_config->get("language");
            wxString    region = L"en";
            if (language.find("zh") == 0)
            	region = L"zh";
            //y58
            wxGetApp().open_browser_with_warning_dialog(wxString::Format("https://wiki.qidi3d.com/%s/software/qidi-studio/print-settings/prime-tower", region));
            return false;
            });
        return;
    }
    if (selCount > 1 && !selection.is_single_full_object()) {
        notify_manager->qdt_close_objectsinfo_notification();
        ...
        notify_manager->qdt_show_objectsinfo_notification(info_text, false, !(p->current_panel == p->view3D));
        return;
    }
...
bool Plater::leave_gizmos_stack() { return p->leave_gizmos_stack(); } // QDS: return false if not changed
...
// QDS
//QDS: add popup logic for table object
