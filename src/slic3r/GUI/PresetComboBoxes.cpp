...
int PresetComboBox::update_ams_color()
{
    ...
    if (idx < 0) {
        ...
        if (iter == ams_list.end()) {
            BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << boost::format(": box %1% out of range %2%") % idx % ams_list.size();
            return -1;
        }
...
bool PresetComboBox::add_ams_filaments(std::string selected, bool alias_name)
{
    ...
    if (is_qdt_vendor_preset && !m_preset_bundle->filament_ams_list.empty()) {
        ...
        if (fila_switch_ready) {
            set_label_marker(Append(_L("BOX filaments"), wxNullBitmap, DD_ITEM_STYLE_SPLIT_ITEM));
        } else {
            set_label_marker(Append(dual_extruder ? _L("Left filaments") : _L("BOX filaments"), wxNullBitmap, DD_ITEM_STYLE_SPLIT_ITEM));
        }
...
bool PresetComboBox::add_box_filaments(std::string selected, bool alias_name)
{
    ...
    if (is_qdt_vendor_preset && !m_preset_bundle->filament_ams_list.empty()) {
        ...
        for (auto& entry : m_preset_bundle->filament_ams_list) {
            ...
            auto iter = std::find_if(filaments.begin(), filaments.end(),
            //y80
                [&filament_id, this](auto& f) { return m_collection->get_preset_base(f) == &f && f.filament_id == filament_id; });
            if (iter == filaments.end()) {
                auto filament_type = tray.opt_string("filament_type", 0u);
                if (!filament_type.empty()) {
                    //y59
                    filament_type = "QIDI " + filament_type;
                    iter = std::find_if(filaments.begin(), filaments.end(),
                        [&filament_type](auto& f) { return f.is_compatible && f.is_system && f.name.find(filament_type) != std::string::npos ; });
                }
            }
            if (iter == filaments.end()) {
                BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << boost::format(": filament_id %1% not found or system or compatible") % filament_id;
    ...
    PresetComboBox(parent, preset_type, wxSize(25 * wxGetApp().em_unit(), 30 * wxGetApp().em_unit() / 10))
{
    ...
    if (m_type == Preset::TYPE_FILAMENT) {
        ...
        clr_picker->Bind(wxEVT_BUTTON, [this](wxCommandEvent& e) {
            ...
            bool is_official = boost::algorithm::starts_with(fila_type, "QIDI");
            if (is_official) {
...
bool PlaterPresetComboBox::switch_to_tab()
{
    ...
    //QDS  Select NoteBook Tab params
...
void PlaterPresetComboBox::update()
{
    ...
    for (size_t i = presets.front().is_visible ? 0 : m_collection->num_default_presets(); i < presets.size(); ++i)
    {
        ...
        if (m_type == Preset::TYPE_FILAMENT)
        {
            ...
            if (preset.is_system) {
                ...
                if (preset_filament_vendors[name] == "QIDI")
                    preset_filament_vendors[name] = "QIDI";
    ...
    bool selected_in_box = false;
    if (m_type == Preset::TYPE_FILAMENT) {
        set_replace_text("QIDI", "QIDIStudioBlack");
    ...
    std::vector<std::string> filament_orders = {"QIDI PLA Basic", "QIDI PLA Matte", "QIDI PLA Rapido", "QIDI PLA Lite", "QIDI PLA Tough+", "QIDI PETG Basic", "QIDI PETG Rapido", "QIDI ABS Rapido",
                                                ...
                                                "QIDI PPA-CF", "QIDI PPS-CF", "QIDI PLA Aero", "QIDI ASA-Aero", "QIDI TPU for AMS", "QIDI TPU 95A HF", "QIDI TPU 90A",
                                                "QIDI TPU 85A", "QIDI Support For PLA", "QIDI Support For PLA/PETG", "QIDI Support for ABS", "QIDI PVA", "QIDI Support For PA/PET",
                                                "QIDI TPU 95A", "QIDI PA-CF", "QIDI PLA Tough", "QIDI PLA Dynamic", "QIDI Support W", "QIDI Support G"};
    //w38
    std::vector<std::string> first_vendors     = {"QIDI", "Generic","Bambu Lab","HATCHBOX","Overture","Polymaker"};
    ...
    auto  add_presets       = [this, &preset_descriptions, &filament_orders, &preset_filament_vendors, &first_vendors, &preset_filament_types, &first_types, &selected_in_box, &polymaker_priority]
    ...
    //QDS: remove unused pysical printer logic
    /*if (m_type == Preset::TYPE_PRINTER)
    {
...
void TabPresetComboBox::update()
{
    ...
    // QDS Add/Remove filaments select
    ...
    for (unsigned int ui_id = 0; ui_id < GetCount(); ++ui_id) {
        ...
        if (preset) {
            ...
            if (old_label != new_label) {
                ...
                if (ui_id == GetSelection()) SetToolTip(wxString::FromUTF8(new_label.c_str())); // QDS
            }
        }
    }
#ifdef __APPLE__
