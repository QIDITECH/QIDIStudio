...
struct StaticQIDILib : QIDILib {
    static StaticQIDILib &get(QIDILib * copy = nullptr);
    static int Fake_QIDI_Create(QIDI_Tunnel*, char const*) { return -2; }
    static void reset();
    static void release();
private:
    std::vector<QIDILib *> copies_;
};
...
void PrinterFileSystem::DumpLog(void * thiz, int, tchar const *msg)
{
    ...
    static_cast<PrinterFileSystem*>(thiz)->QIDI_FreeLogMsg(msg);
}
...
void PrinterFileSystem::RecvMessageThread()
{
    ...
    while (true) {
        ...
        if (!m_messages.empty()) {
            ...
            l.unlock();
            int n = QIDI_SendMessage(m_session.tunnel, CTRL_TYPE, msg.c_str(), msg.length());
            l.lock();
            if (n == 0)
                m_messages.pop_front();
            else if (n != QIDI_would_block) {
                Reconnect(l, n);
                continue;
            }
        }
        l.unlock();
        int n = QIDI_ReadSample(m_session.tunnel, &sample);
        l.lock();
        if (n == 0) {
            HandleResponse(l, sample);
        } else if (n == QIDI_stream_end) {
            m_stopped = true;
...
void PrinterFileSystem::HandleResponse(boost::unique_lock<boost::mutex> &l, QIDI_Sample const &sample)
{
...
void PrinterFileSystem::Reconnect(boost::unique_lock<boost::mutex> &l, int result)
{
    ...
    while (true) {
        ...
        if (url.size() < 2) {
            ...
            int ret = QIDI_Create(&tunnel, url.c_str());
            if (ret == 0) {

                QIDI_SetLogger(tunnel, DumpLog, this);
                ret = QIDI_Open(tunnel);
            }
            ...
            if (ret == 0)
            {
                ...
                do{
                    ret = QIDI_StartStreamEx ? QIDI_StartStreamEx(tunnel, CTRL_TYPE) : QIDI_StartStream(tunnel, false);
                    if (ret == QIDI_would_block)
                ...
                } while (ret == QIDI_would_block && !m_stopped);
            }
            l.lock();
            if (ret == 0) {
...
StaticQIDILib &StaticQIDILib::get(QIDILib *copy)
{
    static StaticQIDILib lib;
    ...
    if (!module) {
        module = Slic3r::NetworkAgent::get_qidi_source_entry();
    }
    ...
    GET_FUNC(QIDI_Create);
    GET_FUNC(QIDI_Open);
    GET_FUNC(QIDI_StartStream);
    GET_FUNC(QIDI_StartStreamEx);
    GET_FUNC(QIDI_GetStreamCount);
    GET_FUNC(QIDI_GetStreamInfo);
    GET_FUNC(QIDI_SendMessage);
    GET_FUNC(QIDI_ReadSample);
    GET_FUNC(QIDI_Close);
    GET_FUNC(QIDI_Destroy);
    GET_FUNC(QIDI_SetLogger);
    GET_FUNC(QIDI_FreeLogMsg);
    GET_FUNC(QIDI_Deinit);

    if (!lib.QIDI_Create) {
        lib.QIDI_Create = Fake_QIDI_Create;
        if (copy)
...
void StaticQIDILib::reset()
{
    get().QIDI_Create = nullptr;
    auto &lib = get();
    for (auto c : lib.copies_)
        *c = lib;
}

void StaticQIDILib::release()
{
    if (auto f = get().QIDI_Deinit)
        f();
}

extern "C" QIDILib *qidilib_get() {
    return &StaticQIDILib::get(); }
