...
QDTStatusBar::QDTStatusBar(wxWindow *parent, int id)
...
int QDTStatusBar::get_progress() const
{
    return m_prog->GetValue();
}

void QDTStatusBar::set_progress(int val)
{
    if(val < 0)
        return;
...
int QDTStatusBar::get_range() const
{
    return m_prog->GetRange();
}

void QDTStatusBar::set_range(int val)
{
    if(val != m_prog->GetRange()) {
        m_prog->SetRange(val);
    }
...
void QDTStatusBar::clear_percent()
{

}

void QDTStatusBar::show_error_info(wxString msg, int code, wxString description, wxString extra)
{

}

void QDTStatusBar::show_progress(bool show)
{
    if (show) {
...
void QDTStatusBar::start_busy(int rate)
{
    m_busy = true;
    show_progress(true);
    show_cancel_button();
...
void QDTStatusBar::set_cancel_callback(QDTStatusBar::CancelFn ccb) {
    m_cancel_cb = ccb;
    if (ccb) {
        m_sizer->Show(m_cancelbutton);
    }
...
wxPanel* QDTStatusBar::get_panel()
{
    return m_self;
}

void QDTStatusBar::set_status_text(const wxString& txt)
{
    m_status_text->SetLabelText(txt);
}

void QDTStatusBar::set_status_text(const std::string& txt)
{ 
    this->set_status_text(txt.c_str());
}

void QDTStatusBar::set_status_text(const char *txt)
{ 
    this->set_status_text(wxString::FromUTF8(txt));
}

wxString QDTStatusBar::get_status_text() const
{
    return m_status_text->GetLabelText();
}

void QDTStatusBar::set_object_info(const wxString& txt)
{
    if (txt == "") {
...
void QDTStatusBar::set_slice_info(const wxString& txt)
{
    if (!txt.empty()) {
        if (!m_sizer->IsShown(m_slice_info_sizer)) {
            m_sizer->Show(m_slice_info_sizer);
...
void QDTStatusBar::show_slice_info(bool show)
{
    if (show) {
        m_sizer->Show(m_slice_info_sizer);
        m_sizer->Layout();
...
bool QDTStatusBar::is_slice_info_shown()
{
    return m_sizer->IsShown(m_slice_info_sizer);
}

void QDTStatusBar::set_font(const wxFont &font)
{
    m_self->SetFont(font);
}

void QDTStatusBar::show_cancel_button()
{
    m_sizer->Show(m_cancelbutton);
    m_sizer->Layout();
}

void QDTStatusBar::hide_cancel_button()
{
    m_sizer->Hide(m_cancelbutton);
    m_sizer->Layout();
}
