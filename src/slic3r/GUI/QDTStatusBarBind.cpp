...
QDTStatusBarBind::QDTStatusBarBind(wxWindow *parent, int id)
...
void QDTStatusBarBind::set_prog_block()
{
}

int QDTStatusBarBind::get_progress() const
{
    return m_prog->GetValue();
}

void QDTStatusBarBind::set_progress(int val)
{
    set_prog_block();
...
int QDTStatusBarBind::get_range() const
{
    return m_prog->GetRange();
}

void QDTStatusBarBind::set_range(int val)
{
...
void QDTStatusBarBind::clear_percent()
{

}

void QDTStatusBarBind::show_error_info(wxString msg, int code, wxString description, wxString extra)
{

}

void QDTStatusBarBind::show_progress(bool show)
{
    if (show) {
        m_sizer->Show(m_prog);
        m_sizer->Layout();
...
void QDTStatusBarBind::start_busy(int rate)
{
    m_busy = true;
    show_progress(true);
    show_cancel_button();
...
void QDTStatusBarBind::stop_busy()
{
    show_progress(false);
    hide_cancel_button();
    m_prog->SetValue(0);
...
void QDTStatusBarBind::set_cancel_callback_fina(QDTStatusBarBind::CancelFn ccb) 
{ 
...
void QDTStatusBarBind::set_cancel_callback(QDTStatusBarBind::CancelFn ccb) {
    /*  m_cancel_cb = ccb;
      if (ccb) {
          m_sizer->Show(m_cancelbutton);
      }
...
wxPanel* QDTStatusBarBind::get_panel()
{
    return m_self;
}

void QDTStatusBarBind::set_status_text(const wxString& txt)
{
...
void QDTStatusBarBind::set_percent_text(const wxString &txt)
{
    m_stext_percent->SetLabelText(txt);
}

void QDTStatusBarBind::set_status_text(const std::string& txt)
{ 
    this->set_status_text(txt.c_str());
}

void QDTStatusBarBind::set_status_text(const char *txt)
{ 
    this->set_status_text(wxString::FromUTF8(txt));
}

void QDTStatusBarBind::msw_rescale() { 
    set_prog_block();
    m_cancelbutton->SetMinSize(wxSize(m_self->FromDIP(56), m_self->FromDIP(24)));
}

wxString QDTStatusBarBind::get_status_text() const
{
    return m_status_text->GetLabelText();
}

bool QDTStatusBarBind::update_status(wxString &msg, bool &was_cancel, int percent, bool yield)
{
    set_status_text(msg);
...
void QDTStatusBarBind::set_font(const wxFont &font)
{
    m_self->SetFont(font);
}

void QDTStatusBarBind::show_cancel_button()
{
    m_sizer->Show(m_cancelbutton);
    m_sizer->Layout();
}

void QDTStatusBarBind::hide_cancel_button()
{
    m_sizer->Hide(m_cancelbutton);
    m_sizer->Layout();
}
