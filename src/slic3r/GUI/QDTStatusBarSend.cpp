...
void QDTStatusBarSend::set_prog_block()
{
}

int QDTStatusBarSend::get_progress() const
{
    return m_prog->GetValue();
}

void QDTStatusBarSend::set_progress(int val)
{
    if(val < 0) return;
...
int QDTStatusBarSend::get_range() const
{
    return m_prog->GetRange();
}

void QDTStatusBarSend::set_range(int val)
{
...
void QDTStatusBarSend::clear_percent()
{
    //set_percent_text(wxEmptyString);
    m_cancelbutton->Hide();
}

void QDTStatusBarSend::show_error_info(wxString msg, int code, wxString description, wxString extra)
{
    set_status_text(msg);
    m_prog->Hide();
...
void QDTStatusBarSend::show_progress(bool show)
{
    if (show) {
        m_sizer->Show(m_prog);
        m_sizer->Layout();
...
void QDTStatusBarSend::start_busy(int rate)
{
    m_busy = true;
    show_progress(true);
    show_cancel_button();
...
void QDTStatusBarSend::stop_busy()
{
    show_progress(false);
    hide_cancel_button();
    m_prog->SetValue(0);
...
void QDTStatusBarSend::set_cancel_callback(QDTStatusBarSend::CancelFn ccb) {
    /*  m_cancel_cb = ccb;
      if (ccb) {
          m_sizer->Show(m_cancelbutton);
      }
...
wxPanel* QDTStatusBarSend::get_panel()
{
    return m_self;
}

bool QDTStatusBarSend::is_english_text(wxString str)
{
...
bool QDTStatusBarSend::format_text(wxStaticText* dc, int width, const wxString& text, wxString& multiline_text)
{
...
void QDTStatusBarSend::set_status_text(const wxString& txt)
{
...
void QDTStatusBarSend::set_percent_text(const wxString &txt)
{
    m_stext_percent->SetLabelText(txt);
}

void QDTStatusBarSend::set_status_text(const std::string& txt)
{ 
    this->set_status_text(txt.c_str());
}

void QDTStatusBarSend::set_status_text(const char *txt)
{ 
...
void QDTStatusBarSend::msw_rescale() { 
    //set_prog_block();
    m_cancelbutton->SetMinSize(wxSize(m_self->FromDIP(56), m_self->FromDIP(24)));
}

wxString QDTStatusBarSend::get_status_text() const
{
    return m_status_text->GetLabelText();
}

bool QDTStatusBarSend::update_status(wxString &msg, bool &was_cancel, int percent, bool yield)
{
    set_status_text(msg);
    if (percent >= 0)
...
void QDTStatusBarSend::set_font(const wxFont &font)
{
    m_self->SetFont(font);
}

void QDTStatusBarSend::show_cancel_button()
{
    m_sizer->Show(m_cancelbutton);
    m_sizer->Layout();
}

void QDTStatusBarSend::hide_cancel_button()
{
    m_sizer->Hide(m_cancelbutton);
    m_sizer->Layout();
}

void QDTStatusBarSend::change_button_label(wxString name) 
{
    m_cancelbutton->SetLabel(name);
}

void QDTStatusBarSend::disable_cancel_button()
{
