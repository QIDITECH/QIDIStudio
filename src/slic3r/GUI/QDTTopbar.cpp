...
class QDTTopbarArt : public wxAuiDefaultToolBarArt
{
public:
...
void QDTTopbarArt::DrawLabel(wxDC& dc, wxWindow* wnd, const wxAuiToolBarItem& item, const wxRect& rect)
{
    dc.SetFont(m_font);
#ifdef __WINDOWS__
...
void QDTTopbarArt::DrawBackground(wxDC& dc, wxWindow* wnd, const wxRect& rect)
{
...
void QDTTopbarArt::DrawButton(wxDC& dc, wxWindow* wnd, const wxAuiToolBarItem& item, const wxRect& rect)
{
    int textWidth = 0, textHeight = 0;
...
void QDTTopbar::Init(wxFrame* parent)
{
    ...
    */
    ...
    this->Bind(wxEVT_MOTION, &QDTTopbar::OnMouseMotion, this);
    this->Bind(wxEVT_MOUSE_CAPTURE_LOST, &QDTTopbar::OnMouseCaptureLost, this);
    this->Bind(wxEVT_MENU_CLOSE, &QDTTopbar::OnMenuClose, this);
    this->Bind(wxEVT_AUITOOLBAR_TOOL_DROPDOWN, &QDTTopbar::OnFileToolItem, this, ID_TOP_FILE_MENU);
    this->Bind(wxEVT_AUITOOLBAR_TOOL_DROPDOWN, &QDTTopbar::OnDropdownToolItem, this, ID_TOP_DROPDOWN_MENU);
    this->Bind(wxEVT_AUITOOLBAR_TOOL_DROPDOWN, &QDTTopbar::OnCalibToolItem, this, ID_CALIB);
    this->Bind(wxEVT_AUITOOLBAR_TOOL_DROPDOWN, &QDTTopbar::OnIconize, this, wxID_ICONIZE_FRAME);
    this->Bind(wxEVT_AUITOOLBAR_TOOL_DROPDOWN, &QDTTopbar::OnFullScreen, this, wxID_MAXIMIZE_FRAME);
    this->Bind(wxEVT_AUITOOLBAR_TOOL_DROPDOWN, &QDTTopbar::OnCloseFrame, this, wxID_CLOSE_FRAME);
    this->Bind(wxEVT_LEFT_DCLICK, &QDTTopbar::OnMouseLeftDClock, this);
    this->Bind(wxEVT_LEFT_DOWN, &QDTTopbar::OnMouseLeftDown, this);
    this->Bind(wxEVT_LEFT_UP, &QDTTopbar::OnMouseLeftUp, this);
    this->Bind(wxEVT_AUITOOLBAR_TOOL_DROPDOWN, &QDTTopbar::OnOpenProject, this, wxID_OPEN);
    this->Bind(wxEVT_AUITOOLBAR_TOOL_DROPDOWN, &QDTTopbar::OnSaveProject, this, wxID_SAVE);
    this->Bind(wxEVT_AUITOOLBAR_TOOL_DROPDOWN, &QDTTopbar::OnRedo, this, wxID_REDO);
    this->Bind(wxEVT_AUITOOLBAR_TOOL_DROPDOWN, &QDTTopbar::OnUndo, this, wxID_UNDO);
    //this->Bind(wxEVT_AUITOOLBAR_TOOL_DROPDOWN, &QDTTopbar::OnModelStoreClicked, this, ID_MODEL_STORE);
    this->Bind(wxEVT_AUITOOLBAR_TOOL_DROPDOWN, &QDTTopbar::OnPublishClicked, this, ID_PUBLISH);
}
...
void QDTTopbar::OnOpenProject(wxAuiToolBarEvent& event)
{
...
void QDTTopbar::show_publish_button(bool show)
{
    this->EnableTool(m_publish_item->GetId(), show);
    Refresh();
}

void QDTTopbar::OnSaveProject(wxAuiToolBarEvent& event)
{
    MainFrame* main_frame = dynamic_cast<MainFrame*>(m_frame);
    Plater* plater = main_frame->plater();
    plater->save_project();
...
void QDTTopbar::OnRedo(wxAuiToolBarEvent& event)
{
    MainFrame* main_frame = dynamic_cast<MainFrame*>(m_frame);
    Plater* plater = main_frame->plater();
    plater->redo();
...
void QDTTopbar::DisableUndoRedoItems()
{
...
void QDTTopbar::SaveNormalRect()
{
    m_normalRect = m_frame->GetRect();
}

void QDTTopbar::ShowCalibrationButton(bool show)
{
...
void QDTTopbar::OnModelStoreClicked(wxAuiToolBarEvent& event)
{
    //GUI::wxGetApp().load_url(wxString(wxGetApp().app_config->get_web_host_url() + MODEL_STORE_URL));
}

void QDTTopbar::OnPublishClicked(wxAuiToolBarEvent& event)
{
...
void QDTTopbar::SetFileMenu(wxMenu* file_menu)
{
    m_file_menu = file_menu;
}

void QDTTopbar::AddDropDownSubMenu(wxMenu* sub_menu, const wxString& title)
{
    m_top_menu.AppendSubMenu(sub_menu, title);
}

void QDTTopbar::AddDropDownMenuItem(wxMenuItem* menu_item)
{
    m_top_menu.Append(menu_item);
}

wxMenu* QDTTopbar::GetTopMenu()
{
    return &m_top_menu;
}

wxMenu* QDTTopbar::GetCalibMenu()
{
    return &m_calib_menu;
}

void QDTTopbar::SetTitle(wxString title)
{
    wxGCDC dc(this);
...
void QDTTopbar::SetMaximizedSize()
{
    maximize_btn->SetBitmap(maximize_bitmap);
}

void QDTTopbar::SetWindowSize()
{
    maximize_btn->SetBitmap(window_bitmap);
}

void QDTTopbar::UpdateToolbarWidth(int width)
{
    this->SetSize(width, m_toolbar_h);
}

void QDTTopbar::Rescale() {
    int em = em_unit(this);
...
void QDTTopbar::OnIconize(wxAuiToolBarEvent& event)
{
    m_frame->Iconize();
}

void QDTTopbar::OnFullScreen(wxAuiToolBarEvent& event)
{
    if (m_frame->IsMaximized()) {
        m_frame->Restore();
    }
...
void QDTTopbar::OnCloseFrame(wxAuiToolBarEvent& event)
{
    m_frame->Close();
}

void QDTTopbar::OnMouseLeftDClock(wxMouseEvent& mouse)
{
...
void QDTTopbar::OnFileToolItem(wxAuiToolBarEvent& evt)
{
...
void QDTTopbar::OnDropdownToolItem(wxAuiToolBarEvent& evt)
{
    wxAuiToolBar* tb = static_cast<wxAuiToolBar*>(evt.GetEventObject());
...
void QDTTopbar::OnCalibToolItem(wxAuiToolBarEvent &evt)
{
    wxAuiToolBar *tb = static_cast<wxAuiToolBar *>(evt.GetEventObject());
...
void QDTTopbar::OnMouseLeftDown(wxMouseEvent& event)
{
    wxPoint mouse_pos = ::wxGetMousePosition();
...
void QDTTopbar::OnMouseLeftUp(wxMouseEvent& event)
{
    wxPoint mouse_pos = ::wxGetMousePosition();
    if (HasCapture())
    {
...
void QDTTopbar::OnMouseMotion(wxMouseEvent& event)
{
    wxPoint mouse_pos = ::wxGetMousePosition();
...
void QDTTopbar::OnMouseCaptureLost(wxMouseCaptureLostEvent& event)
{
}

void QDTTopbar::OnMenuClose(wxMenuEvent& event)
{
...
wxAuiToolBarItem* QDTTopbar::FindToolByCurrentPosition()
{
    wxPoint mouse_pos = ::wxGetMousePosition();
