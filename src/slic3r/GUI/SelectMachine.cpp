...
void SelectMachineDialog::sync_ams_mapping_result(const std::vector<FilamentInfo> &result)
{
    ...
    //y59
    bool exceed_slot_num = result.size() > (wxGetApp().preset_bundle->filament_ams_list).size() ? true : false;
        ...
        while (iter != m_materialList.end()) {
            ...
            if (f->id == id) {
                ...
                if (f->tray_id == VIRTUAL_TRAY_MAIN_ID || f->tray_id == VIRTUAL_TRAY_DEPUTY_ID)
                {
                    ...
                    ams_id = wxGetApp().transition_tridid(std::stoi(f->slot_id));   //y59
                } else {
                    ams_id = "-";
                }
...
bool SelectMachineDialog::do_ams_mapping(MachineObject *obj_,bool use_ams, bool from_sdcard_view)
{
    ...
    //single nozzle
        ...
        //y78
        filament_result = DevMappingUtil::ams_filament_mapping(obj_, m_filaments, m_ams_mapping_result, map_opt, {}, false, from_sdcard_view);
        // auto_supply_with_ext(obj_->vt_slot);
    }
...
void SelectMachineDialog::on_ok_btn(wxCommandEvent &event)
{
    ...
    //        }
        ...
        // const auto& nozzle = get_mapped_nozzle(fila.id); // not check it if it's nullopt
        // if (nozzle.has_value()) {
...
void SelectMachineDialog::reset_and_sync_ams_list()
{
    ...
    for (auto i = 0; i < extruders.size(); i++) {
        ...
        item->Bind(wxEVT_LEFT_DOWN, [this, item, materials, extruder](wxMouseEvent &e) {
            ...
            //y68
            if (obj_ && !m_ams_mapping_result.empty()) {
        ...
        item->Bind(wxEVT_LEFT_DOWN, [this, obj, item, materials, diameters_count, fo](wxMouseEvent& e) {
            ...
            if (obj) {
                ...
                if (obj) {
                    ...
                    m_mapping_popup.set_tag_texture(fo.type);
                    m_mapping_popup.set_send_win(this);
                    m_mapping_popup.update(nullptr, m_ams_mapping_result, obj);
                    m_mapping_popup.Popup();
                }
            }
            });
...
bool SelectMachineDialog::CheckErrorWarningFilamentMapping(MachineObject* obj_)
{
    ...
    for (const auto& fila : m_ams_mapping_result) {
        ...
        check_info.extruder_id = obj_->GetFilaSystem()->GetExtruderIdByAmsId(std::to_string(check_info.ams_id));
        const auto& nozzle = get_mapped_nozzle(fila.id);
        if (nozzle.has_value()) {
            check_info.nozzle_flow = DevNozzle::GetNozzleFlowTypeString(nozzle->GetNozzleFlowType());
            check_info.nozzle_diameter = nozzle->GetNozzleDiameter();
        }
