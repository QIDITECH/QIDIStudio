    ...
    , m_mapping_tutorial_popup(AmsTutorialPopup(this))
{
    ...
    //y58
    wxPanel* switch_button_panel = new wxPanel(m_basic_panel, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBU_LEFT | wxTAB_TRAVERSAL | wxBU_RIGHT);
    ...
    m_switch_button->Bind(wxEVT_TOGGLEBUTTON, [this](wxCommandEvent& evt) {
        //y78
        bool is_checked;
        bool is_from_sdcard = false;
        if (m_print_type == FROM_SDCARD_VIEW)
        {
            auto qdsdev = wxGetApp().qdsdevmanager;
            auto obj = qdsdev->getSelectedDevice();
            is_checked = obj->is_net_device;
            is_from_sdcard = true;
        }
        else {
            is_checked = evt.GetInt();
        }
        ...
        //y65
        if (!m_isNetMode)
            m_printer_box->SetQDTPrinters(machine_list_local, is_from_sdcard);
        else
            m_printer_box->SetQDTPrinters(machine_list_link, is_from_sdcard);

        // y27
        if (m_printer_box->GetSelectedPrinterName().empty())
    ...
    sizer_switch_area->AddSpacer(20);
    sizer_switch_area->Add(m_isSwitch, 1, wxALIGN_CENTER);
    ...
    wxBoxSizer* sizer_of_printer_switch = new wxBoxSizer(wxVERTICAL);
    sizer_of_printer_switch->Add(m_printer_box, 0, wxTOP, 0);
    sizer_of_printer_switch->Add(switch_button_panel, 0, wxTOP, 10);
    ...
    sizer_basic_right_info->Add(sizer_of_printer_switch, 0, wxTOP, 0);
    sizer_basic_right_info->Add(0, 0, 0, wxTOP, FromDIP(10));
    sizer_basic_right_info->Add(m_text_printer_msg, 0, wxLEFT, 0);
    sizer_basic_right_info->AddSpacer(FromDIP(4));
    ...
    m_link_edit_nozzle->SetForegroundColour(0x4479fb);
...
//y76
    ...
    m_pa_value_tips->Bind(wxEVT_BUTTON, [this](wxCommandEvent &e){
        ...
        wxString url = wxString::Format("https://wiki.qidi3d.com/%s/software/qidi-studio", region);
    ...
    m_btn_bg_enable = StateColor(std::pair<wxColour, int>(wxColour(0, 66, 255), StateColor::Pressed), std::pair<wxColour, int>(wxColour(116, 168, 255), StateColor::Hovered),
    ...
    // sending mode
    m_status_bar    = std::make_shared<QDTStatusBarPrint>(m_simplebook);
...
void SelectMachineDialog::popup_filament_backup()
{
    ...
    //m_replace_material_popup->update_to_nozzle(MAIN_NOZZLE_ID);
...
//y78
bool SelectMachineDialog::do_ams_mapping(MachineObject *obj_,bool use_ams, bool from_sdcard_view)
{
    //y59
...
void SelectMachineDialog::show_status(PrintDialogStatus status, std::vector<wxString> params, wxString wiki_url, prePrintInfoStyle style)
{
    ...
    if (status == PrintDialogStatus::PrintStatusInit) {
        ...
        Enable_Refresh_Button(true);
        Enable_Send_Button(false);
        msg = _L("Please do not mix-use the Ext with BOX");
    ...
    //y65
       ...
       msg = _L("The printer is offline, please check.");
        Enable_Refresh_Button(true);
        Enable_Send_Button(false);
    }else if(status == PrintDialogStatus::PrintStatusPinrterPrinting){
        msg = _L("Current printer is printing now");
        Enable_Refresh_Button(true);
        Enable_Send_Button(false);
    } else if (status == PrintDialogStatus::PrintStatusPrinterNotStandby) {
       msg = _L("The printer is not standby, please check.");
        Enable_Refresh_Button(true);
        Enable_Send_Button(false);
    }else if(status == PrinterNotConnectBox){
        msg = _L("The selected printer is not connected to the BOX or the filaments are not synchronized. Please check.");
        Enable_Refresh_Button(true);
        Enable_Send_Button(true);
    } else if (status == PrintDialogStatus::PrintStatusNeedUpgradingBox) {
        if (params.size() > 0)
            msg = wxString::Format(_L("Filament %s exceeds the number of BOX slots. Please update the printer firmware to support BOX slot assignment."), params[0]);
        else
            msg = _L("Filament exceeds the number of BOX slots. Please update the printer firmware to support BOX slot assignment.");
        Enable_Refresh_Button(true);
        Enable_Send_Button(false);
    } else if(status == PrintDialogStatus::BoxhasSomeProblem){
        msg = _L("Failed to set the box printing slot...");
        Enable_Refresh_Button(true);
        Enable_Send_Button(false);
    ...
    //y72 
        ...
        msg = _L("Failed to set the bed leveling...");
        Enable_Refresh_Button(true);
        Enable_Send_Button(false);
    } else if(status == PrintDialogStatus::TimelapseSettingFailed){
...
void SelectMachineDialog::on_ok_btn(wxCommandEvent &event)
{
    ...
    // y16
    machine_name = into_u8(m_printer_box->GetPrinterComboBox()->GetValue());
...
void SelectMachineDialog::start_to_send(PrintHostJob upload_job) {
    ...
    {
       show_status(PrintDialogStatus::PrintStatusPinrterPrinting);
       return;
    }
    ...
    //y76
                ...
                m_status_bar->update_status(msg, m_is_canceled, 10, true);
                //y80
                if(item.second->getValue() == "on" && qds_dev->m_enable_polar_cooler){
    ...
    if (success)
    {
        ...
        if (is_switch_to_device) {
            ...
            //cj_4
            std::string target_device_id = m_printer_last_select;
            //cj_4
            if (target_device_id.empty()) {
                target_device_id = select_machine.device_id;
            }
            //cj_4
            wxGetApp().mainframe->jump_to_monitor(target_device_id);
        }                 
        else{
            for(int i = 3; i > 0; i--){
                if(m_is_canceled)
...
//y78
void SelectMachineDialog::send_to_sd_card(){
    Enable_Send_Button(false);
    m_is_canceled = false;
    show_status(PrintDialogStatus::PrintStatusSending);
    ...
    if(m_isNetMode){
            ...
            if(item.second->getParam() == "enable_multi_box"){
                if (item.second->getValue() == "on" && !m_ams_mapping_result.empty()){
                    ...
                    for(int i = 0; i < m_filaments.size(); i++){
                        json item;
                        item["filament"] = m_filaments[i].id;
                        item["slot"] = 16;
                        bodyJson["selects"].push_back(item);
                    }
                }
		...
		std::string resultBody = MakerHttpHandle::getInstance().httpPostTask(http_data, isSucceed);
        
        if (isSucceed){
            for(int i = 3; i > 0; i--){
                if(m_is_canceled)
                    return;
                wxString msg = wxString::Format(_L("The model is beginning to print. Close current page in %s s."), std::to_string(i));
                m_status_bar->update_status(msg, m_is_canceled, 100, true);
                std::this_thread::sleep_for(std::chrono::seconds(1));
            }
            EndDialog(wxID_CLOSE);
            ...
            wxString msg = _L("Failed to set printing task, please try again.");
            m_status_bar->update_status(msg, m_is_canceled, 100, true);
        ...
        for (auto item : m_checkbox_list) {
            ...
            if(item.second->getParam() == "enable_multi_box"){
                ...
                if(has_box_machine){
                    wxString msg = _L("Box Setting..");
                    m_status_bar->update_status(msg, m_is_canceled, 20, true);
                    if (item.second->getValue() == "on"){
                        command = "SAVE_VARIABLE VARIABLE=enable_box VALUE=1";
                        use_box = true;
                    }
                ...
                wxString msg = _L("Set automatic leveling..");
                m_status_bar->update_status(msg, m_is_canceled, 40, true);
                if (item.second->getValue() == "on") {
                    command = "G31";
                }
                else {
                ...
                bool open_timelapse;
                wxString msg = _L("Set timelapse..");
                m_status_bar->update_status(msg, m_is_canceled, 60, true);
                if (item.second->getValue() == "on") {
                    open_timelapse = true;
                }
                else {
...
void SelectMachineDialog::update_option_opts(MachineObject *obj)
{
    ...
    if(select_machine.bed_leveling){
...
void SelectMachineDialog::load_option_vals(MachineObject *obj)
{
    ...
    for (auto item : m_checkbox_list) {
        ...
        const std::string &val = config->get("print", item.first);
...
//y76
    auto sel_obj =  wxGetApp().qdsdevmanager->getDevice(m_printer_last_select);
    //y77
...
void SelectMachineDialog::save_option_vals(MachineObject *obj) {
    ...
    // if (!obj) return;
    for (auto item : m_checkbox_list) {
        config->set("print", item.first, item.second->getValue());
    }
}
    ...
    if (selection_data_arr.size() == 8) {
        ...
        for (auto i = 0; i < m_ams_mapping_result.size(); i++) {
            if (m_ams_mapping_result[i].slot_id == new_info.slot_id && new_info.id != m_ams_mapping_result[i].id) {
                ...
                auto color_change = DevAmsTray::decode_color(m_ams_mapping_result[i].color);
                if (m_print_type == PrintFromType::FROM_NORMAL) {//todo:support sd card
...
void SelectMachineDialog::update_user_printer()
{
    ...
    for (size_t i = 0; i < ph_printers.size(); i++)
    {
        ...
        machine.device_id = wxGetApp().qdsdevmanager->getLocalDeviceIDByIp(machine.ip); //y80
    ...
    if (wxGetApp().app_config->get("user_token") != "")
    {
        ...
        for (const auto& device : m_devices) {
            ...
            machine.device_id = wxGetApp().qdsdevmanager->getNetDeviceIDByIp(machine.ip);
...
void SelectMachineDialog::on_selection_changed(wxCommandEvent &event)
{
    ...
    //y24
    std::string selection_name = into_u8(m_printer_box->GetPrinterComboBox()->GetValue());
    std::string select_printer_ip;
    std::string select_machine_type = "";
...
void SelectMachineDialog::set_default_from_sdcard()
{
    ...
    m_printer_box->SetPrinterName(wxString::FromUTF8(obj->m_name));
    ...
    std::string opt_nozzle_diameters = m_required_data_plate_data_list[m_print_plate_idx]->nozzle_diameters;
    auto diameters_count = 1;
    //if (opt_nozzle_diameters) {
    //    diameters_count = opt_nozzle_diameters->size();
    //}
    ...
    //std::vector<int> filament_map;
    m_filaments_map.clear();
    if (!m_required_data_plate_data_list[m_print_plate_idx]->filament_maps.empty()) {
        m_filaments_map = m_required_data_plate_data_list[m_print_plate_idx]->filament_maps;
    }
    ...
    for (auto i = 0; i < m_required_data_plate_data_list[m_print_plate_idx]->slice_filaments_info.size(); i++) {
        ...
        item->Bind(wxEVT_LEFT_UP, [this, item, materials](wxMouseEvent& e) {});
        item->Bind(wxEVT_LEFT_DOWN, [this, obj, item, materials, diameters_count, fo](wxMouseEvent& e) {
            if (!item->m_enable) { return; }
            if (!m_check_flag || m_print_status == PrintDialogStatus::PrintStatusUnsupportedPrinter) { return; } /*STUDIO-11301*/
    ...
    // basic info
    try {
        double float_weight = std::stof(m_required_data_plate_data_list[m_print_plate_idx]->gcode_weight);
        wxString   time;
        time = wxString::Format("%s", m_required_data_plate_data_list[m_print_plate_idx]->gcode_prediction.c_str());
        char weight[64];
        ::sprintf(weight, "  %.2f g", float_weight);
        m_stext_time->SetLabel(time);
        m_stext_weight->SetLabel(weight);
...
void SelectMachineDialog::sys_color_changed()
{
    if (wxGetApp(). dark_mode()) {
        //rename_button->SetIcon("box_editable_light");
...
//y24
std::string SelectMachineDialog::NormalizeVendor(const std::string& str)
{
...
void PrintOptionItem::doRender(wxDC& dc)
{
    ...
    for (auto it = m_ops.begin(); it != m_ops.end(); ++it)
    {
        ...
        {
            const wxColour& clr = m_enable ? StateColor::darkModeColorFor("#4479FB") : StateColor::darkModeColorFor(wxColour(144, 144, 144));
            dc.SetPen(wxPen(clr));
...
{
    wxLaunchDefaultBrowser(wxT("https://wiki.qidiprinter3d.com/en/software/qidi-studio/failed-to-connect-printer"));
}
...
void PrinterInfoBox::SetQDTPrinters(std::vector<Machine_info> machine_list, bool is_from_sdcard_view)
{
    ...
    for (Machine_info& machine : machine_list)
    {
        DropDown::Item drop_item;
        ...
        drop_item.tip = wxString::FromUTF8(machine_type);
        drop_items.emplace_back(drop_item);
    }
    m_comboBox_printer->SetItems(drop_items);
    ...
    if (!is_from_sdcard_view) {
        preset_typename_normalized = NormalizeVendor(preset_typename);
        auto qds_dev = wxGetApp().qdsdevmanager->getSelectedDevice();
        for (int i = 0; i < machine_list.size(); ++i){
            Machine_info& machine = machine_list[i];
            if(preset_typename_normalized.find(NormalizeVendor(machine.type)) != std::string::npos){
                m_comboBox_printer->SetSelection(i);
                if (qds_dev && machine.ip == qds_dev->m_ip)
                    break;
        ...
        auto qdsdev = wxGetApp().qdsdevmanager;
        auto obj = qdsdev->getSelectedDevice();
        preset_typename_normalized = NormalizeVendor(obj->m_type);
        for (int i = 0; i < machine_list.size(); ++i) {
            Machine_info& machine = machine_list[i];
            if (preset_typename_normalized.find(NormalizeVendor(machine.type)) != std::string::npos) {
                m_comboBox_printer->SetSelection(i);
                BOOST_LOG_TRIVIAL(trace) << "Selected machine ip is " << machine.ip << std::endl;
                if (obj && machine.ip == obj->m_ip)
                    break;
            }
        }
    }
...
//y65
std::string PrinterInfoBox::NormalizeVendor(const std::string& str)
{
    std::string normalized;
    for (char c : str) {
        if (std::isalnum(c)) {
