...
#define SyncBoxInfoDialogWidth  FromDIP(675)
#define SyncBoxInfoDialogHeightMIN FromDIP(620)
#define SyncBoxInfoDialogHeightMIDDLE FromDIP(630)
#define SyncBoxInfoDialogHeightMAX FromDIP(660)
...
bool SyncBoxInfoDialog::Show(bool show)
{
    ...
    if (dirty_filament) {
        m_two_thumbnail_panel->Hide();
         ...
         m_confirm_title->SetLabel(_L("Synchronizing BOX filaments will discard your modified but unsaved filament presets.\nAre you sure you want to continue?"));
    ...
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << " SyncBoxInfoDialog finish show";
    return DPIDialog::Show(show);
}

void SyncBoxInfoDialog::updata_ui_data_after_connected_printer() {
...
void SyncBoxInfoDialog::update_select_layout(MachineObject *obj)
{
...
void SyncBoxInfoDialog::set_default_normal(const ThumbnailData &data)
{
...
bool SyncBoxInfoDialog::is_must_finish_slice_then_connected_printer() {
...
void SyncBoxInfoDialog::hide_no_use_controls() {
...
void SyncBoxInfoDialog::show_sizer(wxSizer *sizer, bool show)
{
    if (!sizer) { return; }
...
bool SyncBoxInfoDialog::get_is_double_extruder()
{
...
bool SyncBoxInfoDialog::is_dirty_filament() {
...
bool SyncBoxInfoDialog::is_need_show()
{
    //init begin
    m_result.direct_sync = true;
...
wxBoxSizer *SyncBoxInfoDialog::create_sizer_thumbnail(wxButton *image_button, bool left)
{
    auto sizer_thumbnail = new wxBoxSizer(wxVERTICAL);
    if (left) {
        wxBoxSizer *text_sizer = new wxBoxSizer(wxHORIZONTAL);
        auto        sync_text  = new Label(image_button->GetParent(), _CTX(L_CONTEXT("Original", "Sync_BOX"), "Sync_BOX"));
...
void SyncBoxInfoDialog::update_when_change_plate(int idx) {
    if (idx < 0) {
        return;
    }
...
void SyncBoxInfoDialog::update_when_change_map_mode(int idx)
{
    ...
    if (m_map_mode == MapModeEnum::ColorMap) {
        ...
        m_scrolledWindow->SetMinSize(wxSize(-1, SyncBoxInfoDialogHeightMAX));
        m_scrolledWindow->SetMaxSize(wxSize(-1, SyncBoxInfoDialogHeightMAX));
        ...
        m_scrolledWindow->SetMinSize(wxSize(-1, SyncBoxInfoDialogHeightMIN));
        m_scrolledWindow->SetMaxSize(wxSize(-1, SyncBoxInfoDialogHeightMIN));
    }
...
void SyncBoxInfoDialog::update_plate_combox()
{
    if (m_combobox_plate) {
...
wxColour SyncBoxInfoDialog::decode_ams_color(const std::string &color_str) {
...
void SyncBoxInfoDialog::update_map_when_change_map_mode()
{
    if (m_map_mode == MapModeEnum::ColorMap) {
...
void SyncBoxInfoDialog::update_when_change_map_mode(wxCommandEvent &e)
{
    int win_id = e.GetId();
...
void SyncBoxInfoDialog::update_panel_status(PageType page)
{
...
void SyncBoxInfoDialog::show_color_panel(bool flag, bool update_layout)
{
...
void SyncBoxInfoDialog::update_more_setting(bool layout, bool from_more_seting_text)
{
    if (!m_expand_more_settings) {
        ...
        if (from_more_seting_text) {
            m_scrolledWindow->SetMinSize(wxSize(-1, SyncBoxInfoDialogHeightMIDDLE));
            m_scrolledWindow->SetMaxSize(wxSize(-1, SyncBoxInfoDialogHeightMIDDLE));
        }
    } else {
...
void SyncBoxInfoDialog::init_bitmaps()
{
...
void SyncBoxInfoDialog::add_two_image_control()
{// thumbnail
    ...
    m_swipe_left_button->Bind(wxEVT_BUTTON, &SyncBoxInfoDialog::to_previous_plate, this);
    ...
    m_swipe_right_button->Bind(wxEVT_BUTTON, &SyncBoxInfoDialog::to_next_plate, this);
    ...
    wxStaticText *chose_combox_title = new wxStaticText(m_two_thumbnail_panel, wxID_ANY, _CTX(L_CONTEXT("Plate", "Sync_BOX"), "Sync_BOX"));
...
void SyncBoxInfoDialog::to_next_plate(wxCommandEvent &event) {
...
void SyncBoxInfoDialog::to_previous_plate(wxCommandEvent &event) {
    auto cobox_idx = m_combobox_plate->GetSelection();
    cobox_idx--;
    if (cobox_idx < 0) {
        return;
...
void SyncBoxInfoDialog::update_swipe_button_state()
{
...
SyncBoxInfoDialog::SyncBoxInfoDialog(wxWindow *parentm, SyncInfo& info) :
    DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY, _L("Synchronize BOX Filament Information"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX)
    , m_input_info(info)
    ...
    , m_mapping_tutorial_popup(AmsTutorialPopup(this))
{
    ...
    SetMinSize(wxSize(SyncBoxInfoDialogWidth, -1));
    SetMaxSize(wxSize(SyncBoxInfoDialogWidth, -1));

    // bind
    Bind(wxEVT_CLOSE_WINDOW, &SyncBoxInfoDialog::on_cancel, this);
    ...
    m_pages->SetSize(wxSize(SyncBoxInfoDialogWidth, -1));
    ...
    auto &bSizer = m_sizer_main;
        ...
        m_mode_combox_sizer->AddSpacer(SyncBoxInfoDialogWidth / 2.0f - FromDIP(8) / 2.0f - m_colormap_btn->GetSize().GetX());
        ...
        m_mode_combox_sizer->AddSpacer(SyncBoxInfoDialogWidth / 2.0f - FromDIP(8) / 2.0f - m_override_btn->GetSize().GetX() - FromDIP(60));
        ...
        m_colormap_btn->Bind(wxEVT_BUTTON, &SyncBoxInfoDialog::update_when_change_map_mode,this); // update_when_change_map_mode(e.GetSelection());
        m_override_btn->Bind(wxEVT_BUTTON, &SyncBoxInfoDialog::update_when_change_map_mode,this);
    ...
    auto option_use_ams = new PrintOption(m_options_other, _L("Use BOX"), wxEmptyString, ops_no_auto);
   ...
   // m_sizer_main->Add(m_basic_panel, 0, wxEXPAND | wxLEFT | wxRIGHT, FromDIP(15));
    ...
    {//new content//tip confirm ok button
        ...
        m_tip_attention_color_map = _L("Only synchronize filament type and color, not including Box slot information.");
        ...
        m_more_setting_tips->SetForegroundColour(wxColour(68, 121, 251));
        ...
        m_append_color_text                     = new wxStaticText(m_scrolledWindow, wxID_ANY, _L("Add unused BOX filaments to filaments list."));
        ...
        m_are_you_sure_title = new wxStaticText(m_scrolledWindow, wxID_ANY, _L("Are you sure to synchronize the filaments from Box?"));
        ...
        StateColor btn_bg_blue(std::pair<wxColour, int>(wxColour(0, 0, 121), StateColor::Pressed),
                                std::pair<wxColour, int>(wxColour(63, 111, 221), StateColor::Hovered),
...
void SyncBoxInfoDialog::reinit_dialog()
{
...
void SyncBoxInfoDialog::init_bind()
{
    Bind(wxEVT_TIMER, &SyncBoxInfoDialog::on_timer, this);
    Bind(EVT_CLEAR_IPADDRESS, &SyncBoxInfoDialog::clear_ip_address_config, this);
    Bind(EVT_UPDATE_USER_MACHINE_LIST, &SyncBoxInfoDialog::update_printer_combobox, this);
    Bind(EVT_PRINT_JOB_CANCEL, &SyncBoxInfoDialog::on_print_job_cancel, this);
    Bind(EVT_SET_FINISH_MAPPING, &SyncBoxInfoDialog::on_set_finish_mapping, this);
...
void SyncBoxInfoDialog::check_fcous_state(wxWindow *window)
{
    auto children = window->GetChildren();
    for (auto child : children) { check_fcous_state(child); }
}

void SyncBoxInfoDialog::popup_filament_backup()
{
    DeviceManager *dev = Slic3r::GUI::wxGetApp().getDeviceManager();
    if (!dev) return;
...
void SyncBoxInfoDialog::prepare_mode(bool refresh_button)
{
    // disable combobox
...
void SyncBoxInfoDialog::finish_mode()
{
...
void SyncBoxInfoDialog::sync_ams_mapping_result(std::vector<FilamentInfo> &result)
{
...
bool SyncBoxInfoDialog::do_ams_mapping()
{
    ...
    for (std::string color : box_colors) {
        if (!color.empty()) {
            color.erase(0, 1);
            color += "FF";
            auto box_color = AmsTray::decode_color(color);
            m_cur_colors_in_thumbnail.emplace_back(box_color);
        }
    }
...
void SyncBoxInfoDialog::deal_only_exist_ext_spool() {
    ...
    bool only_exist_ext_spool_flag = m_only_exist_ext_spool_flag = false;
    SetTitle(only_exist_ext_spool_flag ? _L("Synchronize Filament Information") : _L("Synchronize Box Filament Information"));
    m_append_color_text->SetLabel(only_exist_ext_spool_flag ? _L("Add unused filaments to filaments list.") :
                                                              _L("Add unused Box filaments to filaments list."));
    if (m_map_mode == MapModeEnum::ColorMap) {
        m_tip_attention_color_map = only_exist_ext_spool_flag ? _L("Only synchronize filament type and color, not including slot information.") :
                                                                _L("Only synchronize filament type and color, not including Box slot information.");
        m_tip_text->SetLabel(m_tip_attention_color_map);

    }
    if (m_ams_or_ext_text_in_colormap) {
        m_ams_or_ext_text_in_colormap->SetLabel((only_exist_ext_spool_flag ? _L("Ext spool") : _L("Box")) + ":");
    }
    if (m_ams_or_ext_text_in_override) {
        m_ams_or_ext_text_in_override->SetLabel((only_exist_ext_spool_flag ? _L("Ext spool") : _L("Box")) + ":");
    }
}

void SyncBoxInfoDialog::show_thumbnail_page()
{
    m_pages->ChangeSelection(1);
    m_pages->SetMinSize(wxSize(SyncBoxInfoDialogWidth, -1));
    m_pages->SetMaxSize(wxSize(SyncBoxInfoDialogWidth, -1));
}

bool SyncBoxInfoDialog::get_ams_mapping_result(std::string &mapping_array_str, std::string &mapping_array_str2, std::string &ams_mapping_info)
{
...
bool SyncBoxInfoDialog::build_nozzles_info(std::string &nozzles_info)
{
    /* init nozzles info */
...
bool SyncBoxInfoDialog::can_hybrid_mapping(ExtderData data)
{
...
void SyncBoxInfoDialog::auto_supply_with_ext(std::vector<AmsTray> slots)
{
...
bool SyncBoxInfoDialog::is_nozzle_type_match(ExtderData data, wxString &error_message) const
{
    if (data.total_extder_count <= 1 || data.extders.size() <= 1 || !wxGetApp().preset_bundle) return false;
...
int SyncBoxInfoDialog::convert_filament_map_nozzle_id_to_task_nozzle_id(int nozzle_id)
{
...
void SyncBoxInfoDialog::prepare(int print_plate_idx) { m_print_plate_idx = print_plate_idx; }

void SyncBoxInfoDialog::update_ams_status_msg(wxString msg, bool is_warning)
{
...
void SyncBoxInfoDialog::stripWhiteSpace(std::string &str)
{
    if (str == "") { return; }
...
wxString SyncBoxInfoDialog::format_text(wxString &m_msg)
{
...
void SyncBoxInfoDialog::update_priner_status_msg(wxString msg, bool is_warning)
{
    auto colour = is_warning ? wxColour(0xFF, 0x6F, 0x00) : wxColour(0x6B, 0x6B, 0x6B);
...
void SyncBoxInfoDialog::update_print_status_msg(wxString msg, bool is_warning, bool is_printer_msg)
{
    if (is_printer_msg) {
...
void SyncBoxInfoDialog::update_print_error_info(int code, std::string msg, std::string extra)
{
...
bool SyncBoxInfoDialog::has_tips(MachineObject *obj)
{
    if (!obj) return false;
...
void SyncBoxInfoDialog::show_status(PrintDialogStatus status, std::vector<wxString> params)
{
    ...
    if (status == PrintDialogStatus::PrintStatusInit) {
        ...
        wxString msg_text = _L("Please do not mix-use the Ext with BOX");
        update_print_status_msg(msg_text, true, false);
        ...
        wxString msg_text;
        if (params.size() > 1)
            msg_text = wxString::Format(_L("Filament %s does not match the filament in BOX slot %s. Please update the printer firmware to support BOX slot assignment."),
                                        params[0], params[1]);
        else
        ...
        wxString msg_text = _L("You selected external and BOX filament at the same time in an extruder, you will need manually change external filament.");
        update_print_status_msg(msg_text, true, false);
    }
...
void SyncBoxInfoDialog::init_timer()
{
...
bool SyncBoxInfoDialog::is_same_nozzle_diameters(NozzleType &tag_nozzle_type, float &nozzle_diameter)
{
...
bool SyncBoxInfoDialog::is_same_nozzle_type(std::string &filament_type, NozzleType &tag_nozzle_type)
{
...
bool SyncBoxInfoDialog::is_same_printer_model()
{
    bool           result = true;
    DeviceManager *dev    = Slic3r::GUI::wxGetApp().getDeviceManager();
...
void SyncBoxInfoDialog::show_errors(wxString &info)
{
...
wxString SyncBoxInfoDialog::format_steel_name(NozzleType type)
{
...
void SyncBoxInfoDialog::Enable_Auto_Refill(bool enable)
{
...
void SyncBoxInfoDialog::connect_printer_mqtt()
{
...
void SyncBoxInfoDialog::clear_ip_address_config(wxCommandEvent &e) { prepare_mode(); }

void SyncBoxInfoDialog::update_user_machine_list()
{
...
void SyncBoxInfoDialog::on_refresh(wxCommandEvent &event)
{
    if (m_is_empty_project) {
        return;
    }
...
void SyncBoxInfoDialog::on_print_job_cancel(wxCommandEvent &evt)
{
...
std::vector<std::string> SyncBoxInfoDialog::sort_string(std::vector<std::string> strArray)
{
...
bool SyncBoxInfoDialog::is_timeout()
{
    if (m_timeout_count > 15 * 1000 / LIST_REFRESH_INTERVAL) { return true; }
    return false;
}

int SyncBoxInfoDialog::update_print_required_data(
...
void SyncBoxInfoDialog::reset_timeout() { m_timeout_count = 0; }

void SyncBoxInfoDialog::update_user_printer()
{
...
void SyncBoxInfoDialog::update_printer_combobox(wxCommandEvent &event)
{
    show_status(PrintDialogStatus::PrintStatusInit);
    update_user_printer();
}
...
void SyncBoxInfoDialog::update_show_status()
{
    // refreshing return
...
bool SyncBoxInfoDialog::has_timelapse_warning()
{
    PartPlate *plate = m_plater->get_partplate_list().get_curr_plate();
    for (auto warning : plate->get_slice_result()->warnings) {
...
void SyncBoxInfoDialog::update_timelapse_enable_status()
{
    AppConfig *config = wxGetApp().app_config;
    if (!has_timelapse_warning()) {
        if (!config || config->get("print", "timelapse") == "0")
...
void SyncBoxInfoDialog::reset_ams_material()
{
    MaterialHash::iterator iter = m_materialList.begin();
    while (iter != m_materialList.end()) {
        int           id      = iter->first;
...
void SyncBoxInfoDialog::reset_all_ams_info()
{
    for (int i = 0; i < m_ams_mapping_result.size(); i++) {
...
void SyncBoxInfoDialog::reset_one_ams_material(const std::string &index_str, bool reset_to_first)
{
    MaterialHash::iterator iter = m_materialList.begin();
    while (iter != m_materialList.end()) {
        int       id      = iter->first;
...
void SyncBoxInfoDialog::on_dpi_changed(const wxRect &suggested_rect)
{
...
void SyncBoxInfoDialog::set_default(bool hide_some)
{
    if (m_print_type == PrintFromType::FROM_NORMAL) {
        bool is_show = true;
    } else if (m_print_type == PrintFromType::FROM_SDCARD_VIEW) {
...
void SyncBoxInfoDialog::reset_and_sync_ams_list()
{
    ...
    //zhushi: huo qu hao cai lie biao 
        ...
        if (contronal_index % SYNC_FLEX_GRID_COL == 0) {
            ...
            if (is_first_row) {
                ...
                if (!m_original_in_colormap) {
                    m_original_in_colormap = new wxStaticText(m_filament_panel, wxID_ANY, _CTX(L_CONTEXT("Original", "Sync_BOX"), "Sync_BOX") + ":");
                ...
                if (!m_ams_or_ext_text_in_colormap) {
                    m_ams_or_ext_text_in_colormap = new wxStaticText(m_filament_panel, wxID_ANY, _L("Box") + ":");
    ...
    if (use_double_extruder) {
        //m_filament_left_panel->Show();//SyncBoxInfoDialog::reset_and_sync_ams_list()
        //m_filament_right_panel->Show();
        m_filament_panel->Show(); // SyncBoxInfoDialog::reset_and_sync_ams_list()//special
        ...
        //m_filament_left_panel->Hide();//SyncBoxInfoDialog::reset_and_sync_ams_list()
        //m_filament_right_panel->Hide();
        m_filament_panel->Show();//SyncBoxInfoDialog::reset_and_sync_ams_list()
...
void SyncBoxInfoDialog::generate_override_fix_ams_list()
{
        ...
        if (contronal_index % SYNC_FLEX_GRID_COL == 0) {
            ...
            if (is_first_row) {
                ...
                if (!m_original_in_override) {
                    m_original_in_override = new wxStaticText(m_fix_filament_panel, wxID_ANY, _CTX(L_CONTEXT("Original", "Sync_BOX"), "Sync_BOX") + ":");
                ...
                if (!m_ams_or_ext_text_in_override) {
                    auto text = (m_only_exist_ext_spool_flag ? _L("Ext spool") : _L("Box")) + ":";
    ...
    if (m_preview_colors_in_thumbnail.empty()) {
        ...
        while (iter != m_materialList.end()) {
            ...
            if (item) {
                ...
                BOOST_LOG_TRIVIAL(error) << "check error:SyncBoxInfoDialog::clone_thumbnail_data:item is nullptr";
            }
            iter++;
        }
    }
...
void SyncBoxInfoDialog::record_edge_pixels_data()
{
...
wxColour SyncBoxInfoDialog::adjust_color_for_render(const wxColour &color)
{
...
void SyncBoxInfoDialog::final_deal_edge_pixels_data(ThumbnailData &data)
{
...
void SyncBoxInfoDialog::updata_thumbnail_data_after_connected_printer()
{
    updata_ui_data_after_connected_printer();
    update_thumbnail_data_accord_plate_index(true);
}

void SyncBoxInfoDialog::show_advanced_settings(bool flag, bool update_layout)
{
...
void SyncBoxInfoDialog::show_ams_controls(bool flag)
{
...
void SyncBoxInfoDialog::update_thumbnail_data_accord_plate_index(bool allow_clone_ams_color)
{
...
void SyncBoxInfoDialog::update_final_thumbnail_data() {
...
void SyncBoxInfoDialog::unify_deal_thumbnail_data(ThumbnailData &input_data, ThumbnailData &no_light_data, bool allow_clone_ams_color)
{
    if (input_data.width == 0 || input_data.height == 0 || no_light_data.width == 0 || no_light_data.height == 0) {
        BOOST_LOG_TRIVIAL(error) << "SyncBoxInfoDialog::no_light_data is empty,error";
        return;
    }
...
void SyncBoxInfoDialog::change_default_normal(int old_filament_id, wxColour temp_ams_color)
{
    if (m_cur_colors_in_thumbnail.size() == 0) {
        BOOST_LOG_TRIVIAL(error) << "SyncBoxInfoDialog::change_default_normal:error:m_cur_colors_in_thumbnail.size() == 0";
        return;
    }
    if (old_filament_id >= 0) {
        if (old_filament_id < m_cur_colors_in_thumbnail.size()) {
            ...
            BOOST_LOG_TRIVIAL(error) << "SyncBoxInfoDialog::change_default_normal:error:old_filament_id > m_cur_colors_in_thumbnail.size()";
            return;
        }
    }
    ...
    if (data.width > 0 && data.height > 0 && data.width == no_light_data.width && data.height == no_light_data.height) {
        ...
        BOOST_LOG_TRIVIAL(error) << "SyncBoxInfoDialog::change_defa:no_light_data is empty,error";
    }
}
...
void SyncBoxInfoDialog::set_info(SyncInfo &info)
{
    m_input_info = info;
}

void SyncBoxInfoDialog::update_lan_machine_list()
{
    ...
    BOOST_LOG_TRIVIAL(info) << "SyncBoxInfoDialog update_lan_devices end";
}

std::string SyncBoxInfoDialog::get_print_status_info(PrintDialogStatus status)
{
    switch (status) {
...
SyncNozzleAndBoxDialog::SyncNozzleAndBoxDialog(InputInfo &input_info)
    : BaseTransparentDPIFrame(static_cast<wxWindow *>(wxGetApp().mainframe),
                              320,
                              input_info.dialog_pos,
                              90,
                              _L("Successfully synchronized nozzle and BOX number information."),
...
SyncNozzleAndBoxDialog::~SyncNozzleAndBoxDialog() {}
...
void SyncNozzleAndBoxDialog::update_info(InputInfo &info) {
    m_input_info = info;
    restart();
    SetPosition(m_input_info.dialog_pos);
}

FinishSyncBoxDialog::FinishSyncBoxDialog(InputInfo &input_info)
    : BaseTransparentDPIFrame(static_cast<wxWindow *>(wxGetApp().mainframe),
                              310,
                              input_info.dialog_pos,
                              68,
                              _L("Successfully synchronized color and type of filament from printer."),
                              _CTX(L_CONTEXT("OK", "FinishSyncBox"), "FinishSyncBox"),
                              "",
                              DisappearanceMode::TimedDisappearance)
    , m_input_info(input_info)
{
...
FinishSyncBoxDialog::~FinishSyncBoxDialog() {}

void FinishSyncBoxDialog::deal_ok() {
    on_hide();
}

void FinishSyncBoxDialog::update_info(InputInfo &info)
{
    m_input_info = info;
    restart();
    SetPosition(m_input_info.dialog_pos);
    ...
    , m_plater(plater)
{
    ...
    m_switch_button->Bind(wxEVT_TOGGLEBUTTON, [this](wxCommandEvent& evt) {
        ...
        if (m_comboBox_printer->GetValue().empty()){
            ...
            m_button_sync->SetBackgroundColor(wxColour(0x90, 0x90, 0x90));
            m_button_sync->SetBorderColor(wxColour(0x90, 0x90, 0x90));
        }
        else {
            m_button_sync->Enable();
            m_button_sync->SetBackgroundColor(btn_bg_enable);
            m_button_sync->SetBorderColor(btn_bg_enable);
        }
    });
    ...
    m_sizer_printer->Add(switch_button_panel, 0, wxALL | wxLEFT, FromDIP(5));
    ...
    m_button_sync->SetCornerRadius(FromDIP(10));
    m_button_sync->Bind(wxEVT_BUTTON, &GetBoxInfoDialog::synchronization, this);

    m_button_cancel = new Button(this, _L("Cancel"));
    m_button_cancel->SetBackgroundColor(btn_bg_enable);
    m_button_cancel->SetBorderColor(btn_bg_enable);
    m_button_cancel->SetTextColor(StateColor::darkModeColorFor("#FFFFFE"));
    m_button_cancel->SetCornerRadius(FromDIP(10));
...
void GetBoxInfoDialog::generate_filament_id(GUI::Box_info& machine_filament_info)
{
    ...
    std::string box_id = wxGetApp().preset_bundle->printers.get_edited_preset().config.opt_string("box_id");
