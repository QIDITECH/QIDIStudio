...
// sub new
//QDS: GUI refactor, change tab to fit into ParamsPanel
...
/*#ifdef __WXOSX__
    ...
    if (m_type < Preset::TYPE_COUNT) {
        ...
        // m_presets_choice->SetFont(Label::Body_10); // QDS
    ...
    // QDS: open this tab by select first
    ...
    if (dynamic_cast<TabPrinter *>(this) || dynamic_cast<TabPrint *>(this)) {
        ...
        auto set_hover = [this, wiki_icon, wiki_icon_hover](bool hover) {
            wxColour color = hover ? wxColour("#4479fb") : wxColour("#6B6B6B");
        ...
        auto open_wiki = [](wxMouseEvent&) {
            wxLaunchDefaultBrowser("https://wiki.qidi3d.com/");
        };
...
{
    // QDS: restore all pages in preset
...
void Tab::on_roll_back_value(const bool to_sys /*= true*/)
{
    ...
    // QDS: restore all pages in preset, update_dirty also update combobox
    update_dirty();
}
...
void Tab::on_value_change(const std::string& opt_key, const boost::any& value)
{
    ...
    // QDS: 用户在 UI 上主动开启支撑时，联动开启识别悬空外墙（辅助体验，配合支撑可改善打印效果）。
    ...
    // QDS set support style to default when support type changes
...
void Tab::on_presets_changed()
{
    ...
    // y9
    bool is_qdt_vendor_preset = true;
...
void TabPrint::build()
{
    ...
    auto page = add_options_page(L("Quality"), "empty");
        ...
        optgroup->append_single_option_line("top_solid_infill_flow_ratio","print-settings/quality-advance-settings");
        optgroup->append_single_option_line("initial_layer_flow_ratio","print-settings/quality-advance-settings");
        optgroup->append_single_option_line("top_one_wall_type","print-settings/quality-advance-settings");
        optgroup->append_single_option_line("top_area_threshold","print-settings/quality-advance-settings");
        optgroup->append_single_option_line("only_one_wall_first_layer","print-settings/quality-advance-settings");
        optgroup->append_single_option_line("detect_overhang_wall","print-settings/quality-advance-settings");
        optgroup->append_single_option_line("smooth_speed_discontinuity_area","print-settings/quality-advance-settings");
    ...
    page = add_options_page(L("Strength"), "empty");
        ...
        optgroup->append_single_option_line("detect_floating_vertical_shell","print-settings/strength-advance-settings");
    ...
    page = add_options_page(L("Speed"), "empty");
        ...
        optgroup->append_single_option_line("resonance_avoidance", "print-settings/resonance-avoidance", 0);
    ...
    page = add_options_page(L("Support"), "support");
        ...
        optgroup->append_single_option_line("tree_support_branch_diameter_angle", "print-settings/support");
    ...
    page = add_options_page(L("Others"), "advanced");
        ...
        optgroup->append_single_option_line("prime_tower_skip_points", "print-settings/prime-tower");
        optgroup->append_single_option_line("prime_tower_enable_framework", "print-settings/prime-tower");
        optgroup->append_single_option_line("prime_tower_width","print-settings/prime-tower");
        optgroup->append_single_option_line("prime_tower_max_speed","print-settings/prime-tower");
        optgroup->append_single_option_line("prime_tower_brim_width","print-settings/prime-tower");
        optgroup->append_single_option_line("prime_tower_infill_gap","print-settings/prime-tower");
        optgroup->append_single_option_line("prime_tower_rib_wall", "print-settings/prime-tower");
        optgroup->append_single_option_line("prime_tower_extra_rib_length","print-settings/prime-tower");
        optgroup->append_single_option_line("prime_tower_rib_width","print-settings/prime-tower");
        optgroup->append_single_option_line("prime_tower_fillet_wall","print-settings/prime-tower");
...
void TabPrint::toggle_options()
{
    ...
    if (m_preset_bundle) {
        bool is_QDT_printer = m_preset_bundle->printers.get_edited_preset().is_qdt_vendor_preset(m_preset_bundle);
        m_config_manipulation.set_is_QDT_Printer(is_QDT_printer);
    }
...
void TabFilament::build()
{
    ...
    auto page = add_options_page(L("Filament"), "spool");
        ...
        //y58
        line = { L("Recommended box temperature"), L("Recommended box temperature range of this filament. 0 means no set") };
        line.append_option(optgroup->get_option("box_temperature_range_low"));
        line.append_option(optgroup->get_option("box_temperature_range_high"));
        optgroup->append_line(line);
        ...
        line = {L("QIDI Cool Plate SuperTack"), L("Bed temperature when cool plate is installed. Value 0 means the filament does not support to print on the QIDI Cool Plate SuperTack")};
        ...
        optgroup->m_on_change = [this, optgroup](t_config_option_key opt_key, boost::any value)
        {
            ...
            //y58
            else if (opt_key == "box_temperature") {
                m_config_manipulation.check_box_temperature(&filament_config);
            }
    ...
    page = add_options_page(L("Cooling"), "empty");
        ...
        optgroup->append_single_option_line("slow_down_min_speed","print-settings/auto-cooling", 0);
        ...
        optgroup->append_single_option_line("overhang_threshold_participating_cooling", "print-settings/auto-cooling");
        optgroup->append_single_option_line("overhang_fan_speed", "print-settings/auto-cooling");
        optgroup->append_single_option_line("pre_start_fan_time", "print-settings/auto-cooling");
        ...
        optgroup->append_line(line);
        //w13
        optgroup->append_single_option_line("additional_cooling_fan_speed_unseal");
...
void TabFilament::toggle_options()
{
    ...
    if (m_active_page->title() == "Filament")
    {
        ...
        //y58
        bool support_box_temp_control = this->m_preset_bundle->printers.get_edited_preset().config.opt_bool("support_box_temp_control");
        toggle_line("box_temperature", support_box_temp_control);
        for(auto el : {"box_temperature_range_high", "box_temperature_range_low"})
            toggle_line(el, support_box_temp_control);
    ...
    if (m_active_page && m_active_page->title() == "Multi Filament") {
        ...
        toggle_line("long_retractions_when_ec", is_multi_extruder && is_QDT_printer, 256 + extruder_idx);
        toggle_line("retraction_distances_when_ec", is_multi_extruder && is_QDT_printer && m_config->opt_bool_nullable("long_retractions_when_ec", extruder_idx), 256 + extruder_idx);
        m_active_page->update_visibility(m_mode, true);
        update_changed_ui();
    }
                ...
                if (opt_key == "thumbnail_size" && m_config->has("thumbnails_formats")) {
                    ...
                    if (!value.empty()) {
                        ...
                        if (errors != enum_bitmask<ThumbnailError>()) {
                            std::string error_str = format(_u8L("Invalid value provided for parameter %1%: %2%"), "thumbnail_size", val);
                            error_str += GCodeThumbnails::get_error_string(errors);
                            InfoDialog(parent(), _L("G-code flavor is switched"), from_u8(error_str)).ShowModal();
                        }

                        if (!thumbnails_list.empty()) {
                            GCodeThumbnailsFormat old_format = GCodeThumbnailsFormat(m_config->option("thumbnails_formats")->getInt());
                            GCodeThumbnailsFormat new_format = thumbnails_list.begin()->first;
                            if (old_format != new_format) {
                                DynamicPrintConfig new_conf = *m_config;

                                auto* opt = m_config->option("thumbnails_formats")->clone();
                                opt->setInt(int(new_format));
                                new_conf.set_key_value("thumbnails_formats", opt);
...
void TabPrinter::build_unregular_pages(bool from_initial_build/* = false*/)
{
    ...
    //QDS: please add our single extruder multimaterial parameters here. Currently
    ...
    // QDS. Just create one extruder page because QDT machine has only on physical extruder.
    ...
    // QDS. No extra extruder page for single physical extruder machine
    // # remove extra pages
#if 1
...
void TabPrinter::on_preset_loaded()
{
    ...
    if (base_name != m_base_preset_name) {
        ...
        if (base_model != m_base_preset_model) {
            ...
            wxGetApp().plater()->get_notification_manager()->remove_notification_of_type(NotificationType::QDTArcFittingInfo);
        }
...
void TabPrinter::toggle_options()
{
    ...
    if (m_active_page->title() == "Basic information") {
        //toggle_line("printable_area", !is_configed_by_QDT);//all printer can entry and view data
        toggle_option("single_extruder_multi_material", have_multiple_extruders);
        //QDS: gcode_flavore of QDT printer can't be edited and changed
        toggle_option("gcode_flavor", !is_QDT_printer);
        toggle_option("thumbnail_size",!is_QDT_printer);
        toggle_option("printer_structure", !is_QDT_printer);
        toggle_option("use_relative_e_distances", !is_QDT_printer);
        toggle_option("support_chamber_temp_control",!is_QDT_printer);
        //y58
        ...
        toggle_option("print_in_clockwise", !is_QDT_printer);
        auto flavor = m_config->option<ConfigOptionEnum<GCodeFlavor>>("gcode_flavor")->value;
        ...
        val > 0 && (size_t)val <= m_extruders_count))
    {
        ...
        for (auto el : { "extruder_type" , "nozzle_diameter"}) {
            toggle_option(el, !is_QDT_printer, i);
        }

        toggle_option("extruder_type", !is_QDT_printer, i);
        toggle_option("nozzle_diameter", !is_QDT_printer || config_mode == ConfigOptionMode::comDevelop, i);
        toggle_option("extruder_offset", !is_QDT_printer || config_mode == ConfigOptionMode::comDevelop, i);
    ...
    if (m_active_page->title() == "Motion ability") {
        ...
        //QDS: limits of QDT printer can't be edited, except jerk.
...
//QDS: reactive the preset combo box
void Tab::reactive_preset_combo_box()
{
    if (!m_presets_choice) return;
    //QDS: add workaround to fix the issue caused by wxwidget 3.15 upgrading
...
void Tab::rebuild_page_tree()
{
    ...
    // QDS: fix new layout, record last select
    if (sel_item < 0)
    ...
    // QDS: on mac, root is selected, this fix it
    m_tabctrl->Unselect();
    // QDS: not select on hide tab
    ...
    std::string preset_name, bool delete_current /*=false*/, const std::string &last_selected_ph_printer_name /* =""*/, bool force_select, bool force_no_transfer)
{
    ...
    if (preset_name.empty()) {
        if (delete_current) {
            ...
            //QDS select first visible item first
    ...
    //QDS: add project embedded preset logic and refine is_external
...
//QDS: GUI refactor: unselect current item
...
bool Tab::update_current_page_in_background(int& item)
{
    ...
    if (!active_tab || (!m_parent->is_active_and_shown_tab((wxPanel*)this)))
    {
        ...
        // clear pages from the controlls
        // QDS: fix after new layout, clear page in backgroud
        for (auto p : m_pages)
            p->clear();
        ...
        // QDS: this is not used, because we not SelectItem in background
...
//!	m_tabctrl->OnSetFocus();
    ...
    //QDS record current preset name
    ...
    {
        bool is_configed_by_QDT =  wxGetApp().plater()->is_preset_configed_by_QDT(curr_preset);
        if (is_configed_by_QDT) {//only record svg
            if (wxGetApp().app_config->has_section("user_qdt_svg_list")) {
                ...
                if (user_qdt_svg_list.size() > 0 && user_qdt_svg_list[curr_preset_name].size() > 0) {
                    extra_map["bed_custom_texture"] = ConfigOptionString(user_qdt_svg_list[curr_preset_name]);
                }
            }
        }
    ...
    if (exist_preset) {
        ...
        if (wxGetApp().is_user_login())
            new_preset->user_id = wxGetApp().get_current_user_id();
    ...
    //QDS if create a new prset name, preset changed from preset name to new preset name
    if (!exist_preset) {
...
wxSizer* TabPrinter::create_bed_shape_widget(wxWindow* parent)
{
    ...
    btn->Bind(wxEVT_BUTTON, ([this](wxCommandEvent e) {
            bool  is_configed_by_QDT = PresetUtils::system_printer_bed_model(m_preset_bundle->printers.get_edited_preset()).size() > 0;
            ConfigOptionString custom_texture     = *m_config->option<ConfigOptionString>("bed_custom_texture");
            PresetBundle &     preset_bundle      = *wxGetApp().preset_bundle;
            auto               cur_preset_name    = preset_bundle.printers.get_edited_preset().name;
            if (is_configed_by_QDT && wxGetApp().app_config->has_section("user_qdt_svg_list")) {
                auto user_qdt_svg_list = wxGetApp().app_config->get_section("user_qdt_svg_list");
                if (user_qdt_svg_list.size() > 0 && user_qdt_svg_list[cur_preset_name].size() > 0) {
                    custom_texture = ConfigOptionString(user_qdt_svg_list[cur_preset_name]);
                }
            }
            BedShapeDialog dlg(this);
            dlg.build_dialog(*m_config->option<ConfigOptionPoints>("printable_area"), custom_texture,
                             *m_config->option<ConfigOptionString>("bed_custom_model") , !is_configed_by_QDT);
            if (dlg.ShowModal() == wxID_OK) {
                if (dlg.get_valid()) {
                    std::string custom_texture = dlg.get_custom_texture();
                    if (is_configed_by_QDT) {
                        {//save to user_qdt_svg_list
                            if (!wxGetApp().app_config->has_section("user_qdt_svg_list")) {
                                std::map<std::string, std::string> data;
                                data[cur_preset_name] = custom_texture;
                                wxGetApp().app_config->set_section("user_qdt_svg_list", data);
                            } else {
                                auto data                       = wxGetApp().app_config->get_section("user_qdt_svg_list");
                                auto data_modify                = const_cast<std::map<std::string, std::string> *>(&data);
                                (*data_modify)[cur_preset_name] = custom_texture;
                                wxGetApp().app_config->set_section("user_qdt_svg_list", *data_modify);
                            }
                        }
...
void TabPrinter::cache_extruder_cnt()
{
    ...
    // QDS. Get extruder count from preset instead of m_extruders_count.
...
void Tab::update_nozzle_status_display()
{
    ...
    reminder_text->SetFont(Label::Body_13);
    reminder_text->SetForegroundColour(wxColour("#4479fb"));
    ...
    auto create_nozzle_button = [this](const wxString &name) {
        ...
        StateColor fg_color(wxColour("#4479fb"));
...
void Page::update_visibility(ConfigOptionMode mode, bool update_contolls_visibility)
{
    ...
    // QDS: no line spliter for first group
    bool first = true;
#endif
    for (auto group : m_optgroups) {
...
#ifdef __WXMSW__
    if (!m_show) return;
    // QDS: fix field control position
    wxTheApp->CallAfter([this]() {
        for (auto group : m_optgroups) {
