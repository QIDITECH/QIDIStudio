...
static constexpr const char* DEFAULT_VIRTUAL_FILAMENT_NAME = "QIDI PLA Basic";
...
static int draw_brand_icon_and_strip(wxDC& dc, wxWindow* win, wxString& name, int x, int cy)
{
    ...
    if (name.StartsWith("QIDI ")) {
        name = name.Mid(6);
        wxBitmap bmp = create_scaled_bitmap("QIDIStudioBlack", win, 16);
        if (bmp.IsOk())
