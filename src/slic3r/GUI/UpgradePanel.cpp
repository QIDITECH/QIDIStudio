...
static const wxColour TEXT_NORMAL_CLR = wxColour(68, 121, 251);
    ...
    :wxPanel(parent, id, pos, size, style)
{
    ...
    StateColor btn_bg(std::pair<wxColour, int>(wxColour(255, 255, 255), StateColor::Disabled), std::pair<wxColour, int>(wxColour(0, 66, 255), StateColor::Pressed),
                      std::pair<wxColour, int>(wxColour(116, 168, 255), StateColor::Hovered), std::pair<wxColour, int>(wxColour(68, 121, 251), StateColor::Enabled),
...
void MachineInfoPanel::init_bitmaps()
{
    try {
        ...
        m_img_monitor_ams = ScalableBitmap(this, "monitor_upgrade_box_png", 150);
...
void MachineInfoPanel::update_ams_ext(MachineObject *obj)
{
           ...
           for (auto iter_ams = ver_list.cbegin(); iter_ams != ver_list.cend(); iter_ams++) {
              ...
              /*  auto it = ver_list.find(atoi(iter->first.c_str()));
                ...
                else
                {
                     ...
                     wxString ams_device_name = "Box-%s";
    ...
    if (contain_four_slot) {
        if (m_img_monitor_ams.name() != "monitor_upgrade_box") {
            m_img_monitor_ams = ScalableBitmap(this, "monitor_upgrade_box_png", 150);
            m_ams_img->SetBitmap(m_img_monitor_ams.bmp());
        }
