...
int GuideFrame::LoadProfile()
{
    try {
        ...
        //handle = _findfirst((qdt_bundle_path / "*.json").make_preferred().string().c_str(), &findData); // 查找目录中的第一个文件
        // if (handle == -1) { return -1; }
        ...
        for (boost::filesystem::directory_iterator iter(myPath); iter != endIter; iter++) {
            if (boost::filesystem::is_directory(*iter)) {
                ...
                if (w2s(strVendor) == PresetBundle::QDT_BUNDLE && strExtension.CmpNoCase("json") == 0)
        ...
        //LoadProfileFamily(PresetBundle::QDT_BUNDLE, qdt_bundle_path.string());
