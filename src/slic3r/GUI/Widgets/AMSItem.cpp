    ...
    for (int i = 0; i < ams->GetTrays().size(); i++) {
        ...
        if (it != ams->GetTrays().end() && it->second->is_exists) {
            if (it->second->is_tray_info_ready()) {
                ...
                if (DevFilaSystem::IsQDT_Filament(it->second->tag_uid)) {
                    info.material_state = AMSCanType::AMS_CAN_TYPE_BRAND;
                } else {
                    info.material_state = AMSCanType::AMS_CAN_TYPE_THIRDBRAND;
                }

                if (!DevFilaSystem::IsQDT_Filament(it->second->tag_uid) || !remain_flag) {
...
static wxColour _get_diff_clr(wxWindow *win, wxColour pen_clr) /*STUDIO-10093 get diff color*/
{
    if (!wxGetApp().dark_mode())
    {
        if (pen_clr.GetRGB() == wxColour("#FAFAFA").GetRGB())
        {
    ...
    : AMSHumidity()
{
    ...
    ams_drying_img = ScalableBitmap(this, "dev_box_dry_ctr_heating_icon", 16);
...
void AmsItem::AddCan(Caninfo caninfo, int canindex, int maxcan, wxBoxSizer* sizer)
{
    ...
    if (m_ams_model != DevAmsType::EXT_SPOOL)
    {
        ...
        //cj_4
        // QDS box slots should always show the refresh/RFID control, including
        // empty slots. Legacy AMS keeps the old empty-slot disabled behavior.
        const bool disable_refresh = (caninfo.material_state == AMSCanType::AMS_CAN_TYPE_EMPTY);
        m_panel_refresh->setDisableMode(disable_refresh);
...
void AmsItem::Update(AMSinfo info)
{
    ...
    for (int i = 0; i < m_can_count; i++) {
        //cj_4
        // Use the can id carried by the current data rather than assuming
        // contiguous map keys. QDS used to pass global can ids here, which
        // made later boxes miss updates because the controls are keyed by
        // local slot ids.
        auto it = m_can_refresh_list.find(info.cans[i].can_id);
        if (it == m_can_refresh_list.end()) continue;
    ...
    for (int i = 0; i < m_can_count; i++) {
        //cj_4
        auto it = m_can_lib_list.find(info.cans[i].can_id);
        if (it == m_can_lib_list.end()) continue;

        AMSLib* lib = it->second;
        if (lib != nullptr) {
