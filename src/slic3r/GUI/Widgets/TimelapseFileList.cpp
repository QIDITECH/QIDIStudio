...
// 与 DeviceModelList::file_list_primary_text_colour 一致（列头文字）
...
// 列表行/表头底部分割条高度（与原先 line_reserve 一致）
    ...
    , m_modified_text(modified_text)
{
    ...
    //cj_3 Checkbox column matches DeviceModelItem (stacked cancel + FileListBitmapCheckBox).
    ...
    wxBoxSizer* mts_v = new wxBoxSizer(wxVERTICAL);
    wxBoxSizer* mts_h = new wxBoxSizer(wxHORIZONTAL);
    ...
    //cj_3
    // Fixed-width meta bundle (like DeviceModelItem weight+time); no stretch — avoids inflated virtual width / horizontal scrollbar.
...
//cj_3 Align with DeviceModelItem::sync_row_surface_colours.
void TimelapseFileItem::sync_row_surface_colours(const wxColour& bg, const wxColour& fg)
{
...
wxPanel* TimelapseFileListCtrl::CreateHeaderPanel()
{
    ...
    //cj_3 Same list background as DeviceModelListCtrl::CreateHeaderPanel (theme-aware).
    ...
    wxPanel* metaHeaderWrap = new wxPanel(headerPanel);
    metaHeaderWrap->SetBackgroundColour(header_bg);
    wxStaticText* mtimeHeader = new wxStaticText(metaHeaderWrap, wxID_ANY, _L("Last modified"));
    ...
    wxStaticText* sizeHeader = new wxStaticText(metaHeaderWrap, wxID_ANY, _L("File size"));
    ...
    wxBoxSizer* metaHdrVBox = new wxBoxSizer(wxVERTICAL);
    wxBoxSizer* metaHdrHBox = new wxBoxSizer(wxHORIZONTAL);
