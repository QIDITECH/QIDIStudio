...
,m_matrix(matrix), m_min_flush_volume(extra_flush_volume), m_max_flush_volume(Slic3r::g_max_flush_volume)
{
    ...
    {
        wxString message = _L("Studio would re-calculate your flushing volumes everytime the filaments color changed or filaments changed. You could disable the auto-calculate in QIDI Studio > Preferences");
...
void WipingPanel::toggle_advanced(bool user_action) {
    if (user_action)
        ...
        // QDS: show advanced mode by default
