...
ScalableBitmap::ScalableBitmap( wxWindow *parent,
    ...
    m_px_cnt(px_cnt), m_grayscale(grayscale), m_resize(resize) // QDS: support resize by fill border
{
...
void ScalableButton::msw_rescale()
{
    ...
    if (!m_current_icon_name.empty()) {
        wxBitmap bmp = create_scaled_bitmap(m_current_icon_name, m_parent, m_px_cnt);
        SetBitmap(bmp);
        // QDS: why disappear on hover? why current HBITMAP differ from other 
        //SetBitmapCurrent(bmp);
        //SetBitmapPressed(bmp);
        //SetBitmapFocus(bmp);
