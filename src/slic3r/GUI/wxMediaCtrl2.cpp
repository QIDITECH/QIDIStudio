...
#define CLSID_QIDI_SOURCE L"{233E64FB-2041-4A6C-AFAB-FF9BCF83E7AA}"
...
void wxMediaCtrl2::Load(wxURI url)
{
    ...
    {
        wxRegKey key11(wxRegKey::HKCU, L"SOFTWARE\\Classes\\CLSID\\" CLSID_QIDI_SOURCE L"\\InProcServer32");
        wxRegKey key12(wxRegKey::HKCR, L"CLSID\\" CLSID_QIDI_SOURCE L"\\InProcServer32");
        wxString path = key11.Exists() ? key11.QueryDefaultValue() 
                                       : key12.Exists() ? key12.QueryDefaultValue() : wxString{};
        wxRegKey key2(wxRegKey::HKCR, "qidi");
        wxString clsid;
        if (key2.Exists())
        ...
        auto                    dll_path = data_dir_path / "plugins" / "QIDISource.dll";

        if (path.empty() || !wxFile::Exists(path) || clsid != CLSID_QIDI_SOURCE) {
            if (boost::filesystem::exists(dll_path)) {
                CallAfter(
                    [dll_path] {
                    int res = wxMessageBox(_L("QIDISource has not correctly been registered for media playing! Press Yes to re-register it."), _L("Error"), wxYES_NO | wxICON_ERROR);
                    if (res == wxYES) {
                ...
                CallAfter([] {
                    wxMessageBox(_L("Missing QIDISource component registered for media playing! Please re-install QIDIStutio or seek after-sales help."), _L("Error"), wxOK | wxICON_ERROR);
                });
            }
            m_error = clsid != CLSID_QIDI_SOURCE ? 101 : path.empty() ? 102 : 103;
            wxMediaEvent event(wxEVT_MEDIA_STATECHANGED);
            event.SetId(GetId());
            event.SetEventObject(this);
            wxPostEvent(this, event);
        ...
        if (path != dll_path) {
            ...
            if (!notified) CallAfter([dll_path] {
                int res = wxMessageBox(_L("Using a QIDISource from a different install, video play may not work correctly! Press Yes to fix it."), _L("Warning"), wxYES_NO | wxICON_WARNING);
                if (res == wxYES) {
                    auto path = dll_path.wstring();
                    if (path.find(L' ') != std::wstring::npos)
                        path = L"\"" + path + L"\"";
    ...
    if (!hasplugins) {
        CallAfter([] {
            wxMessageBox(_L("Your system is missing H.264 codecs for GStreamer, which are required to play video.  (Try installing the gstreamer1.0-plugins-bad or gstreamer1.0-libav packages, then restart QIDI Studio?)"), _L("Error"), wxOK);
        });
        m_error = 101;
        wxMediaEvent event(wxEVT_MEDIA_STATECHANGED);
        event.SetId(GetId());
