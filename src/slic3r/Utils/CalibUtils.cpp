...
void CalibUtils::calib_pa_pattern(const MachineObject *obj, const CalibInfo &calib_info, Model& model, DynamicPrintConfig& full_config)
{
    ...
    if (calib_info.extruder_type == ExtruderType::etBowden)
        pa_pattern.set_qdt_bowden_mode();
...
bool CalibUtils::process_and_store_3mf(Model *model, const DynamicPrintConfig &full_config, const Calib_Params &params, wxString &error_message)
{
    ...
    bool success = Slic3r::store_qds_3mf(store_params);
...
void CalibUtils::send_to_print(const CalibInfo &calib_info, wxString &error_message, int flow_ratio_mode)
{
    ...
    print_job->set_print_config(MachineBedTypeString[bed_type], true, false, false, false, true, false, 0, 0, 0, 0, nozzle_offset_cali, 0);
...
void CalibUtils::send_to_print(const std::vector<CalibInfo> &calib_infos, wxString &error_message, int flow_ratio_mode)
{
    ...
    print_job->set_print_config(MachineBedTypeString[bed_type], true, true, false, false, true, false, 0, 1, 0, 0, nozzle_offset_cali, 0);
    { // after send: record the print job
        json j;
        j["print"]["ams_mapping"] = print_job->task_ams_mapping;
        j["print"]["ams_mapping_2"] = print_job->task_ams_mapping2;
