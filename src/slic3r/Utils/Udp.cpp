...
struct DnsRR_TXT
{
	...
	UdpReply::TxtData data;

	static optional<DnsRR_TXT> decode(const DnsResource &rr, const Udp::TxtKeys &txt_keys)
	{
...
struct DnsUdpMessage
{
	...
	static optional<DnsUdpMessage> decode(const std::vector<char>& buffer, const Udp::TxtKeys& txt_keys)
	{
...
private:
	void parse_rr(const std::vector<char>& buffer, DnsResource&& rr, size_t dataoffset, const Udp::TxtKeys& txt_keys)
	{
		switch (rr.type) {
		case DnsRR_A::TAG:
...
std::ostream& operator<<(std::ostream &os, const DnsUdpMessage &msg)
{
	os << boost::format("DnsUdpMessage(ID: %1%, Q: %2%, A: %3%, AAAA: %4%, services: [")
		% msg.header.id
...
const asio::ip::address_v4 UdpRequest::MCAST_IP4{ 0xffffffff };
const asio::ip::address_v6 UdpRequest::MCAST_IP6 = asio::ip::make_address_v6("ff02::fb");
const uint16_t UdpRequest::MCAST_PORT = 8989;

optional<UdpRequest> UdpRequest::make_PTR(const std::string &service, const std::string &protocol)
{
	...
	return UdpRequest(std::move(data));
}

optional<UdpRequest> UdpRequest::make_A(const std::string& hostname)
{
...
optional<UdpRequest> UdpRequest::make_AAAA(const std::string& hostname)
{
	// todo: why is this and what is real max
	if (hostname.size() > 30) {
		return boost::none;
...
{
	buffer.resize(DnsUdpMessage::MAX_SIZE);
}

UdpUdpSocket::UdpUdpSocket( Udp::ReplyFn replyfn, const asio::ip::address& multicast_address, const asio::ip::address& interface_address, std::shared_ptr< boost::asio::io_service > io_service)
	: replyfn(replyfn)
	...
	, io_service(io_service)
{
	try {
        ...
        mcast_endpoint = udp::endpoint(boost::asio::ip::address_v4::broadcast(), UdpRequest::MCAST_PORT);
		// open socket
		//boost::asio::ip::udp::endpoint listen_endpoint(multicast_address.is_v4() ? udp::v4() : udp::v6(), UdpRequest::MCAST_PORT);
  ...
  //      socket.set_option(boost::asio::socket_base::broadcast(true));
		...
		//}
		//mcast_endpoint = udp::endpoint(multicast_address, UdpRequest::MCAST_PORT);
        //mcast_endpoint = udp::endpoint(boost::asio::ip::address_v4::broadcast(), UdpRequest::MCAST_PORT);
        BOOST_LOG_TRIVIAL(error) << "Socket created. Multicast: " << multicast_address << ". Interface: " << interface_address;
...
UdpUdpSocket::UdpUdpSocket( Udp::ReplyFn replyfn, const asio::ip::address& multicast_address, std::shared_ptr< boost::asio::io_service > io_service)
	: replyfn(replyfn)
	, multicast_address(multicast_address)
	, socket(*io_service)
	, io_service(io_service)
...
void UdpUdpSocket::async_receive()
{
	try {
		// our session to hold the buffer + endpoint
		auto session = create_session();

        socket.async_receive_from(boost::asio::buffer(m_recvBuf)
			, session->remote_endpoint
			, boost::bind(&UdpUdpSocket::receive_handler, this, session, asio::placeholders::error, asio::placeholders::bytes_transferred));
	}
	catch (std::exception& e) {
		BOOST_LOG_TRIVIAL(error) << e.what();
	}
...
void UdpUdpSocket::receive_handler(SharedUdpSession session, const boost::system::error_code& error, size_t bytes)
{
    ...
    //asio::ip::address ip ;
    //BOOST_LOG_TRIVIAL(error) << "\nIP :" << pData.substr(pData.find_last_of(",") + 1)
    //                         << "NAME :" << pData.substr(pData.find("mkswifi:") + 8, pData.find(",") - 8);
    //BOOST_LOG_TRIVIAL(error) << "pData2 :" << pData.substr(pData.find("mkswifi:") + 8, pData.find(",") - 8);
    //ip = pData.substr(pData.data().find_last_of(",") + 1));
    //list->SetItem(item, 1, get_reply[n].substr(get_reply[n].find("mkswifi:") + 8, get_reply[n].find(",") - 8));
    //UdpReply reply(ip, srv.port, std::move(service_name), srv.hostname, std::move(txt_data));
    //replyfn(std::move(reply));
    //UdpReply::TxtData txt_data;
    //asio::ip::address addr = asio::ip::address::from_string(pData.substr(pData.find_last_of(",") + 1));
    //UdpReply          reply(addr, 0, std::move(pData.substr(pData.find("mkswifi:") + 8, pData.find(",") - 8)),
    //                   pData.substr(pData.find("mkswifi:") + 8, pData.find(",") - 8), std::move(txt_data));
    //replyfn(std::move(reply));
	async_receive();
}

SharedUdpSession LookupUdpSocket::create_session() const { return std::shared_ptr<LookupUdpSession>(new LookupUdpSession(this, replyfn)); }
...
void LookupUdpSession::handle_receive(const error_code &error, size_t bytes, std::string pData)
{
	...
	//buffer.resize(bytes);
	//auto dns_msg = DnsUdpMessage::decode(buffer, socket->get_txt_keys());
	//if (dns_msg) {
	...
	//		UdpReply reply(ip, srv.port, std::move(service_name), srv.hostname);
	//		replyfn(std::move(reply));
	//	}
	//}
}

SharedUdpSession ResolveUdpUdpSocket::create_session() const 
{ 
	return std::shared_ptr< ResolveUdpSession > (new ResolveUdpSession(this, replyfn));
}


void ResolveUdpSession::handle_receive(const error_code &error, size_t bytes, std::string pData)
{
	assert(socket);
	if (error) {
	...
	auto dns_msg = DnsUdpMessage::decode(buffer, Udp::TxtKeys());
	if (dns_msg) {
		asio::ip::address ip;
		...
		if (!answer_name.empty()) {
			...
			if (name_tolower == hostname_tolower) {
				UdpReply reply(ip, 0, std::string(), answer_name);
				replyfn(std::move(reply));
			}
		}
	}
...
//	std::vector<UdpReply> replies;
	...
	Udp::ReplyFn replyfn;
	Udp::CompleteFn completefn;
	Udp::ResolveFn resolvefn;
...
Udp::priv::priv(std::string&& service)
...
void Udp::priv::lookup_perform()
{
	service_dn = (boost::format("_%1%._%2%.local") % service % protocol).str();

	std::shared_ptr< boost::asio::io_service > io_service(new boost::asio::io_service);

	std::vector<LookupUdpSocket*> sockets;
	...
	if (!ec) {	
		...
		for (const auto& intrfc : interfaces) 		
			sockets.emplace_back(new LookupUdpSocket(txt_keys, service, service_dn, protocol, replyfn, UdpRequest::MCAST_IP4, intrfc, io_service));
	} else {
		BOOST_LOG_TRIVIAL(info) << "Failed to resolve ipv4 interfaces: " << ec.message();
	}
	if (sockets.empty())
		sockets.emplace_back(new LookupUdpSocket(txt_keys, service, service_dn, protocol, replyfn, UdpRequest::MCAST_IP4, io_service));
	// ipv6 interfaces
	interfaces.clear();
	...
	if (!ec)
	{
		...
		// each will send to querry to for both ipv4 and ipv6
		for (const auto& intrfc : interfaces)
			sockets.emplace_back(new LookupUdpSocket(txt_keys, service, service_dn, protocol, replyfn, UdpRequest::MCAST_IP6, intrfc, io_service));
		if (interfaces.empty())
			sockets.emplace_back(new LookupUdpSocket(txt_keys, service, service_dn, protocol, replyfn, UdpRequest::MCAST_IP6, io_service));
	} else {
...
void Udp::priv::resolve_perform()
{
	// reply callback is shared to every UdpUdpSession which is called on same thread as io_service->run();
	...
	const auto reply_callback = [&rpls = replies](UdpReply&& reply)
	{
	...
	std::shared_ptr< boost::asio::io_service > io_service(new boost::asio::io_service);
	std::vector<ResolveUdpUdpSocket*> sockets;
	...
	if (!ec) {
		...
		// each will send to querry to for both ipv4 and ipv6
		for (const auto& intrfc : interfaces)
			sockets.emplace_back(new ResolveUdpUdpSocket(hostname, reply_callback, UdpRequest::MCAST_IP4, intrfc, io_service));
	} else {
		BOOST_LOG_TRIVIAL(info) << "Failed to resolve ipv4 interfaces: " << ec.message();
	}
	if (sockets.empty())
		sockets.emplace_back(new ResolveUdpUdpSocket(hostname, reply_callback, UdpRequest::MCAST_IP4, io_service));
	...
	if (!ec) {
		...
		// each will send to querry to for both ipv4 and ipv6
		for (const auto& intrfc : interfaces) 
			sockets.emplace_back(new ResolveUdpUdpSocket(hostname, reply_callback, UdpRequest::MCAST_IP6, intrfc, io_service));
		if (interfaces.empty())
			sockets.emplace_back(new ResolveUdpUdpSocket(hostname, reply_callback, UdpRequest::MCAST_IP6, io_service));
	} else {
		BOOST_LOG_TRIVIAL(info) << "Failed to resolve ipv6 interfaces: " << ec.message();
	}
...
std::string UdpReply::path() const
{
    const auto it = "11";
    return it;
}

bool UdpReply::operator==(const UdpReply &other) const
{
...
std::ostream& operator<<(std::ostream &os, const UdpReply &reply)
{
	os << boost::format("UdpReply(%1%, %2%, %3%, %4%")
		% reply.ip.to_string()
		% reply.service_name
		% reply.hostname
		% reply.full_address;
...
Udp::Udp(std::string service)
	: p(new priv(std::move(service)))
{}

Udp::Udp(Udp &&other) : p(std::move(other.p)) {}
...
Udp& Udp::set_protocol(std::string protocol)
{
	if (p) { p->protocol = std::move(protocol); }
	return *this;
}

Udp& Udp::set_txt_keys(TxtKeys txt_keys)
{
	if (p) { p->txt_keys = std::move(txt_keys); }
	return *this;
}

Udp& Udp::set_timeout(unsigned timeout)
{
	if (p) { p->timeout = timeout; }
	return *this;
}

Udp& Udp::set_hostname(const std::string& hostname)
{
	if (p) { p->hostname = hostname; }
	return *this;
}

Udp& Udp::set_retries(unsigned retries)
{
	if (p && retries > 0) { p->retries = retries; }
	return *this;
}

Udp& Udp::on_udp_reply(ReplyFn fn)
{
	if (p) { p->replyfn = std::move(fn); }
	return *this;
}

Udp& Udp::on_complete(CompleteFn fn)
{
	if (p) { p->completefn = std::move(fn); }
	return *this;
}

Udp& Udp::on_resolve(ResolveFn fn)
{
...
Udp::Ptr Udp::lookup()
{
	auto self = std::make_shared<Udp>(std::move(*this));
