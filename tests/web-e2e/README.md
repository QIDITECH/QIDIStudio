# QIDIStudio Filament Manager · Web E2E

End-to-end tests for the QIDIStudio filament manager **webview**, driven
...
> **not** drive Win32 menus or the QIDIStudio main window — the
...
The whole framework hinges on QIDIStudio honouring the
...
| 2    | qidi-studio.exe not found | set `STUDIO_E2E_BIN` in `.env.local` |
| 3    | port already in use | pick a free `STUDIO_E2E_CDP_PORT` |
| 4    | port silent | QIDIStudio overrides the env var; needs a 1-line C++ patch in the webview creator (see `docs/cdp-fallback.md` if/when added) |
...
│   ├── helpers/
│   │   ├── studio-launcher.ts    spawns qidi-studio.exe with env var
...
| symptom | diagnosis |
| ------- | --------- |
| `chromium.connectOverCDP failed` | QIDIStudio not running, or port blocked / already taken |
