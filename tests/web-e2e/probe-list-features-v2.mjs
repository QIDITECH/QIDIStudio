...
//        - preview-color-name  reflects the official QDT colour name when
//          the AMS slot encodes a single-hex SKU that exists in the official
//          PLA Silk / PLA Basic / etc. library
//        - preview-fila-code   shows the QDT code (e.g. "11400") for the
//          same single-hex SKU
//        - For multi-color slots, the preview swatch becomes a strip /
//          gradient and preview-color-name still resolves to the QDT
...
if (trackB.dialogVisible) {
  ...
  for (const slot of slots) {
    ...
    const ev = await page.evaluate(() => {
      ...
      // Selected candidate (if any) reflects which QDT SKU the slot resolved to.
    ...
    // Acceptance rules:
    //  - Every non-empty AMS slot must reduce to a selected QDT candidate;
    ...
    //    today every QIDI OEM AMS slot encodes a recognised SKU.
    ...
    //  - When the selected candidate has a non-hex color_code (i.e. real QDT
