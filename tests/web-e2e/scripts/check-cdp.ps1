<#
.SYNOPSIS
    P0 verification: confirm we can attach Playwright to QIDIStudio's
...
.PARAMETER StudioBin
    Absolute path to qidi-studio.exe.  Defaults to STUDIO_E2E_BIN or the
    repo's Release build output.

.PARAMETER LaunchStudio
    When set ($true is default), the script spawns QIDIStudio for you.
...
.PARAMETER Mode
    Three-stage execution control:
      ...
      probe               — assume QIDIStudio is already running with the
...
if ([string]::IsNullOrWhiteSpace($StudioBin)) {
    ...
    if ([string]::IsNullOrWhiteSpace($StudioBin)) {
        ...
        $StudioBin = Join-Path $repoRoot 'build_release\src\Release\qidi-studio.exe'
    }
}
...
Write-Host "  qidi-studio.exe : $StudioBin"
...
if ($Mode -ne 'probe') {
    if (-not (Test-Path $StudioBin)) {
        Write-Host ''
        Write-Host "[FAIL] qidi-studio.exe not found at the path above." -ForegroundColor Red
    ...
    if (Test-Port-Open -P $Port) {
        Write-Host ''
        Write-Host "[FAIL] Port $Port is already in use before QIDIStudio starts." -ForegroundColor Red
    ...
    if ($LaunchStudio) {
        Write-Host ''
        Write-Host '[step 2/4] Starting QIDIStudio.exe ...' -ForegroundColor Green
        ...
        Write-Host '           Start QIDIStudio.exe FROM THIS SHELL only - opening it from'
        Write-Host '           Explorer will spawn a fresh process without the env var.'
    }

    if ($Mode -eq 'launch') {
        Write-Host ''
        Write-Host '[launch] QIDIStudio is starting in the background.' -ForegroundColor Green
    ...
    Write-Host ''
    Write-Host '[step 3/4] Drive QIDIStudio:' -ForegroundColor Green
    ...
    Write-Host ''
    Write-Host '[probe] Skipping launch + Read-Host; assuming QIDIStudio is up.' -ForegroundColor Yellow
}
...
if (-not $ok) {
    ...
    Write-Host "       This usually means QIDIStudio overrides" -ForegroundColor Red
