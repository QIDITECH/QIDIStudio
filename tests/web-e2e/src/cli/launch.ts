 ...
 *
 *   1. confirm qidi-studio.exe path
...
function resolveConfig(): ResolvedConfig {
  ...
  const studioBin =
    ...
    path.resolve(E2E_ROOT, '..', '..', 'build_release', 'src', 'Release', 'qidi-studio.exe')
...
async function step1_confirmBin(cfg: ResolvedConfig): Promise<string> {
  banner('Step 1/4 \u00b7 Locate qidi-studio.exe')
  if (!fs.existsSync(cfg.studioBin)) {
    process.stderr.write(chalk.red(`  qidi-studio.exe not found at: ${cfg.studioBin}\n`))
    const newPath = await askInput({
      message: 'Enter the full path to qidi-studio.exe:',
...
async function step2_startOrAttach(
  ...
  banner('Step 2/4 \u00b7 Start QIDIStudio (or attach to a running one)')
  const mode = await select({
    message: 'How should we get the QIDIStudio process?',
    choices: [
  ...
  if (mode === 'attach') {
    process.stdout.write(
      chalk.yellow(
        `  Attaching to an existing QIDIStudio on CDP port ${cdpPort}.\n` +
  ...
  process.stdout.write(chalk.gray('  Spawning QIDIStudio...\n'))
...
async function step3_waitForReady(): Promise<void> {
  banner(
    'Step 3/4 \u00b7 Drive QIDIStudio (manual)\n\n' +
...
async function main(): Promise<void> {
  ...
  banner(
    `QIDIStudio Filament Manager \u00b7 E2E launcher\n\n` +
