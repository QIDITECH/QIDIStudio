 ...
 *     which is loaded from a custom scheme (`qidistudio://...`) inside
...
// Confirmed against a real QIDIStudio Release build (2026-05): the
...
async function findFilamentPage(
  ...
  throw new CdpAttachError(
    'Could not locate the filament manager page within the CDP target list. ' +
      'Make sure you actually opened it in QIDIStudio (Device -> Filament Manager) ' +
  ...
  try {
    ...
    throw new CdpAttachError(
      `chromium.connectOverCDP(${endpoint}) failed: ${(err as Error).message}. ` +
        `Confirm QIDIStudio was launched with WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS set.`,
    )
  }
