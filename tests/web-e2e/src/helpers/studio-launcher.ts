/**
 * QIDIStudio process launcher.
 *
 * Spawns qidi-studio.exe with WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS set so
...
export interface LaunchOptions {
  /** Absolute path to qidi-studio.exe. */
  bin: string
...
async function waitForPort(
  ...
  throw new StudioLaunchError(
    `WebView2 CDP port ${port} did not open within ${timeoutMs} ms. ` +
      `Either QIDIStudio failed to start or it is not honoring ` +
...
export async function launchStudio(opts: LaunchOptions): Promise<StudioHandle> {
  ...
  if (!fs.existsSync(bin)) {
    throw new StudioLaunchError(`qidi-studio.exe not found: ${bin}`)
  }
  if (await isPortOpen(cdpPort)) {
    throw new StudioLaunchError(
      `CDP port ${cdpPort} is already in use before QIDIStudio starts. ` +
  ...
  if (!proc.pid) {
    throw new StudioLaunchError('Failed to obtain a PID for the spawned QIDIStudio process.')
  }
