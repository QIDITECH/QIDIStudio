...
test.describe('STUDIO-17977 brand/material switch alignment @studio-17977 @webview-only', () => {
  test('switching material rebases the color to a valid candidate', async ({ filamentList }) => {
    ...
    if (realMaterials.length < 2) {
      test.skip(true, 'QIDI has fewer than 2 materials in this account; nothing to switch between.')
    }
    ...
    // from the user-owned cloud library, not the QDT official catalogue)
    // are intentionally NOT shown in the preview's `preview-fila-code`
    // slot — that slot is reserved for real QDT fila codes. A hex-only
    ...
    //   - QDT code (digits / mixed): expect filaCode to equal it
