 ...
 *   - selecting one updates the preview bar with name + official QDT
 ...
 * library has at least one official QIDI material registered.
 */
import { test, expect } from '../src/fixtures/filament'

const OFFICIAL_BRAND = 'QIDI'

test.describe('STUDIO-17977 official color query @studio-17977 @webview-only', () => {
  test('QIDI + first available material surfaces a non-empty candidate list', async ({ filamentList }) => {
    ...
    test.skip(materials.length === 0, 'No materials registered for QIDI in this library.')
    ...
    expect(
      candidates.length,
      `official QDT palette should not be empty for "${firstMaterial}"`,
    ).toBeGreaterThan(0)
  ...
  test('picking a candidate fills name + official code + hex list in preview bar', async ({ filamentList }) => {
    ...
    expect(preview.filaCode, 'official QDT code').toBe(first.colorCode)
