...
1. 启动带 WebView2 CDP 的 QIDIStudio。
...
9. 在表单中选择第一个非 \`QIDI\` 的第三方品牌，以及该品牌下第一个可用 material。
10. 点击 \`Add\`。
...
test.describe('STUDIO-18340 real AMS third-party import dry-run @studio-18340 @printer', () => {
  ...
  test('STUDIO-18340 real AMS import submits update for existing RFID and preserves third-party setting_id', async ({
    ...
    const thirdPartyBrand = brands.find((brand) => brand && !/qidi\s*lab/i.test(brand))
    expect(
      thirdPartyBrand,
      `Precondition failed: need a non-QIDI brand in the real brand dropdown. Brands: ${brands.join(', ')}`,
    ).toBeTruthy()
